...
FILE_VERSION_STR = "1.0.0"

IDENT_PREFIX = "Allen Institute, AI file " + FILE_VERSION_STR + ": "
...
if TESTING:
	...
	if TEST_N < n:
		n = TEST_N
	print "Processing %d sweeps" % TEST_N
else:
...
#for i in range(len(sweep_stimuli)):
#	print "%d\t%s:%s" % (i, sweep_stimuli[i]["stim"], sweep_stimuli[i]["scalefactor"])
#
#sys.exit(1)
...
for i in range(len(sweep_num_list)):
	...
	stim = sweep_stimuli[swp_num]["stim"]
	scale_factor = sweep_stimuli[swp_num]["scalefactor"]
	desc = "Application of stimulus %s:%f" % (stim, scale_factor)
	...
	epoch_insert_sequence(epo, "acq_voltage", t0, t1, idx0, idx1, desc, seq)
	# add stim sequence
	seq = h_stimulus["presentation"][swp_name]
	desc = "%s test-pulse current" % swp_name
	epoch_insert_sequence(epo, "stim_current", t0, t1, idx0, idx1, desc, seq)

#	seq = h_acq_sequences[swp_name]
#
#	stim_cur["sequence"] = h_stimulus["presentation"][swp_name]
#	desc = "%s test-pulse current" % swp_name
#	epoch_insert_seq(epo, "stim_current", t0, t1, idx0, idx1, desc, seq)
#
#	acq_volts = epo.create_group("acq_voltage")
#	acq_volts.create_dataset("t_start", data=t0)
#	acq_volts.create_dataset("idx_start", data=int(0))
#	acq_volts.create_dataset("t_stop", data=t1)
#	acq_volts.create_dataset("idx_stop", data=samples-1)
#	acq_volts["sequence"] = h_acq_sequences[swp_name]
#	# add stimulus to epoch
#	stim_cur = epo.create_group("stim_current")
#	stim_cur.create_dataset("t_start", data=t0)
#	stim_cur.create_dataset("idx_start", data=int(0))
#	stim_cur.create_dataset("t_stop", data=t1)
#	stim_cur.create_dataset("idx_stop", data=samples-1)
#	stim_cur["sequence"] = h_stimulus["presentation"][swp_name]
