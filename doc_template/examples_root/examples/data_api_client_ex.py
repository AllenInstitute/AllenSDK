...
associations = "".join(["[id$eq1]", "structure_graph(ontology),", "graphic_group_labels"])
...
structures = pd.DataFrame(rma.model_query("Structure", criteria="[graph_id$eq1]", num_rows="all"))
...
summary_structures = pd.DataFrame(
    rma.model_query("Structure", criteria=criteria_string, include=include_string, num_rows="all")
)
ontologies = pd.DataFrame(list(summary_structures.ontology)).drop_duplicates()
...
summary_structures[["id", "parent_structure_id", "acronym"]].to_csv(
    "summary_structures.csv", index_label="structure_id"
)
...
for id, name, parent_structure_id in summary_structures[["name", "parent_structure_id"]].itertuples():
...
cache_writer = Cache()
do_cache = True
structures_from_api = cache_writer.wrap(
    rma.model_query, path="summary.csv", cache=do_cache, model="Structure", criteria="[graph_id$eq1]", num_rows="all"
)
