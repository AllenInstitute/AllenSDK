...
ephys_sweep = next(s for s in ephys_sweeps if s["stimulus_units"] == "Amps")
...
if grid_spike_indices is not None:
    plt.plot(interpolated_spike_times, interpolated_spike_voltages, "x", label="interpolated spike")

    plt.plot((grid_spike_indices - 1) * neuron.dt, voltage[grid_spike_indices - 1], ".", label="last step before spike")
...
method = neuron.configure_method("custom", custom_voltage_reset_rule, {"custom_param_a": 0.1, "custom_param_b": 0.0})
