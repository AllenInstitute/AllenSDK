...
parser.add_argument("--data_path", help="dir containing eye tracking data", required=True)
...
def main():
    ...
    if len(files) > 0:
        ...
        for exp_id in exp_ids:
            ...
            os.rename(Path(args.data_path) / f"{exp_id}.npy", Path(args.data_path) / exp_id / "eye_tracking.npy")
...
def _write_metadata():
    exp_ids = [x for x in os.listdir(args.data_path) if (Path(args.data_path) / x).is_dir()]
