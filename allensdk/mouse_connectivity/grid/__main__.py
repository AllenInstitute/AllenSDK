...
def run_grid(args):
    ...
    input_dimensions = [
        sub_images[0]["dimensions"]["column"],
        sub_images[0]["dimensions"]["row"],
        args["sub_image_count"],
    ]

    input_spacing = [
        sub_images[0]["spacing"]["column"],
    ...
    logging.info(
        "{} sub images with indices: {}".format(len(sub_images), [si["specimen_tissue_index"] for si in sub_images])
    )

    output_dimensions = [
        args["reference_dimensions"]["slice"],
        args["reference_dimensions"]["row"],
        args["reference_dimensions"]["column"],
    ]

    output_spacing = [
        args["reference_spacing"]["slice"],
