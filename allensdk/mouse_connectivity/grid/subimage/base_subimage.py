...
class SubImage(object):
    ...
    def __init__(self, reduce_level, in_dims, in_spacing, coarse_spacing, *args, **kwargs):
        ...
        self.blocks, self.coarse_dims = iu.grid_image_blocks(self.in_dims, self.in_spacing, self.coarse_spacing)
    ...
    def make_pixel_counter(self):
        ...
        return functools.partial(
            iu.block_apply, out_shape=self.coarse_dims, dtype=np.float32, blocks=self.blocks, fn=fn
        )
...
class SegmentationSubImage(SubImage):
    ...
    def __init__(self, reduce_level, in_dims, in_spacing, coarse_spacing, segmentation_paths, *args, **kwargs):
        super(SegmentationSubImage, self).__init__(reduce_level, in_dims, in_spacing, coarse_spacing, *args, **kwargs)
    ...
    def extract_signal_from_segmentation(self, segmentation_name="segmentation", signal_name="signal"):
        """
    ...
    def extract_injection_from_segmentation(self, segmentation_name="segmentation", injection_name="injection"):
        """
...
class IntensitySubImage(SubImage):
    ...
    def __init__(self, reduce_level, in_dims, in_spacing, coarse_spacing, intensity_paths, *args, **kwargs):
        super(IntensitySubImage, self).__init__(reduce_level, in_dims, in_spacing, coarse_spacing, *args, **kwargs)
...
class PolygonSubImage(SubImage):
    ...
    def __init__(self, reduce_level, in_dims, in_spacing, coarse_spacing, polygon_info, *args, **kwargs):
        super(PolygonSubImage, self).__init__(reduce_level, in_dims, in_spacing, coarse_spacing, *args, **kwargs)
    ...
    def get_polygons(self):
        ...
        for key in polygon_keys:
            ...
            self.images[key] = iu.rasterize_polygons(
                self.in_dims.astype(int)[::-1], [1.0 / 2**self.reduce_level, 1.0 / 2**self.reduce_level], points
            ).T
