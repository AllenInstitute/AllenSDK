...
class ImageSeriesGridder(object):
    ...
    def set_coarse_grid_parameters(self):
        self.coarse_dims, self.coarse_spacing, self.coarse_grid_radius = iu.compute_coarse_parameters(
            self.in_dims, self.in_spacing, self.out_spacing[::-1], self.reduce_level
        )
    ...
    def resample_volume(self, key):
        ...
        self.volumes[key] = iu.resample_volume(self.volumes[key], self.out_dims, self.out_spacing, None, self.transform)
