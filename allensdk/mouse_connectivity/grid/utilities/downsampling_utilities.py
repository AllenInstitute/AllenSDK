...
def apply_divisions(image, window_size):
    for axis in range(image.ndim):
        slc = tuple(
            [slice(window_size - 1, None, window_size) if ii == axis else slice(0, None) for ii in range(image.ndim)]
        )
...
def extract(image, factor, window_size, window_step, output_shape):
    ...
    for case in it.product(*([[0, 1]] * image.ndim)):
        inp = tuple(
            [
                slice(window_size - 2, None, window_step) if not ii else slice(window_size, None, window_step)
                for ii in case
            ]
        )
