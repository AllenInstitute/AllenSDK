...
class InputParameters(ArgSchema):
    ...
    image_series_slice_spacing = Float(
        required=True, help="Distance (microns) between successive images in this series."
    )
    target_spacings = List(
        Float, required=True, help="For each volume produced, downsample to this isometric resolution"
    )
    ...
    filter_bit = Int(
        default=None,
        allow_none=True,
        help="if provided, signals that pixels with this bit high have passed the optional post-filter stage",
    )
