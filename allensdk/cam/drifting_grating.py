...
class DriftingGrating(OPAnalysis):    
    ...
    def getPeak(self):
        ...
        peak = pd.DataFrame(index=range(self.numbercells), columns=('Ori','TF','response_variability','OSI','DSI','peak_DFF','ptest', 'p_run','run_modulation'))
        ...
        for nc in range(self.numbercells):
            ...
            peak.response_variability.iloc[nc] = self.response[prefori, preftf, nc, 2]/0.15
            ...
            peak.OSI.iloc[nc] = (pref-orth)/(pref+orth)
            peak.DSI.iloc[nc] = (pref-null)/(pref+null)
            peak.peak_DFF.iloc[nc] = pref
            ...
            if (len(subset_run)>2) & ( len(subset_stat)>2):
                (f, peak.p_run.iloc[nc]) = st.ks_2samp(subset_run[str(nc)], subset_stat[str(nc)])
                peak.run_modulation.iloc[nc] = subset_run[str(nc)].mean()/subset_stat[str(nc)].mean()
            else:
                peak.p_run.iloc[nc] = np.NaN
                peak.run_modulation.iloc[nc] = np.NaN
        return peak
