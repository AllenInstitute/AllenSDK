# -*- coding: utf-8 -*-
"""
Created on Fri Jan 15 23:10:34 2016
        ...
        for ori in self.orivals:
            ...
            for tf in self.tfvals[1:]:
                ...
                try:
                    ...
                    subset_response_p =  subset_response.iloc[:,nc].apply(np.mean) + (subset_response.iloc[:,nc].apply(np.std) / subset_response.iloc[:,nc].apply(len))
                    subset_response_n =  subset_response.iloc[:,nc].apply(np.mean) - (subset_response.iloc[:,nc].apply(np.std) / subset_response.iloc[:,nc].apply(len))
...
#                    ax = plt.subplot2grid((13,11),(sf_pt,))
                    ax.fill_between(xtime, subset_response_p, subset_response_n, color='b', alpha=0.5)
                    ax.fill_between(xtime, blank_p, blank_n, color='k', alpha=0.5)                 
                    ax.plot(xtime, subset_response[str(nc)].mean(), color='b', lw=2)                 
                    ax.plot(xtime, blank[str(nc)].mean(), color='k', lw=2)
...
def plot_3SA(dg, nm1, nm3):
    ...
    for nc in range(dg.numbercells):
        ...
        for i in range(len(test)-1): 
            ax1.axvspan(xmin=(dg.stim_table.Start.iloc[test[i]].values/dg.acquisition_rate), xmax=(dg.stim_table.End.iloc[test[i+1]-1].values/dg.acquisition_rate), color='gray', alpha=0.3)
        ax1.axvspan(xmin=nm1.stim_table.Start.min()/nm1.acquisition_rate, xmax=((nm1.stim_table.Start.max()+nm1.sweeplength)/nm1.acquisition_rate), color='red', alpha=0.3)
        dif = np.ediff1d(nm3.stim_table.Start.values, to_begin=8000, to_end=8000)
        test = np.argwhere(dif>5000)
        for i in range(len(test)-1): 
            ax1.axvspan(xmin=(nm3.stim_table.Start.iloc[test[i]].values/nm3.acquisition_rate), xmax=((nm3.stim_table.End.iloc[test[i+1]-1].values+nm3.sweeplength)/nm3.acquisition_rate), color='blue', alpha=0.3)
        ax1.set_xlabel("Time (s)", fontsize=20)
        ax1.set_ylabel("Fluorescence", fontsize=20)
        ...
        smax = nm1.binned_cells_sp[nc,np.argmax(nm1.binned_cells_sp[nc,:,0]),0] + nm1.binned_cells_sp[nc,np.argmax(nm1.binned_cells_sp[nc,:,0]),1]
        vmax = nm1.binned_cells_vis[nc,np.argmax(nm1.binned_cells_vis[nc,:,0]),0] + nm1.binned_cells_vis[nc,np.argmax(nm1.binned_cells_vis[nc,:,0]),1]
        rmax = np.where(smax>vmax, smax, vmax)
        smin = nm1.binned_cells_sp[nc,np.argmin(nm1.binned_cells_sp[nc,:,0]),0] - nm1.binned_cells_sp[nc,np.argmin(nm1.binned_cells_sp[nc,:,0]),1]
        vmin = nm1.binned_cells_vis[nc,np.argmin(nm1.binned_cells_vis[nc,:,0]),0] - nm1.binned_cells_vis[nc,np.argmin(nm1.binned_cells_vis[nc,:,0]),1]
        rmin = np.where(smin<vmin, smin, vmin)      
        
        ax4.errorbar(nm1.binned_dx_sp[:,0], nm1.binned_cells_sp[nc,:,0], yerr=nm1.binned_cells_sp[nc,:,1], fmt='.', color='k')
        ...
        ax5.errorbar(nm1.binned_dx_vis[:,0], nm1.binned_cells_vis[nc,:,0], yerr=nm1.binned_cells_vis[nc,:,1], fmt='.')
        ...
        ax6.annotate(str(dg.tfvals[peaktf]) + " Hz", xy=(0,0.9), xycoords='axes fraction', fontsize=14)
        ...
        ax7.errorbar(range(5), dg.response[peakori,1:,nc,0], yerr=dg.response[peakori,1:,nc,1],  fmt='bo-', lw=2)
        ax7.fill_between(range(5), np.repeat(dg.response[0,0,nc,0]+dg.response[0,0,nc,1], 5), np.repeat(dg.response[0,0,nc,0]-dg.response[0,0,nc,1], 5), color='gray', alpha=0.5)
        ax7.axhline(y=dg.response[0,0,nc,0], ls='--', color='k', lw=2)
        ax7.annotate(str(dg.orivals[peakori]) + " Deg", xy=(0,0.9), xycoords='axes fraction', fontsize=14)
        ...
        subset = dg.sweep_response[(dg.stim_table.Orientation==dg.orivals[peakori])&(dg.stim_table.Temporal_frequency==dg.tfvals[peaktf])]
        ...
        ax8.set_xlim(-1,3)
        ax8.annotate(str(dg.orivals[peakori])+" Deg / "+str(dg.tfvals[peaktf])+" Hz", xy=(0,0.9), xycoords='axes fraction', fontsize=14)
        ax8.set_xlabel("Time (s)", fontsize=20)
        ax8.set_ylabel("DF/F (%)", fontsize=20)
        ax8.axvspan(0, dg.sweeplength/dg.acquisition_rate ,ymin=0, ymax=1, facecolor='gray', alpha=0.3)
        ax8.yaxis.set_major_locator(MaxNLocator(6))
        ax8.set_title("Trial responses to prefered ori/tf", fontsize=20)   
        
        im = ax9.imshow(dg.response[:,1:,nc,0], cmap='gray', interpolation='none')
        ...
        xtime = np.arange(0, nm1.sweeplength/nm1.acquisition_rate, 1/nm1.acquisition_rate)
        ...
        xtime = np.arange(0, nm3.sweeplength/nm3.acquisition_rate, 1/nm3.acquisition_rate)
...
def plot_3SC(lsn, nm1, nm2):
    ...
    for nc in range(lsn.numbercells):
        ...
        dif = np.ediff1d(lsn.stim_table.Start.values, to_begin=8000, to_end=8000)
        test = np.argwhere(dif>5000)
        ax1.plot(xtime, lsn.celltraces[nc,:])        
        for i in range(len(test)-1): 
            ax1.axvspan(xmin=(lsn.stim_table.Start.iloc[test[i]].values/lsn.acquisition_rate), xmax=(lsn.stim_table.End.iloc[test[i+1]-1].values/lsn.acquisition_rate), color='gray', alpha=0.3)
        ...
        ax1.axvspan(nm2.stim_table.Start.min()/nm2.acquisition_rate, nm2.stim_table.End.max()/nm2.acquisition_rate, ymin=0, ymax=1, color='green', alpha=0.3)
        ax1.set_xlabel("Time (s)", fontsize=20)
        ax1.set_ylabel("Fluorescence", fontsize=20)
        ...
        ax12.errorbar(nm1.binned_dx_sp[:,0], nm1.binned_cells_sp[nc,:,0], yerr=nm1.binned_cells_sp[nc,:,1], fmt='.', color='k')
        ax12.set_ylim(rmin, rmax)        
        ax12.set_xlabel("Speed (cm/s)", fontsize=20)
        ax12.set_ylabel("DF/F", fontsize=20)
        ax12.set_title("Spontaneous", fontsize=20)
        
        ax13.errorbar(nm1.binned_dx_vis[:,0], nm1.binned_cells_vis[nc,:,0], yerr=nm1.binned_cells_vis[nc,:,1], fmt='.')
        ...
        xtime = np.arange(0, nm2.sweeplength/nm2.acquisition_rate, 1/nm2.acquisition_rate)
        ...
        vMax = np.where(np.amax(lsn.receptive_field[:,:,nc,0])>np.amax(lsn.receptive_field[:,:,nc,1]), np.amax(lsn.receptive_field[:,:,nc,0]), np.amax(lsn.receptive_field[:,:,nc,1]))
        vMin = np.where(np.amin(lsn.receptive_field[:,:,nc,0])<np.amin(lsn.receptive_field[:,:,nc,1]), np.amin(lsn.receptive_field[:,:,nc,0]), np.amin(lsn.receptive_field[:,:,nc,1]))

        imon = ax7.imshow(lsn.receptive_field[:,:,nc,0], cmap='gray', interpolation='None', vmin=vMin, vmax=vMax)
        ...
        imoff = ax8.imshow(lsn.receptive_field[:,:,nc,1], cmap='gray', interpolation='None', vmin=vMin, vmax=vMax)
        ...
        zon = (lsn.receptive_field[:,:,nc,0]-np.mean(lsn.receptive_field[:,:,nc,0]))/np.std(lsn.receptive_field[:,:,nc,0])
        zon = np.where(abs(zon)>2, zon, 0)
        Vmax_on = np.where(abs(np.amax(zon))>abs(np.amin(zon)), np.amax(zon), -1*np.amin(zon))
        zoff = (lsn.receptive_field[:,:,nc,1]-np.mean(lsn.receptive_field[:,:,nc,1]))/np.std(lsn.receptive_field[:,:,nc,1])
        zoff = np.where(abs(zoff)>2, zoff, 0)
        Vmax_off = np.where(abs(np.amax(zoff))>abs(np.amin(zoff)), np.amax(zoff), -1*np.amin(zoff))
        Vmax = np.where(Vmax_on>Vmax_off, Vmax_on, Vmax_off)
        imzon = ax9.imshow(zon, cmap='RdBu_r', interpolation='none', vmin=-1*Vmax, vmax=Vmax)
        ...
        zoff = (lsn.receptive_field[:,:,nc,1]-np.mean(lsn.receptive_field[:,:,nc,1]))/np.std(lsn.receptive_field[:,:,nc,1])
        zoff = np.where(abs(zoff)>2, zoff, 0)
        imzoff = ax10.imshow(zoff, cmap='RdBu', interpolation='none', vmin=-1*Vmax, vmax=Vmax)
...
def plot_3SB(sg, nm1, ni):
    ...
    for nc in range(sg.numbercells):
        ...
        dif = np.ediff1d(sg.stim_table.Start.values, to_begin=8000, to_end=8000)
        test = np.argwhere(dif>5000)
        ...
        ax3.errorbar(sg.orivals, sg.response[:,peaksf,3,nc,0], yerr=sg.response[:,peaksf,3,nc,1], color='lightskyblue', fmt='o-', lw=2)
        ax3.fill_between(sg.orivals, np.repeat(sg.response[0,0,0,nc,0]+sg.response[0,0,0,nc,1], sg.number_ori), np.repeat(sg.response[0,0,0,nc,0]-sg.response[0,0,0,nc,1], sg.number_ori), color='gray', alpha=0.5)
        ...
        ax4.errorbar(range(5), sg.response[peakori,1:,0,nc,0], yerr=sg.response[peakori,1:,0,nc,1], color='blue', fmt='o-', lw=2)
        ax4.errorbar(range(5), sg.response[peakori,1:,1,nc,0], yerr=sg.response[peakori,1:,1,nc,1], color='cornflowerblue', fmt='o-', lw=2)
        ax4.errorbar(range(5), sg.response[peakori,1:,2,nc,0], yerr=sg.response[peakori,1:,2,nc,1], color='steelblue', fmt='o-', lw=2)
        ax4.errorbar(range(5), sg.response[peakori,1:,3,nc,0], yerr=sg.response[peakori,1:,3,nc,1], color='lightskyblue', fmt='o-', lw=2)
        ax4.fill_between(range(5), np.repeat(sg.response[0,0,0,nc,0]+sg.response[0,0,0,nc,1], 5), np.repeat(sg.response[0,0,0,nc,0]-sg.response[0,0,0,nc,1], 5), color='gray', alpha=0.5)
        ...
        try:
            ...
            xtime = xtime[:-1]
            ax13.fill_between(xtime, subset_p, subset_n, color='b', alpha=0.5)
        blank = sg.sweep_response[(sg.stim_table.Orientation==0)&(sg.stim_table.Spatial_frequency==0)&(sg.stim_table.Phase==0)]
        ...
        ax13.axvspan(0, sg.sweeplength/sg.acquisition_rate ,ymin=0, ymax=1, facecolor='gray', alpha=0.3)        
        ax13.yaxis.set_major_locator(MaxNLocator(4)) 
        ax13.set_xlabel("Time (s)", fontsize=20)
        ax13.set_ylabel("DF/F (%)", fontsize=20)
        
        Vmax = sg.response[:,1:,:,nc,0].max()
        ax5.imshow(sg.response[:,1:,0,nc,0], cmap='gray', interpolation='none', vmin=0, vmax=Vmax)
        ...
        ax6.imshow(sg.response[:,1:,1,nc,0], cmap='gray', interpolation='none', vmin=0, vmax=Vmax)
        ...
        ax7.imshow(sg.response[:,1:,2,nc,0], cmap='gray', interpolation='none', vmin=0, vmax=Vmax)
        ...
        ax8.imshow(sg.response[:,1:,3,nc,0], cmap='gray', interpolation='none', vmin=0, vmax=Vmax)
...
def ExperimentSummary(self):
    ...
    try:
        ax7.errorbar(self.run_modulation['stationary_mean'][self.significant_cells], self.run_modulation['run_mean'][self.significant_cells], xerr=self.run_modulation['stationary_sem'][self.significant_cells], yerr=self.run_modulation['run_sem'][self.significant_cells], fmt='o')
        ax7.set_xlabel("Stationary (% DF/F)", fontsize=14)
        ax7.set_ylabel("Running (% DF/F)", fontsize=14)
        maxval = np.where(np.amax(self.run_modulation['stationary_mean'])>np.amax(self.run_modulation['run_mean']), np.amax(self.run_modulation['stationary_mean']), np.amax(self.run_modulation['run_mean']))
        minval = np.where(np.amin(self.run_modulation['stationary_mean'])<np.amin(self.run_modulation['run_mean']), np.amin(self.run_modulation['stationary_mean']), np.amin(self.run_modulation['run_mean']))
        ax7.set_xlim(minval, maxval)
        ax7.set_ylim(minval, maxval)
        ax7.set_title("Running modulation at peak", fontsize=14)
    except:
        pass
    ...
    plt.tight_layout()  
    plt.suptitle(self.LIMSID + "  " + self.startdatetime, fontsize=20)
...
def plot_Running_A(dg, nm1, nm3):
    ...
    run_peak = np.where(dg.response[:,1:,nc,0] == np.nanmax(dg.response[:,1:,nc,0]))
    peakori = run_peak[0][0]
    peaktf = run_peak[1][0]+1
    
    ax3.errorbar(dg.orivals, dg.response[:,peaktf,nc,0], yerr=dg.response[:,peaktf,nc,1], fmt='b.-', lw=2)
    ax3.fill_between(dg.orivals, np.repeat(dg.response[0,0,nc,0]+dg.response[0,0,nc,1], dg.number_ori), np.repeat(dg.response[0,0,nc,0]-dg.response[0,0,nc,1], dg.number_ori), color='gray', alpha=0.5)
    ax3.axhline(y=dg.response[0,0,nc,0], ls='--', color='k')
    ax3.annotate(str(dg.tfvals[peaktf]) + " Hz", xy=(0,0.9), xycoords='axes fraction', fontsize=14)
    ax3.set_xtick = (dg.orivals)
    ax3.set_xlabel("Direction (deg)", fontsize=20)
    ax3.set_ylabel("Speed (cm/s)", fontsize=20)
    ax3.yaxis.set_major_locator(MaxNLocator(6))
    
    ax4.errorbar(dg.tfvals[1:], dg.response[peakori,1:,nc,0], yerr=dg.response[peakori,1:,nc,1], fmt='b.-', lw=2)
    ax4.fill_between(dg.tfvals[1:], np.repeat(dg.response[0,0,nc,0]+dg.response[0,0,nc,1], dg.number_tf-1), np.repeat(dg.response[0,0,nc,0]-dg.response[0,0,nc,1], dg.number_tf-1), color='gray', alpha=0.5)
    ax4.axhline(y=dg.response[0,0,nc,0], ls='--', color='k')  
    ax4.annotate(str(dg.orivals[peakori]) + " Deg", xy=(0,0.9), xycoords='axes fraction', fontsize=14)
    ax4.set_xticks = (dg.tfvals[1:])
    ax4.set_xlabel("Temporal frequency (Hz)", fontsize=20)
    ax4.yaxis.set_major_locator(MaxNLocator(6))
    
    im = ax5.imshow(dg.response[:,1:,nc,0], cmap='gray', interpolation='none')
