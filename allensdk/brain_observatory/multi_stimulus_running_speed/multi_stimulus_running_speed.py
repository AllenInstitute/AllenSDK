...
class MultiStimulusRunningSpeed(argschema.ArgSchemaParser):
    ...
    def process(self):
        ...
        bstim = BehaviorStimulusFile.from_json(dict_repr={"behavior_stimulus_file": self.args["behavior_pkl_path"]})

        mstim = MappingStimulusFile.from_json(dict_repr={"mapping_stimulus_file": self.args["mapping_pkl_path"]})

        rstim = ReplayStimulusFile.from_json(dict_repr={"replay_stimulus_file": self.args["replay_pkl_path"]})

        (velocities, raw_data) = multi_stim_running_df_from_raw_data(
