...
class MultiStimulusRunningSpeedInputParameters(argschema.ArgSchema):
    output_path = argschema.fields.OutputFile(required=True, description="The location to write the output file")
    ...
    use_lowpass_filter = argschema.fields.Bool(
        required=True, default=True, description=("apply a low pass filter to the running speed results")
    )
...
class MultiStimulusRunningSpeedOutputParameters(MultiStimulusRunningSpeedOutputSchema):
    output_path = argschema.fields.OutputFile(required=True, help="Filtered running speed hdf5 output file.")
