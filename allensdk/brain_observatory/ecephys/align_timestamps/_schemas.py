...
class ProbeMappable(DefaultSchema):
    name = String(required=True, help='What kind of mappable data is this? e.g. \"spike_timestamps\"')
    input_path = String(
        required=True, 
...
class ProbeInputParameters(DefaultSchema):
    ...
    sampling_rate = Float(required=True, help='The sampling rate of the probe, in Hz, assessed on the probe clock.')
    lfp_sampling_rate = Float(required=True, help='The sampling rate of the LFP collected on this probe.')
    start_index = Int(default=0, help='Sample index of probe recording start time. Defaults to 0.')
...
class InputParameters(ArgSchema): 
    probes = Nested(ProbeInputParameters, many=True, help='Probes whose data will be aligned to the master clock.')
...
class ProbeOutputParameters(DefaultSchema):
    name = String(required=True, help='Identifier for this probe')
    output_paths = Dict(required=True, help='Paths of each mappable file written by this run of the module.')
    total_time_shift = Float(required=True, help='Translation (in seconds) from master->probe times computed for this probe.')
    global_probe_sampling_rate = Float(required=True, help='The sampling rate of this probe in Hz, assessed on the master clock.')
    global_probe_lfp_sampling_rate = Float(required=True, help='The sampling rate of LFP collected on this probe in Hz, assessed on the master clock.')
