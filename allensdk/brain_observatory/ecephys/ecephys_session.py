...
NON_STIMULUS_PARAMETERS = tuple(["start_time", "stop_time", "duration", "stimulus_block", "stimulus_condition_id"])
...
class EcephysSession(LazyPropertyMixin):
    ...
    def __init__(self, api: EcephysSessionApi, test: bool = False, **kwargs):
        ...
        self._stimulus_presentations = self.LazyProperty(
    ...
    def _get_valid_time_points(self, time_points, invalid_time_intevals):
        all_time_points = xr.DataArray(
            name="time_points", data=[True] * len(time_points), dims=["time"], coords=[time_points]
        )

        valid_time_points = all_time_points
        for ix, invalid_time_interval in invalid_time_intevals.iterrows():
            invalid_time_points = (time_points >= invalid_time_interval["start_time"]) & (
    ...
    def get_inter_presentation_intervals_for_stimulus(self, stimulus_names):
        ...
        stimulus_names = coerce_scalar(
            stimulus_names,
            "expected stimulus_names to be a collection (list-like), "
            f"but found {type(stimulus_names)}: {stimulus_names}",
        )
        filtered_presentations = self.stimulus_presentations[
            self.stimulus_presentations["stimulus_name"].isin(stimulus_names)
        ]
    ...
    def get_stimulus_table(
        self, stimulus_names=None, include_detailed_parameters=False, include_unused_parameters=False
    ):
    ...
    def get_stimulus_epochs(self, duration_thresholds=None):
        ...
        for key, threshold in duration_thresholds.items():
            epochs = epochs[(epochs["stimulus_name"] != key) | (epochs["duration"] >= threshold)]
    ...
    def presentationwise_spike_counts(
        ...
        domain = build_time_window_domain(
            bin_edges, stimulus_presentations["start_time"].values, callback=time_domain_callback
        )
        ...
        if len(out_of_order[0]) > 0:
            ...
            raise ValueError(
                f"The time domain specified contains out-of-order bin edges at indices: {out_of_order_time_bins}"
            )
        ...
        if len(overlapping) > 0:
            ...
            warnings.warn(
                "You've specified some overlapping time intervals "
                f"between neighboring rows: {overlapping}, "
                "with a maximum overlap of"
                f" {np.abs(np.min(time_diffs))} seconds."
            )

        tiled_data = build_spike_histogram(domain, self.spike_times, units.index.values, dtype=dtype, binarize=binarize)
    ...
    def presentationwise_spike_times(self, stimulus_presentation_ids=None, unit_ids=None):
        ...
        if not spike_times:
            ...
            # empty dataframe
            return pd.DataFrame(
                columns=["spike_times", "stimulus_presentation", "unit_id", "time_since_stimulus_presentation_onset"]
            )

        pres_ids = np.concatenate(presentation_ids).astype(int)

        spike_df = pd.DataFrame(
            {"stimulus_presentation_id": pres_ids, "unit_id": np.concatenate(unit_ids).astype(int)},
            index=pd.Index(np.concatenate(spike_times), name="spike_time"),
        )

        # Add time since stimulus presentation onset
        onset_times = self._filter_owned_df("stimulus_presentations", ids=all_presentation_ids)["start_time"]
    ...
    def conditionwise_spike_statistics(self, stimulus_presentation_ids=None, unit_ids=None, use_rates=False):
        ...
        stimulus_presentation_ids = (
            ...
            else self.stimulus_presentations.index.values
        )  # In case
        ...
        if spikes.empty:
            ...
            spike_counts = pd.DataFrame(
                ...
                index=pd.MultiIndex.from_product(
                    [stimulus_presentation_ids, unit_ids], names=["stimulus_presentation_id", "unit_id"]
                ),
            )
    ...
    def _build_stimulus_presentations(self, stimulus_presentations, nonapplicable="null"):
        ...
        stimulus_presentations = naming_utilities.map_stimulus_names(stimulus_presentations, default_stimulus_renames)
        stimulus_presentations = naming_utilities.map_column_names(
        ...
        stimulus_presentations = literal_col_eval(stimulus_presentations, columns=col_list)
        stimulus_presentations = df_list_to_tuple(stimulus_presentations, columns=col_list)
    ...
    def _build_units_table(self, units_table):
        ...
        table = pd.merge(
            units_table, channels, left_on="peak_channel_id", right_index=True, suffixes=["_unit", "_channel"]
        )
    ...
    def _build_mean_waveforms(self, mean_waveforms):
        ...
        for uid in list(mean_waveforms.keys()):
            ...
            time_vals = np.arange(data.shape[1]) / self.probes.loc[probe_id]["sampling_rate"]
    ...
    def _warn_invalid_spike_intervals(self):
        ...
        if not invalid_time_intervals.empty:
            warnings.warn(
                "Session includes invalid time intervals that could "
