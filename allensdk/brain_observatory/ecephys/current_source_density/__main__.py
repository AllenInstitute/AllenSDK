...
from allensdk.brain_observatory.ecephys.current_source_density._schemas import InputParameters, OutputParameters
...
from allensdk.brain_observatory.argschema_utilities import write_or_print_outputs, optional_lims_inputs

from allensdk.brain_observatory.ecephys.lfp_subsampling.subsampling import remove_lfp_noise
...
def run_csd(args: dict) -> dict:
    ...
    if args["start_field"] not in stimulus_table:
        stimulus_table = stimulus_table.rename(columns={"Start": args["start_field"]})
    ...
    for probe_idx, probe in enumerate(args["probes"]):
        ...
        if len(clean_channels) == 0:
            logging.error(f"There are no clean channels. Skipping probe {probe['name']}")
            probewise_outputs.append(
                {"name": probe["name"], "csd_path": None, "clean_channels": clean_channels.tolist()}
            )
            continue
        try:
            interp_lfp, spacing = interp_channel_locs(
                lfp=filt_lfp, actual_locs=clean_actual_locs, interp_locs=interp_locs
            )
        except QhullError:
            logging.error(
                f"There are only {len(clean_channels)} "
        ...
        current_source_density, csd_channels = compute_csd(trial_mean_lfp=trial_mean_lfp, spacing=spacing)
        ...
        probewise_outputs.append(
            {"name": probe["name"], "csd_path": probe["csd_output_path"], "clean_channels": clean_channels.tolist()}
        )
...
def write_csd_to_h5(
    ...
    relative_window,
    channels: np.ndarray,
    csd_locations: np.ndarray,
    stimulus_name: str,
    stimulus_index: Optional[int],
