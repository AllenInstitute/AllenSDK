...
class ProbeInputParameters(DefaultSchema):
    ...
    spacing = Float(
        default=0.04,
        help="distance (in millimiters) between lengthwise-adjacent rows of recording sites on this probe.",
    )
...
class StimulusInputParameters(DefaultSchema):
    ...
    index = Int(
        default=None,
        allow_none=True,
        help="CSD is calculated from a specific stimulus, defined (in part) by this index.",
    )
...
class InputParameters(ArgSchema):
    ...
    num_trials = Int(
        default=None, allow_none=True, help="Number of trials after stimulus onset from which to compute CSD"
    )
    volts_per_bit = Float(
        default=1.0,
        help="If the data are not in units of volts, they must be converted. In the past, this value was 0.195",
    )
    memmap = Bool(default=False, help="whether to memory map the data file on disk or load it directly to main memory")
    memmap_thresh = Float(
        default=np.inf,
        help="files larger than this threshold (bytes) will be memmapped, regardless of the memmap setting.",
    )
    filter_cuts = List(
        Float, default=[5.0, 150.0], cli_as_single_argument=True, help="Cutoff frequencies for bandpass filter"
    )
    ...
    start_field = String(default="Start", help="Column from which to extract start times.")


class ProbeOutputParameters(DefaultSchema):
    name = String(required=True, help="Identifier for this probe.")
    csd_path = String(required=True, allow_none=True, help="Path to current source density file.")
    clean_channels = List(Int, required=True, help="List of channels used in CSD calculation")
