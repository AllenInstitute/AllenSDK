...
def select_good_channels(
    lfp: np.ndarray, reference_channels: List[int], noisy_channel_threshold: float
...
def filter_lfp_channels(
    lfp: np.ndarray, sampling_rate: float, filter_cuts: List[float], filter_order: int
) -> np.ndarray:
