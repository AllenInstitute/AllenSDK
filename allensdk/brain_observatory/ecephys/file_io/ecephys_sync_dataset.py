...
class EcephysSyncDataset(Dataset):
    ...
    def extract_led_times(self, keys=Dataset.OPTOGENETIC_STIMULATION_KEYS, fallback_line=18):
        try:
            led_times = self.get_edges(kind="rising", keys=keys, units="seconds")
        except KeyError:
            warnings.warn("unable to find LED times using line labels" + f"{keys}, returning line {fallback_line}")
    ...
    def extract_frame_times_from_photodiode(
        ...
        vsync_times_chunked, pd_times_chunked = stimulus_sync.separate_vsyncs_and_photodiode_times(
        ...
        for i in range(len(vsync_times_chunked)):
            ...
            irregular_interval_policy = functools.partial(
                stimulus_sync.allocate_by_vsync, np.diff(vsync_times_chunked[i])
            )
    ...
    def extract_frame_times_from_vsyncs(
        self, photodiode_cycle=60, frame_keys=Dataset.FRAME_KEYS, photodiode_keys=Dataset.PHOTODIODE_KEYS
    ):
