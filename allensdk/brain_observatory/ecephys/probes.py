...
from allensdk.core import DataObject, JsonReadableInterface, NwbReadableInterface, NwbWritableInterface


class Probes(DataObject, JsonReadableInterface, NwbReadableInterface, NwbWritableInterface):
    """Probes"""
    ...
    def __init__(self, probes: List[Probe]):
        ...
        """
        self._probes = probes
        super().__init__(name="probes", value=None, is_value_self=True)
    ...
    def spike_times(self) -> Dict[int, np.ndarray]:
        ...
        """
        return {unit.id: unit.spike_times for probe in self.probes for unit in probe.units.value}
    ...
    def mean_waveforms(self) -> Dict[int, np.ndarray]:
        ...
        """
        return {unit.id: unit.mean_waveforms for probe in self.probes for unit in probe.units.value}
    ...
    def spike_amplitudes(self) -> Dict[int, np.ndarray]:
        ...
        """
        return {unit.id: unit.spike_amplitudes for probe in self.probes for unit in probe.units.value}
    ...
    def get_units_table(
        ...
        units_table = units_table.drop(columns=["spike_times", "spike_amplitudes", "mean_waveforms"])
        if filter_by_validity or filter_out_of_brain_units:
            channels = pd.concat([p.channels.to_dataframe(filter_by_validity=filter_by_validity) for p in self.probes])

            if filter_out_of_brain_units:
                channels = channels[~(channels["structure_acronym"].isna())]

            # noinspection PyTypeChecker
            channel_ids = set(channels.index.values.tolist())
            units_table = units_table[units_table["peak_channel_id"].isin(channel_ids)]

        if filter_by_validity:
            units_table = units_table[units_table["quality"] == "good"]
            units_table.drop(columns=["quality"], inplace=True)

        units_table = units_table[units_table["amplitude_cutoff"] <= (amplitude_cutoff_maximum or np.inf)]
        units_table = units_table[units_table["presence_ratio"] >= (presence_ratio_minimum or -np.inf)]
        units_table = units_table[units_table["isi_violations"] <= (isi_violations_maximum or np.inf)]

        return units_table

    @classmethod
    def from_json(cls, probes: List[Dict[str, Any]], skip_probes: Optional[List[str]] = None) -> "Probes":
        """
        ...
        invalid_skip_probes = set(skip_probes).difference([p["name"] for p in probes])
        if invalid_skip_probes:
            raise ValueError(f"You passed invalid probes to skip: {invalid_skip_probes} are not valid probe names")
    ...
    @classmethod
    def from_nwb(cls, nwbfile: NWBFile, probe_lfp_meta_map: Optional[Dict[str, ProbeWithLFPMeta]] = None) -> "Probes":
        """
        ...
        probes = [
            Probe.from_nwb(nwbfile=nwbfile, probe_name=probe_name, lfp_meta=probe_lfp_meta_map.get(probe_name))
    ...
    def to_nwb(self, nwbfile: NWBFile) -> Tuple[NWBFile, Dict[str, Optional[NWBFile]]]:
        ...
        for probe in self.probes:
            _, probe_nwbfile = probe.to_nwb(nwbfile=nwbfile)
            probe_nwbfile_map[probe.name] = probe_nwbfile

        nwbfile.units = pynwb.misc.Units.from_dataframe(
            self.get_units_table(filter_by_validity=False, filter_out_of_brain_units=False), name="units"
        )
