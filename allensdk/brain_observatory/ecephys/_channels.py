...
class Channels(DataObject, NwbReadableInterface, JsonReadableInterface):
    ...
    def from_json(cls, channels: dict) -> "Channels":
        ...
        channels = [
            Channel(
                ...
                anterior_posterior_ccf_coordinate=(channel["anterior_posterior_ccf_coordinate"]),
                dorsal_ventral_ccf_coordinate=(channel["dorsal_ventral_ccf_coordinate"]),
    ...
    def to_dataframe(self, external_channel_columns=None, filter_by_validity=True) -> pd.DataFrame:
        """
    ...
    @classmethod
    def from_nwb(cls, nwbfile: NWBFile, probe_id: Optional[int] = None) -> "Channels":
        """
        ...
        for channel_id, row in nwbfile.electrodes.to_dataframe().iterrows():
            ...
            if has_local_index and has_channel_number:
                raise RuntimeError("Unclear how to read channel; has both 'local_index' and 'probe_channel_number'")
                ...
                raise RuntimeError(
                    "Unclear how to read channel; "
                    "has neither 'local_index' nor "
                    "'probe_channel_number'.\n"
                    f"Columns are {row.keys()}"
                )

            structure_acronym = None if row["location"] in ["None", ""] else row["location"]
            channels.append(
                Channel(
                    id=channel_id,
