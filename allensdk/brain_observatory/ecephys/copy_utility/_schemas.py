...
available_hashers = {"sha3_256": hashlib.sha3_256, "sha256": hashlib.sha256, None: None}
...
class FileToCopy(DefaultSchema):
    ...
    destination = String(required=True, description="copy to here (full path, not just directory!)")
    key = String(
        required=True,
        description="will be passed through to outputs, allowing a name or kind to be associated with this file",
    )
...
class NonFileParameters(DefaultSchema):
    ...
    raise_if_comparison_fails = Boolean(
        default=True, description="if a hash comparison fails, throw an error (vs. a warning)"
    )
