...
def build_stimulus_table(
    ...
    **kwargs,
    ...
    frame_times = sync_dataset.extract_frame_times(
        strategy=frame_time_strategy, trim_discontiguous_frame_times=kwargs.get("trim_discontiguous_frame_times", True)
    )
    ...
    stim_table_full = naming_utilities.map_stimulus_names(stim_table_full, stimulus_name_map)

    print(stim_table_full.keys())
    print(column_name_map)

    stim_table_full = naming_utilities.map_column_names(stim_table_full, column_name_map, ignore_case=False)
...
def main():
    mod = ArgSchemaParserPlus(schema_type=InputParameters, output_schema_type=OutputSchema)
