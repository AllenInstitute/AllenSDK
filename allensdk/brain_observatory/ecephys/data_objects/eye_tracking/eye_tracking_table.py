...
from allensdk.brain_observatory.ecephys.data_objects.eye_tracking.probe_sync_qc import get_frame_exposure_times
...
class EyeTrackingTable(DataObject, DataFileReadableInterface,
    ...
    def from_data_file(
        ...
        cam_json = session_data['raw_eye_tracking_video_meta_data']

        sync_dataset = Dataset(str(sync_file.filepath))

        frame_times = get_frame_exposure_times(sync_dataset, cam_json)

        # frame_times.insert(0, None)

        #add frame to start

        # The software we use to acquire these videos appends a metadata frame 
        # to the beginning of each video file (eye, behavior and face). 
        # So the video should be one frame longer than the list of sync pulses. 
        # The annotation data appears to be indexed so that frame '0' is the metadata frame.
        # frame_times = np.insert(frame_times, 0, 0, axis=0)
        ...
        try:
            ...
            warnings.warn(msg)
            eye_tracking_data = cls._get_empty_df()
            raise Exception("ahahahahahah " + str(err))
