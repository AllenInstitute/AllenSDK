...
ns_builder = NWBNamespaceBuilder(
    doc="Allen Institute Ecephys Extension",
    version="0.2.0",
...
probe_id_attr = NWBAttributeSpec(name="probe_id", doc="Unique ID of the neuropixels probe", dtype="int")

# Ecephys probe device extension (inherits from NWB `Device`)
sampling_rate_attr = NWBAttributeSpec(name="sampling_rate", doc="The sampling rate for the device", dtype="float64")

ecephys_probe_attributes = [sampling_rate_attr, probe_id_attr]

ecephys_probe_ext = NWBGroupSpec(
...
has_lfp_data_attr = NWBAttributeSpec(name="has_lfp_data", doc="Indicates availability of LFP data", dtype="bool")

lfp_sampling_rate = NWBAttributeSpec(
    name="lfp_sampling_rate",
    doc=("The sampling rate at which data were acquired on this electrode group's channels"),
    dtype="float64",
)

ecephys_egroup_attributes = [has_lfp_data_attr, probe_id_attr, lfp_sampling_rate]

ecephys_egroup_ext = NWBGroupSpec(
...
specimen_name_attr = NWBAttributeSpec(name="specimen_name", doc="Full name of specimen", dtype="text")

age_in_days_attr = NWBAttributeSpec(name="age_in_days", doc="Age of specimen in days", dtype="float")

strain_attr = NWBAttributeSpec(name="strain", doc="Specimen strain", dtype="text")

ecephys_specimen_attributes = [specimen_name_attr, age_in_days_attr, strain_attr]

ecephys_specimen_ext = NWBGroupSpec(
...
rig_equipment_attr = NWBAttributeSpec(name="equipment", doc="Description of rig", dtype="text")

unit_attr = NWBAttributeSpec("unit", "Unit of measurement for the data", "text")

rig_monitor_position_dset = NWBDatasetSpec(
    name="monitor_position", doc="position of monitor (x, y, z)", attributes=[unit_attr], dtype="float32", dims=(3,)
)

rig_camera_position_dset = NWBDatasetSpec(
    name="camera_position", doc="position of camera (x, y, z)", attributes=[unit_attr], dtype="float32", dims=(3,)
)

rig_led_position_dset = NWBDatasetSpec(
    name="led_position", doc="position of LED (x, y, z)", attributes=[unit_attr], dtype="float32", dims=(3,)
)

rig_monitor_rotation_dset = NWBDatasetSpec(
    name="monitor_rotation", doc="rotation of monitor (x, y, z)", attributes=[unit_attr], dtype="float32", dims=(3,)
)

rig_camera_rotation_dset = NWBDatasetSpec(
    name="camera_rotation", doc="rotation of camera (x, y, z)", attributes=[unit_attr], dtype="float32", dims=(3,)
)
...
csd_timeseries_group = NWBGroupSpec(
    doc="A timeseries containing current source density (CSD) data", neurodata_type_inc="TimeSeries"
)

csd_virtual_electrode_vertical_positions = NWBDatasetSpec(
...
csd_virtual_electrode_horizontal_positions = NWBDatasetSpec(
...
ecephys_csd_ext = NWBGroupSpec(
    ...
    datasets=[csd_virtual_electrode_horizontal_positions, csd_virtual_electrode_vertical_positions],
