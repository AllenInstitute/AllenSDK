...
class EcephysProjectCache(Cache):
    ...
    def __init__(
        ...
        super(EcephysProjectCache, self).__init__(manifest=manifest_, version=version_, cache=cache)
        self.fetch_api = EcephysProjectWarehouseApi.default() if fetch_api is None else fetch_api
        self.fetch_tries = fetch_tries
        self.stream_writer = stream_writer or self.fetch_api.rma_engine.write_bytes
    ...
    def _get_sessions(self):
        ...
        response = one_file_call_caching(
            path, self.fetch_api.get_sessions, write_csv, read_csv, num_tries=self.fetch_tries
        )
    ...
    def _get_probes(self):
        ...
        if all(c in list(probes) for c in ["lfp_sampling_rate", "lfp_temporal_subsampling_factor"]):
            probes["lfp_sampling_rate"] = probes["lfp_sampling_rate"] / probes["lfp_temporal_subsampling_factor"]
        return probes
    ...
    def _get_annotated_probes(self):
        ...
        return pd.merge(
            probes, sessions, left_on="ecephys_session_id", right_index=True, suffixes=["_probe", "_session"]
        )
    ...
    def _get_annotated_units(self, filter_by_validity: bool = True, **unit_filter_kwargs) -> pd.DataFrame:
        ...
        annotated_units = pd.merge(
            units, channels, left_on="ecephys_channel_id", right_index=True, suffixes=["_unit", "_channel"]
        )
        annotated_units = annotated_units.rename(
            columns={
                "name": "probe_name",
                "phase": "probe_phase",
    ...
    def get_units(
        self, suppress: Optional[List[str]] = None, filter_by_validity: bool = True, **unit_filter_kwargs
    ) -> pd.DataFrame:
    ...
    def _get_substitute_channel_columns(self, session_id):
        ...
        return channels.loc[
            channels["ecephys_session_id"] == session_id,
            [
    ...
    def get_unit_analysis_metrics_for_session(
        ...
        **unit_filter_kwargs :
        ...
        metrics = one_file_call_caching(
            path, fetch_metrics, write_metrics_csv, read_metrics_csv, num_tries=self.fetch_tries
        )
    ...
    def get_unit_analysis_metrics_by_session_type(
        self, session_type, annotate: bool = True, filter_by_validity: bool = True, **unit_filter_kwargs
    ):
    ...
    def add_manifest_paths(self, manifest_builder):
        ...
        manifest_builder.add_path(self.SESSIONS_KEY, "sessions.csv", parent_key="BASEDIR", typename="file")

        manifest_builder.add_path(self.PROBES_KEY, "probes.csv", parent_key="BASEDIR", typename="file")

        manifest_builder.add_path(self.CHANNELS_KEY, "channels.csv", parent_key="BASEDIR", typename="file")

        manifest_builder.add_path(self.UNITS_KEY, "units.csv", parent_key="BASEDIR", typename="file")

        manifest_builder.add_path(self.SESSION_DIR_KEY, "session_%d", parent_key="BASEDIR", typename="dir")
        ...
        manifest_builder.add_path(
            self.SESSION_ANALYSIS_METRICS_KEY,
            "session_%d_analysis_metrics.csv",
            parent_key=self.SESSION_DIR_KEY,
            typename="file",
        )
    ...
    def _from_http_source_default(cls, fetch_api_cls, fetch_api_kwargs, **kwargs):
        fetch_api_kwargs = {"asynchronous": True} if fetch_api_kwargs is None else fetch_api_kwargs
        ...
        return cls(fetch_api=fetch_api_cls.default(**fetch_api_kwargs), **kwargs)
    ...
    def from_warehouse(
        ...
        """
        if scheme and host:
            app_kwargs = {"scheme": scheme, "host": host, "asynchronous": asynchronous}
        else:
    ...
    @classmethod
    def fixed(cls, manifest: Optional[Union[str, Path]] = None, version: Optional[str] = None):
        """
        ...
        version : str
            ...
            recorded in the file at manifest, an error will be raised.
        """
        return cls(fetch_api=EcephysProjectFixedApi(), manifest=manifest, version=version)
...
def get_grouped_uniques(this, other, foreign_key, field_key, unique_key, inplace=False):
    ...
    uniques = other.groupby(foreign_key)[field_key].apply(lambda x: x.unique())
