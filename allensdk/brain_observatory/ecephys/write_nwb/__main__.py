...
from allensdk.brain_observatory.ecephys._behavior_ecephys_metadata import BehaviorEcephysMetadata
...
from allensdk.brain_observatory.ecephys.write_nwb.schemas import VCNInputSchema, OutputSchema
...
from allensdk.brain_observatory.ecephys.nwb import EcephysSpecimen, EcephysEyeTrackingRigMetadata
...
def get_inputs_from_lims(host, ecephys_session_id, output_root, job_queue, strategy):
    ...
    uri = (
        f"{host}/input_jsons?object_id={ecephys_session_id}"
...
def read_stimulus_table(
    path: str, column_renames_map: Dict[str, str] = None, columns_to_drop: List[str] = None
) -> pd.DataFrame:
    ...
    if columns_to_drop:
        stimulus_table = stimulus_table.drop(errors="ignore", columns=columns_to_drop)
...
def read_running_speed(path):
    ...
    return (pd.read_hdf(path, key="running_speed"), pd.read_hdf(path, key="raw_data"))


DEFAULT_RUNNING_SPEED_UNITS = {"velocity": "cm/s", "vin": "V", "vsig": "V", "rotation": "radians"}
...
def write_probe_lfp_file(session_id, session_metadata, session_start_time, log_level, probe_meta):
...
def write_probewise_lfp_files(probes, session_id, session_metadata, session_start_time, pool_size=3):
    output_paths = []

    pool = mp.Pool(processes=pool_size)
    write = partial(
        write_probe_lfp_file,
        session_id,
        session_metadata,
        session_start_time,
...
def parse_probes_data(probes: List[Dict[str, Any]]) -> ParsedProbeData:
    ...
    return probes.units_table, probes.spike_times, probes.spike_amplitudes, probes.mean_waveforms
...
def add_eye_tracking_rig_geometry_data_to_nwbfile(
    nwbfile: pynwb.NWBFile, eye_tracking_rig_geometry: dict
) -> pynwb.NWBFile:
...
def add_eye_tracking_data_to_nwbfile(
    ...
    eye_gaze_data: Dict[str, pd.DataFrame],
) -> pynwb.NWBFile:
    if eye_tracking_data_is_valid(
        ...
        if eye_gaze_data:
            add_eye_gaze_mapping_data_to_nwbfile(nwbfile, eye_gaze_data=eye_gaze_data)
...
def write_ecephys_nwb(
    ...
    **kwargs,
    ...
    nwbfile = add_stimulus_timestamps(nwbfile, stimulus_table.value["start_time"].values)
    ...
    if optotagging_table_path is not None:
        optotagging_table = OptotaggingTable.from_json(dict_repr={"optotagging_table_path": optotagging_table_path})
    ...
    if eye_tracking_rig_geometry is not None:
        add_eye_tracking_rig_geometry_data_to_nwbfile(nwbfile, eye_tracking_rig_geometry)
    ...
    if eye_dlc_ellipses_path is not None:
        eye_tracking_frame_times = su.get_synchronized_frame_times(
            session_sync_file=session_sync_path, sync_line_label_keys=Dataset.EYE_TRACKING_KEYS
        )
        ...
        add_eye_tracking_data_to_nwbfile(nwbfile, eye_tracking_frame_times, eye_dlc_tracking_data, eye_gaze_data)
    ...
    probe_outputs = write_probewise_lfp_files(
        probes_with_lfp, session_id, session_metadata, session_start_time, pool_size=pool_size
    )
...
def main():
    ...
    parser = optional_lims_inputs(sys.argv, VCNInputSchema, OutputSchema, get_inputs_from_lims)
