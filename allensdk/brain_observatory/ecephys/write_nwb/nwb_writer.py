...
from allensdk.core import JsonReadableInterface, NwbReadableInterface, NwbWritableInterface
...
class BehaviorEcephysNwbWriter(NWBWriter):
    ...
    def __init__(
        ...
        super().__init__(nwb_filepath=session_nwb_filepath, session_data=session_data, serializer=serializer)
    ...
    def write_nwb(self, **kwargs):
        ...
        """
        from_json_kwargs = {
            k: v for k, v in kwargs.items() if k in inspect.signature(self._serializer.from_json).parameters
        }
        json_session = self._serializer.from_json(session_data=self._session_data, **from_json_kwargs)

        try:
            nwbfile = self._write_nwb(session=json_session, **kwargs)
            self._compare_sessions(nwbfile=nwbfile, loaded_session=json_session, **kwargs)
    ...
    def _write_nwb(self, session: BehaviorEcephysSession, **kwargs) -> NWBFile:
        ...
        logging.info(f"Writing session NWB file to {self.nwb_filepath_inprogress}")
        ...
        for probe_name, probe_nwbfile in probe_nwbfile_map.items():
            probe_id = [p.id for p in session.get_probes_obj() if p.name == probe_name][0]
            if probe_nwbfile is not None:
                probe_nwb_path = Path(self._nwb_filepath).parent / f"lfp_probe_{probe_id}.nwb"
                logging.info(f"Writing probe NWB file to {probe_nwb_path}")
                with NWBHDF5IO(probe_nwb_path, "w") as nwb_file_writer:
                    nwb_file_writer.write(probe_nwbfile)
                logging.info(f"Wrote probe NWB file to {probe_nwb_path}")
