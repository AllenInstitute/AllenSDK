...
class DriftingGratings(DefaultSchema):
    stimulus_key = List(
        String, default=DriftingGratings.known_stimulus_keys(), help="Key for the drifting gratings stimulus"
    )
...
class StaticGratings(DefaultSchema):
    stimulus_key = List(
        String, default=StaticGratings.known_stimulus_keys(), help="Key for the static gratings stimulus"
    )
...
class ReceptiveFieldMapping(DefaultSchema):
    stimulus_key = List(
        String, default=ReceptiveFieldMapping.known_stimulus_keys(), help="Key for the receptive field mapping stimulus"
    )
    trial_duration = Float(default=0.25, help="typical length of a epoch for given stimulus in seconds")
