...
class Flashes(StimulusAnalysis):
    ...
    def metrics(self):
        if self._metrics is None:
            ...
            if len(self.stim_table) > 0:
                ...
                metrics_df["sustained_idx_fl"] = [
                    self._get_sustained_index(unit, self._get_preferred_condition(unit)) for unit in unit_ids
                ]
                metrics_df["firing_rate_fl"] = [self._get_overall_firing_rate(unit) for unit in unit_ids]
                metrics_df["time_to_peak_fl"] = [
    ...
    def _get_stim_table_stats(self):
        ...
        self._colors = np.sort(
            self.stimulus_conditions.loc[self.stimulus_conditions[self._col_color] != "null"][self._col_color].unique()
        )
