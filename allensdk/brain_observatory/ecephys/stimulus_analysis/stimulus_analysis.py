...
class StimulusAnalysis(object):
    def __init__(self, ecephys_session, trial_duration=None, **kwargs):
        ...
        if isinstance(ecephys_session, EcephysSession):
            ...
            raise TypeError("Don't know how to make a stimulus analysis "
                            f"object from a {type(ecephys_session)}")
        ...
        # the overall_firing_rate functions that only need to be calculated
        # once, but not accessable to the user
        ...
        self._spontaneous_threshold = \
            kwargs.get('spontaneous_threshold', 100.0)
    ...
    def unit_ids(self):
        ...
        if self._unit_ids is None:
            ...
            if isinstance(self._unit_filter, (list,
                                              tuple,
                                              np.ndarray,
                                              pd.Series)):
                ...
                if 'unit_id' in self._unit_filter.keys():
                    # If user wants to filter by the unit_id column which
                    # is actually the dataframe index
    ...
    def trial_duration(self):
        if self._trial_duration is None or self._trial_duration < 0.0:
            ...
            raise TypeError(f'Invalid value {self._trial_duration} for '
    ...
    def spikes(self):
        ...
        if self._spikes is None:
            ...
            if len(self._spikes) > self.unit_count:
                # if a filter has been applied such that not all the cells are
                #  being used in the analysis
                self._spikes = {
                    k: v for k, v in self._spikes.items() if k in self.unit_ids
                    }
    ...
    def stim_table(self):
        ...
        if self._stim_table is None:
            if self._stimulus_key is None:
                ...
                if self._stimulus_key is None:
                    raise Exception('Could not find appropriate stimulus_name '
                                    'key for current stimulus type. Please '
                                    'specify using the stimulus_key '
                                    'parameter.')

            self._stim_table = self.ecephys_session.get_stimulus_table(
                [self._stimulus_key] if isinstance(
                    self._stimulus_key,
                    string_types) else self._stimulus_key
            )

            if self._stim_table.empty:
                raise Exception('Could not find stimulus data with '
                                f'"stimulus_key" {self._stimulus_key}')
    ...
    def stim_table_spontaneous(self):
        ...
        # TODO: This may not be need anymore? Ask the scientists
        #       if sweep_p_events will be required in the future.
        if self._stim_table_spontaneous is None:
            stim_table = \
                self.ecephys_session.get_stimulus_table(
                    self.known_spontaneous_keys)
            # TODO: If duration does not exists in stim_table create it from
            #       stop and start times
            self._stim_table_spontaneous = \
                stim_table[
    ...
    def conditionwise_psth(self):
        """For every unit and stimulus-condition construction a PSTH table. i.e.
        the spike-counts at a each time-interval during a stimulus, averaged
        over all trials of the same stim condition.

        Each PSTH will count and average spikes over a time-window as
        determined by class parameter 'trial_duration' which ideally be a
        similar value as the duration of each stimulus (in seconds). The
        length of each time-bin is determined by the class parameter
        'psth_resolution' (in seconds).
        ...
        if self._conditionwise_psth is None:
            if self._psth_resolution > self.trial_duration:
                warnings.warn('parameter "psth_resolution" > "trial_duration",'
            ...
            # every single stim) with the corresponding stimulus_condition_id
            # (which will be shared among presenations with the same
            # conditions.
            ...
            if n_stimuli == n_cond_ids:
                # If every condition_id is unique then calling groupby().mean()
                # is unnecessary and will raise an error.
                self._conditionwise_psth = da
            else:
                self._conditionwise_psth = \
                    da.groupby('stimulus_condition_id').mean(
                        dim='stimulus_condition_id')
    ...
    def conditionwise_statistics(self):
        ...
        conditionwise_statistics: pd.DataFrame
            ...
            spike_count, spike_mean, spike_sem, spike_std and
            stimulus_presentation_count information.
        """
    ...
    def presentationwise_spike_times(self):
        ...
        the stimulus_presentation_id and unit_id for the given spike.
    ...
    def presentationwise_statistics(self):
        ...
        if self._presentationwise_statistics is None:
            ...
            df = self.ecephys_session.presentationwise_spike_counts(
                ...
                level='time_relative_to_stimulus_onset',
                drop=True)

            # left join table with stimulus_condition_id and mean running_speed
            # joined on stimulus_presentation_id
            df = df.join(self.stim_table.loc[
                    df.index.levels[0].values]['stimulus_condition_id'])
    ...
    def running_speed(self):
        ...
        if self._running_speed is None:
            ...
            inds = self.stim_table.index.values

            self._running_speed = \
                pd.DataFrame(index=self.stim_table.index.values,
                             data={'running_speed': [get_velocity(i)
                                                     for i in
                                                     inds]
                                   }).rename_axis('stimulus_presentation_id')

        # TODO: The below is equivalent but uses numpy vectorization,
        # profile to see if it's worth swapping out.
        ...
        # sampled_indicies = np.where((self._ecephys_session.running_speed
        #            .start_time >= stim_times[0])
        #            & (self._ecephys_session.running_speed.start_time
        #            < stim_times[-1]))[0]
        # relevant_dxtimes = self._ecephys_session.running_speed
        #            .start_time[sampled_indicies]
        # relevant_dxcms = self._ecephys_session.
        #             running_speed.velocity[sampled_indicies]
        #
        # indices = np.searchsorted(stim_times, relevant_dxtimes.values,
        #      side='right')
        # rs_tmp_df = pd.DataFrame({'running_speed': relevant_dxcms,
        #  'stim_indicies': indices})
        #
    ...
    def empty_metrics_table(self):
        ...
        # 'object' type, this should enforce the
        # correct data type for each column
        empty_array = np.empty(self.unit_count,
                               dtype=np.dtype(self.METRICS_COLUMNS))
        empty_array[:] = np.nan

        return pd.DataFrame(empty_array,
    ...
    def _get_preferred_condition(self, unit_id):
        ...
        # TODO: Should probably be renamed to preferred_condition_id so there
        #       is no confusion.
        if unit_id not in self._preferred_condition:
            # Use conditionwise_statistics 'spike_mean' column to find
            # stimulus_condition_id that gives the highest value.
            try:
    ...
    def _get_running_modulation(self,
                                unit_id,
                                preferred_condition,
                                threshold=1.0):
        """Get running modulation for the preferred condition of a
        given unit"""
        subset = self.presentationwise_statistics[
            self.presentationwise_statistics['stimulus_condition_id']
            == preferred_condition
    ...
    def _get_time_to_peak(self, unit_id, preferred_condition):
        """Equal to the time of the maximum firing rate of the average PSTH at
    ...
    def _get_overall_firing_rate(self, unit_id):
        ...
        if self._block_starts is None:
            # For the stimulus, create a list of start and stop times for the
            # given block of trials. Only needs to be calculated once TODO: see
            # if python allows for private property variables
            ...
            interval_start_inds = np.concatenate(
                (np.array([0]),
                 np.where(start_time_intervals >
                 self.trial_duration * 2)[0] + 1))

            self._block_starts = \
                self.stim_table.iloc[interval_start_inds]['start_time'].values
            self._block_stops = \
                self.stim_table.iloc[interval_end_inds]['stop_time'].values
    ...
    def plot_conditionwise_raster(self, unit_id):
        ...
        _ = [self.plot_raster(cond, unit_id)
             for cond in self.stimulus_conditions.index.values]
    ...
    def known_stimulus_keys(cls):
        ...
        StimulusAnalysis subclass (when stimulus_key is not explicity set).
        Should return a list of "stimulus_name" strings.
        """
        raise NotImplementedError()
...
def running_modulation(spike_counts, running_speeds, speed_threshold=1.0):
    ...
    running-speed, does a statistical comparison to see if there was any
    difference in spike firing while running and while stationary.
    ...
    if(len(spike_counts) != len(running_speeds)):
        warnings.warn('spike_counts and running_speeds must be '
...
def lifetime_sparseness(responses):
    ...
    responses : array of floats
        An array of a unit's spike-counts over the duration of multiple trials
    ...
    if len(responses) <= 1:
        ...
        warnings.warn('responses array must contain at least two or '
...
def overall_firing_rate(start_times, stop_times, spike_times):
    ...
    if len(start_times) != len(stop_times):
        warnings.warn('start_times and stop_times must be arrays of '
                      'the same length')
        return np.nan
    ...
    return np.sum(
        spike_times.searchsorted(stop_times) -
        spike_times.searchsorted(start_times)
        ) / total_time
...
def reliability(unit_sweeps,
    ...
    if isinstance(unit_sweeps, (list, tuple)):
        unit_sweeps = np.array([np.array(sweep) for sweep in unit_sweeps])
    ...
    for i in range(len(unit_sweeps)):
        ...
        for j in range(len(unit_sweeps)):
            frj = get_fr(unit_sweeps[j],
                         num_timestep_second=num_timestep_second,
                         filter_width=filter_width)
            # Warning: the pearson coefficient is likely to have a denominator
            # of 0 for some cells/stimulus and give a divide by 0 warning.
...
def osi(orivals, tuning):
    """Computes the orientation selectivity of a cell. The calculation of the
    orientation is done using the normalized circular variance (CirVar)
    as described in Ringbach 2002
    ...
    -------
    osi : float
        An N-dimensional array of the circular variance (scalar value, in
        radians) of the responses.
    """
...
def fit_exp(rsc_time_matrix):
    ...
    p, amo = curve_fit(lambda t, a, b, c: a*np.exp(-1/b*t)+c,
                       t,
                       y,
                       p0=(-4, 2, 1),
                       maxfev=1000000000)
    ...
    for unit_idx, unit in enumerate(dataset.unit_id):
        ...
        for i in np.arange(nbins-1):
            for j in np.arange(i+1, nbins):
                ...
                good_trials = (
                    spikes_for_unit[:, i] * spikes_for_unit[:, j]
                    ) > 0
                r, p = st.pearsonr(
                    spikes_for_unit[good_trials, i],
