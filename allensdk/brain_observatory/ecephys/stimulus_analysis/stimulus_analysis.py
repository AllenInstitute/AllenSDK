...
class StimulusAnalysis(object):
    ...
    def stim_table(self):
        ...
        if self._stim_table is None:
            ...
            self._stim_table = self.ecephys_session.get_stimulus_table(
                [self._stimulus_key] if isinstance(self._stimulus_key, str) else self._stimulus_key
            )
    ...
    def presentationwise_spike_times(self):
        ...
        if self._presentationwise_spikes is None:
            self._presentationwise_spikes = self.ecephys_session.presentationwise_spike_times(
                stimulus_presentation_ids=self.stim_table.index.values, unit_ids=self.unit_ids
            )
    ...
    def presentationwise_statistics(self):
        ...
        if self._presentationwise_statistics is None:
            ...
            df = (
                ...
                .to_dataframe()
                .reset_index(level="time_relative_to_stimulus_onset", drop=True)
            )
    ...
    def running_speed(self):
        ...
        if self._running_speed is None:
            ...
            self._running_speed = pd.DataFrame(
                ...
                data={"running_speed": [get_velocity(i) for i in self.stim_table.index.values]},
    ...
    def empty_metrics_table(self):
        ...
        df = pd.DataFrame(
            empty_array, index=pd.Index(self.unit_ids, name="unit_id"), columns=[x[0] for x in self.METRICS_COLUMNS]
        )
    ...
    def _check_multiple_pref_conditions(self, unit_id, stim_cond_col, valid_conditions):
        ...
        # spiking
        similar_conditions = [
            self.stimulus_conditions.index[self.stimulus_conditions[stim_cond_col] == v].tolist()
    ...
    def _get_lifetime_sparseness(self, unit_id):
        ...
        df = self.conditionwise_statistics.drop(index=self.null_condition, level=1, errors="ignore")
    ...
    def _get_overall_firing_rate(self, unit_id):
        ...
        if self._block_starts is None:
            ...
            interval_end_inds = np.concatenate(
                (np.where(start_time_intervals > self.trial_duration * 2)[0], np.array([self.total_presentations - 1]))
            )
            interval_start_inds = np.concatenate(
                (np.array([0]), np.where(start_time_intervals > self.trial_duration * 2)[0] + 1)
            )
    ...
    for unit_idx, unit in enumerate(dataset.unit_id):
        ...
        for i in np.arange(nbins - 1):
            for j in np.arange(i + 1, nbins):
                ...
                good_trials = (spikes_for_unit[:, i] * spikes_for_unit[:, j]) > 0
