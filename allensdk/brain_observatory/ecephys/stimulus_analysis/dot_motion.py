...
class DotMotion(StimulusAnalysis):
        ...
        if self._metrics is None:
            ...
            if len(self.stim_table) > 0:
                ...
                metrics_df["pref_dir_multi_dm"] = [
                    self._check_multiple_pref_conditions(unit_id, self._col_dir, self.directions)
    ...
    def _get_stim_table_stats(self):
        ...
        self._dirvals = np.sort(
            self.stimulus_conditions.loc[self.stimulus_conditions[self._col_dir] != "null"][self._col_dir].unique()
        )
        self._number_dir = len(self._dirvals)

        self._speedvals = np.sort(
            self.stimulus_conditions.loc[self.stimulus_conditions[self._col_speed] != "null"][self._col_speed].unique()
        )
    ...
    def _get_pref_speed(self, unit_id):
        ...
        similar_conditions = [
            self.stimulus_conditions.index[self.stimulus_conditions[self._col_speed] == speed].tolist()
    ...
    def _get_pref_dir(self, unit_id):
        ...
        """
        similar_conditions = [
            self.stimulus_conditions.index[self.stimulus_conditions[self._col_dir] == direction].tolist()
