...
class StaticGratings(StimulusAnalysis):
        ...
        if self._metrics is None:
            ...
            if len(self.stim_table) > 0:
                ...
                metrics_df["pref_phase_multi_sg"] = [
                    self._check_multiple_pref_conditions(unit_id, self._col_phase, self.phasevals)
                    for unit_id in unit_ids
                ]
                metrics_df["g_osi_sg"] = [
                    self._get_osi(unit, metrics_df.loc[unit]["pref_sf_sg"], metrics_df.loc[unit]["pref_phase_sg"])
                    for unit in unit_ids
                ]
                metrics_df["time_to_peak_sg"] = [
    ...
    def _get_stim_table_stats(self):
        ...
        self._sfvals = np.sort(
            self.stimulus_conditions.loc[self.stimulus_conditions[self._col_sf] != "null"][self._col_sf].unique()
        )
        self._number_sf = len(self._sfvals)

        self._phasevals = np.sort(
            self.stimulus_conditions.loc[self.stimulus_conditions[self._col_phase] != "null"][self._col_phase].unique()
        )
    ...
    def _get_pref_sf(self, unit_id):
        ...
        similar_conditions_ids = [
            self.stimulus_conditions.index[self.stimulus_conditions[self._col_sf] == sf].tolist() for sf in self.sfvals
        ]
    ...
    def _get_pref_phase(self, unit_id):
        ...
        """
        combined_cond_ids = [
            self.stimulus_conditions.index[self.stimulus_conditions[self._col_phase] == phase].tolist()
    ...
    def plot_response_summary(self, unit_id, bar_thickness=0.25):
        ...
        x = (
            df.loc[cond_values.values]["sf_index"]
    ...
    def make_fan_plot(self, unit_id):
        ...
        group_data = self.stimulus_conditions.loc[
            self.presentationwise_statistics.xs(unit_id, level=1)["stimulus_condition_id"]
        ][self._col_phase].values
...
def fit_sf_tuning(sf_tuning_responses, sf_values, pref_sf_index):
    ...
    if pref_sf_index in range(1, len(sf_values) - 1):
        ...
        try:
            popt, pcov = curve_fit(
                gauss_function,
                np.arange(len(sf_values)),
                sf_tuning_responses,
                p0=[np.amax(sf_tuning_responses), pref_sf_index, 1.0],
                maxfev=2000,
            )
            sf_prediction = gauss_function(np.arange(0.0, 4.1, 0.1), *popt)
            fit_sf_ind = popt[1]
            fit_sf = 0.02 * np.power(2, popt[1])
            low_cut_ind = np.abs(sf_prediction - (sf_prediction.max() / 2.0))[: sf_prediction.argmax()].argmin()
            high_cut_ind = (
                np.abs(sf_prediction - (sf_prediction.max() / 2.0))[sf_prediction.argmax() :].argmin()
                + sf_prediction.argmax()
            )
            if low_cut_ind > 0:
        ...
        try:
            popt, pcov = curve_fit(
                ...
                np.arange(len(sf_values)),
                sf_tuning_responses,
                p0=[np.amax(sf_tuning_responses), 2.0, np.amin(sf_tuning_responses)],
                maxfev=2000,
            )
            sf_prediction = exp_function(np.arange(0.0, 4.1, 0.1), *popt)
            if pref_sf_index == 0:
                high_cut_ind = (
                    np.abs(sf_prediction - (sf_prediction.max() / 2.0))[sf_prediction.argmax() :].argmin()
                    + sf_prediction.argmax()
