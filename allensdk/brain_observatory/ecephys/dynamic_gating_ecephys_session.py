...
from allensdk.brain_observatory.behavior.dynamic_gating_session import \
...
class DynamicGatingBehaviorSession(DynamicGatingSession):
    """
    ...
    def _read_behavior_stimulus_timestamps(
        ...
        """
        timestamps = StimulusTimestamps.from_dual_stimulus_blocks(
                sync_file=sync_file,
                list_of_stims=[
    ...
    def _read_trials(
        ...
        return DynamicGatingTrials.trials_from_dynamic_gating_stimulus(
...
class DynamicGatingEcephysSession(DynamicGatingBehaviorSession):
    """
    ...
    def behavior_data_class(cls):
        ...
        in this DynamicGatingEcephysSession
        """
        return DynamicGatingSession

    def __init__(
            self,
            behavior_session: DynamicGatingSession,
        ...
        self._behavior_session = behavior_session
        self._probes = probes
    ...
    def from_json(
        ...
        Returns
        -------
        Instantiated `DynamicGatingSession`
        """
        ...
        return DynamicGatingEcephysSession(
    ...
    def to_nwb(self) -> Tuple[NWBFile, Dict[str, Optional[NWBFile]]]:
        ...
        nwbfile, probe_nwbfile_map = self._probes.to_nwb(
            nwbfile=nwbfile)
    ...
    def from_nwb(
        ...
        Returns
        -------
        instantiated `DynamicGatingEcephysSession`
        """
...
if __name__ == '__main__':
    ...
    dynamic_gating_path = pathlib.Path('//allen/programs/mindscope/workgroups/dynamicrouting/dynamic_gating')
    session = args.session
    nwb_filepath = pathlib.Path(dynamic_gating_path, 'nwbs', session, '{}.nwb'.format(session)).as_posix()

    with h5py.File(nwb_filepath) as h5_file:
        with NWBHDF5IO(file=h5_file, mode='r', load_namespaces=True) as nwb_file:
            read_nwb = nwb_file.read()
            print(type(read_nwb))
            dynamic_gating_session_object = DynamicGatingEcephysSession.from_nwb(read_nwb)
            print(dynamic_gating_session_object.probes)
