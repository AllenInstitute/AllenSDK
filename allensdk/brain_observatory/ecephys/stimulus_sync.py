...
def trim_border_pulses(pd_times, vs_times, frame_interval=1 / 60, num_frames=5):
    pd_times = np.array(pd_times)
    return pd_times[np.logical_and(pd_times >= vs_times[0], pd_times <= vs_times[-1] + num_frames * frame_interval)]
...
def flag_unexpected_edges(pd_times, ndevs=10):
    ...
    expected_duration_mask[
        np.logical_or(pd_diff < diff_mean - ndevs * diff_std, pd_diff > diff_mean + ndevs * diff_std)
    ] = 0
...
def fix_unexpected_edges(pd_times, ndevs=10, cycle=60, max_frame_offset=4):
    ...
    bad_blocks = np.sort(np.unique(np.concatenate([[0], np.where(np.diff(bad_edges) > 1)[0] + 1, [len(bad_edges)]])))
