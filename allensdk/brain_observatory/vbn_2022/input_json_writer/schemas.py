...
class VBN2022InputJsonWriterSchema(argschema.ArgSchema):
    ...
    json_output_dir = argschema.fields.OutputDir(
        required=True, description=("Directory where input JSONs will be written")
    )

    nwb_output_dir = argschema.fields.OutputDir(
        required=True, description=("Directory where NWB files will be written")
    )

    clobber = argschema.fields.Boolean(
        default=False, description=("If false, throw an error if output files already exist")
    )
    ...
    json_prefix = argschema.fields.Str(
        ...
        allow_none=False,
        description=("The files written by this module will be named like {json_prefix}_{session_id}_input.json"),
    )
    ...
    nwb_prefix = argschema.fields.Str(
        ...
        default="ecepys",
        allow_none=False,
        description=("The NWB files specified in the input JSONs will be named like {nwb_prefix}_{session_id}.nwb"),
    )
    ...
    def create_path_lookup(self, data, **kwargs):
        ...
        for ecephys_id in data["ecephys_session_id_list"]:
            ...
            if json_name in unq_json:
                raise RuntimeError(f"This configuration would write {json_name} more than once")
            unq_json.add(json_name)
            json_path = json_dir_path / json_name
            if not data["clobber"] and json_path.is_file():
                raise RuntimeError(
                    f"{json_path.resolve().absolute()} already exists; run with clobber=True to overwrite"
                )
