...
from allensdk.internal.api.queries.wkf_lims_queries import wkf_path_from_attachable

from allensdk.internal.api.queries.equipment_lims_queries import experiment_configs_from_equipment_id_and_type
...
from allensdk.brain_observatory.behavior.data_objects.metadata.subject_metadata.reporter_line import ReporterLine

from allensdk.brain_observatory.behavior.data_objects.metadata.subject_metadata.driver_line import DriverLine
...
class NwbConfigErrorLog(object):
    ...
    def log(self, ecephys_session_id: Union[int, str], msg: str) -> None:
        """
...
def vbn_nwb_config_from_ecephys_session_id_list(
    ecephys_session_id_list: List[int], probes_to_skip: Optional[List[dict]]
) -> dict:
    """
    ...
    if probes_to_skip is not None:
        probe_ids_to_skip = get_list_of_bad_probe_ids(lims_connection=lims_connection, probes_to_skip=probes_to_skip)
    else:
    ...
    session_list = session_input_from_ecephys_session_id_list(
        ecephys_session_id_list=ecephys_session_id_list, lims_connection=lims_connection, error_log=error_log
    )
    ...
    for session in session_list:
        ...
        for idx, probe in enumerate(session["probes"]):
            ...
            if probe_id in channel_input:
                ...
                msg = f"could not find channels for probe {probe_id}; not listing in the input.json"
                error_log.log(ecephys_session_id=session_id, msg=msg)
    ...
    return {"sessions": session_list, "log": error_log.write()}


def session_input_from_ecephys_session_id_list(
    ecephys_session_id_list: List[int], lims_connection: PostgresQueryMixin, error_log: NwbConfigErrorLog
) -> List[dict]:
    """
    ...
    session_table.loc[np.logical_not(session_table.behavior_session_id.isna()), "date_of_acquisition"] = None
    ...
    session_table.date_of_acquisition = session_table.date_of_acquisition.dt.floor("S")

    session_table = _add_age_in_days(df=session_table, index_column="ecephys_session_id")

    session_table.age_in_days = session_table.age_in_days.apply(lambda x: f"P{int(x):d}")

    # apply naming conventions from the NWB writer's schema
    session_table.rename(
        columns={
            "equipment_name": "rig_name",
    ...
    session_table.external_specimen_name = session_table.external_specimen_name.astype(int)
    ...
    input_from_wkf_session = [
        ...
        ("raw_eye_tracking_video_meta_data", "RawEyeTrackingVideoMetadata"),
    ...
    wkf_types_to_query_session = [f"'{el[1]}'" for el in input_from_wkf_session]
    ...
    for session_id in ecephys_session_id_list:
        ...
        if session_id not in session_table:
            error_log.log(ecephys_session_id=session_id, msg="No session data was found at all; skipping")
            continue
        ...
        for key_pair in input_from_wkf_session:
            ...
            if this_path is None:
                msg = f"Could not find {key_pair[1]} for ecephys_session {session_id}"
                error_log.log(ecephys_session_id=session_id, msg=msg)
        ...
        data["optotagging_table_path"] = optotagging_path_lookup["EcephysOptotaggingTable"]

        driver_line = DriverLine.from_lims(
            lims_db=lims_connection, behavior_session_id=data["behavior_session_id"], allow_none=True
        ).value
...
def _get_probe_analysis_run_from_probe_id(lims_connection: PostgresQueryMixin, probe_id: int, lims_strategy: str):
    query = f"""
    SELECT earp.id
    ...
    if not res:
        raise OneResultExpectedError(f"Expected to find one analysis probe run for probe {probe_id}")
    return res["id"]


def _get_probe_lfp_meta(lims_connection: PostgresQueryMixin, probe_id: int):
    ...
    current_source_density_well_known_files = ["EcephysCurrentSourceDensity"]
    probe_lfp_subsampling_run_id = _get_probe_analysis_run_from_probe_id(
        lims_connection=lims_connection, probe_id=probe_id, lims_strategy="EcephysLfpSubsamplingStrategy"
    )
    probe_current_source_density_run_id = _get_probe_analysis_run_from_probe_id(
        lims_connection=lims_connection, probe_id=probe_id, lims_strategy="EcephysCurrentSourceDensityStrategy"
    )
    ...
    lfp = {
        "input_data_path": probe_lfp_well_known_files.get("EcephysSubsampledLfpContinuous"),
        "input_timestamps_path": probe_lfp_well_known_files.get("EcephysSubsampledLfpTimestamps"),
        "input_channels_path": probe_lfp_well_known_files.get("EcephysSubsampledChannelStates"),
        "csd_path": probe_csd_well_known_files.get("EcephysCurrentSourceDensity"),
    }
    return lfp
...
def probe_input_from_ecephys_session_id(
    ...
    probes_table.drop(
        labels=["ecephys_session_id", "phase", "unit_count", "channel_count", "structure_acronyms"],
        axis="columns",
        inplace=True,
    )
    ...
    wkf_to_query = [f"'{el[1]}'" for el in input_from_wkf_probe]
    ...
    for probe_id in probe_id_list:
        ...
        if has_lfp:
            lfp_meta = _get_probe_lfp_meta(lims_connection=lims_connection, probe_id=probe_id)
...
def channel_input_from_ecephys_session_id(
    ...
    raw_channels_table.rename(columns={"ecephys_channel_id": "id", "ecephys_probe_id": "probe_id"}, inplace=True)
...
def unit_input_from_ecephys_session_id(
    ...
    if len(raw_unit_table) == 0:
        ...
        error_log.log(ecephys_session_id=ecephys_session_id, msg=msg)
        return dict()
...
def eye_tracking_geometry_from_equipment_id(
    equipment_id: int, date_of_acquisition: pd.Timestamp, lims_connection: PostgresQueryMixin
) -> dict:
    """
    ...
    raw_eye_geometry = _raw_eye_tracking_geometry_from_equipment_id(
        equipment_id=equipment_id, date_of_acquisition=date_of_acquisition, lims_connection=lims_connection
    )
...
def _raw_eye_tracking_geometry_from_equipment_id(
    ...
    """
    config = dict()
    for name in ("led position", "behavior camera position", "eye camera position", "screen position"):
        this_df = experiment_configs_from_equipment_id_and_type(
            equipment_id=equipment_id, config_type=name, lims_connection=lims_connection
        )
        this_df = this_df.loc[this_df.active_date.dt.date <= date_of_acquisition]
...
def _analysis_run_from_session_id(
    lims_connection: PostgresQueryMixin, ecephys_session_id_list: List[int], strategy_class: str
) -> Dict[int, int]:
    """
    ...
    query += build_in_list_selector_query(
        col="ecephys_analysis_runs.ecephys_session_id", valid_list=ecephys_session_id_list, operator="AND", valid=True
    )

    query += build_in_list_selector_query(
        col="ecephys_analysis_runs.job_strategy_class", valid_list=[f"'{strategy_class}'"], operator="AND", valid=True
    )

    query_result = lims_connection.select(query)
    analysis_run_map = dict()
    msg = ""
    for session_id, run_id in zip(query_result.ecephys_session_id, query_result.ecephys_analysis_run_id):
...
def _add_spike_times_path(
    probe_list: List[dict], ecephys_session_id: int, lims_connection: PostgresQueryMixin, error_log: NwbConfigErrorLog
) -> List[dict]:
    """
    ...
    query += build_in_list_selector_query(
        col="ecephys_analysis_run_probes.ecephys_probe_id", valid_list=probe_id_list, operator="WHERE", valid=True
    )
    ...
    for p_id, r_id in zip(query_result.probe_id, query_result.ecephys_analysis_run_probe_id):
    ...
    for this_probe in probe_list:
        ...
        if probe_id in probe_run_lookup:
            ...
            this_probe["spike_times_path"] = timestamp_lookup["EcephysAlignedEventTimestamps"]
        else:
            msg = f"could not find EcephysAlignedEventTimestamps for probe {probe_id}"
            error_log.log(ecephys_session_id=ecephys_session_id, msg=msg)
