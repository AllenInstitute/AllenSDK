...
def create_roi_masks(rois, w, h, motion_border):
    ...
    for roi in rois:
        ...
        mask = roi_masks.create_roi_mask(
            w, h, motion_border, pix_list=px[:, [1, 0]], label=str(roi["id"]), mask_group=roi.get("mask_page", -1)
        )
...
def get_inputs_from_lims(host, ophys_experiment_id, output_root, job_queue, strategy):
...
def extract_traces(motion_corrected_stack, motion_border, storage_directory, rois, log_0, **kwargs):
    ...
    # motion border
    border = [motion_border["x0"], motion_border["x1"], motion_border["y0"], motion_border["y1"]]
    ...
    return {"neuropil_trace_file": np_file, "roi_trace_file": roi_file, "exclusion_labels": exclusions}
