...
class MotionBorder(RaisingSchema):
    x0 = Float(default=0.0, description="")  # TODO: be really certain about how these
    # relate to physical space and then write it here
...
class Roi(RaisingSchema):
    ...
    mask_page = Integer(default=-1, description="")  # TODO: this isn't in the examples
...
class InputSchema(ArgSchema):
    ...
    motion_border = Nested(
        MotionBorder, required=True, description="border widths - pixels outside the border are considered invalid"
    )
    storage_directory = String(required=True, description="used to set output directory")
    motion_corrected_stack = String(
        required=True, description="path to h5 file containing motion corrected image stack"
    )
    rois = Nested(Roi, many=True, description="specifications of individual regions of interest")
    log_0 = String(required=True, description="path to motion correction output csv")  #
    # TODO: is this redundant with motion border?
...
class OutputSchema(RaisingSchema):
    ...
    exclusion_labels = Nested(
        ExclusionLabel, many=True, description="a report of roi-wise problems detected during extraction"
    )
