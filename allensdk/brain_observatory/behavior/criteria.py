...
def two_out_of_three_aint_bad(session_summary):
    ...
    if len(session_summary) < 3:
        raise DataFrameIndexError(
            "Not enough data in session_summary frame. Expected >= 3 rows, got {}".format(len(session_summary))
        )
    try:
        last_three = session_summary["dprime_peak"][-3:]
    except KeyError as e:
        raise DataFrameKeyError(
            "Failed accessing last three values in colum'dprime_peak'.\n df length={}, df columns={}\n".format(
                len(session_summary), list(session_summary)
            ),
            e,
        )
...
def yesterday_was_good(session_summary):
    ...
    if len(session_summary) < 1:
        raise DataFrameIndexError(
            "Not enough data in session_summary frame. Expected >= 1 row(s), got {}".format(len(session_summary))
        )
    try:
...
def no_response_bias(session_summary):
    ...
    try:
        ...
        raise DataFrameKeyError(
            "Failed accessing last values in colum'response_bias'.\n df length={}, df columns={}\n".format(
                len(session_summary), list(session_summary)
            ),
            e,
        )
    criteria = (response_bias < 0.9) & (response_bias > 0.1)
    logger.info("'No response bias' criteria met: {} (response bias={})".format(criteria, response_bias))
    return criteria
...
def whole_lotta_trials(session_summary):
    ...
    try:
        ...
        raise DataFrameKeyError(
            "Failed accessing last values in colum'num_contingent_trials'.\n df length={}, df columns={}\n".format(
                len(session_summary), list(session_summary)
            ),
            e,
        )
...
def mostly_useful(trials):
    ...
    logger.info(
        "Fewer than half the trials were aborted on the last training day: {} (% aborted trials={})".format(
            criteria, aborted
        )
    )
    return criteria
...
def meets_engagement_criteria(session_summary):
    ...
    try:
        session_summary["engagement_criteria"] = (session_summary["dprime_peak"] > 1.0) & (
