...
class OphysMixin:
    ...
    def __init__(self):
        ...
        if (
            "date_of_acquisition_behavior" in self._df
            and "date_of_acquisition_ophys" in self._df
        ):

            # Prioritize ophys_date_of_acquisition
            self._df["date_of_acquisition"] = self._df[
                "date_of_acquisition_ophys"
            ]
    ...
    def _clean_up_project_code(self):
        ...
        if (
            "project_code_behavior" in self._df
            and "project_code_ophys" in self._df
        ):
            ...
            if (self._df["project_code_ophys"].isna()).sum() == 0:
                ...
                if not self._df["project_code_behavior"][
                    ...
                    self._df["project_code_ophys"][has_ophys_session_mask]
                ):
                    warnings.warn(
                        "BehaviorSession and OphysSession "
            ...
            self._df = self._df.drop(
                ["project_code_ophys", "project_code_behavior"], axis=1
            )
