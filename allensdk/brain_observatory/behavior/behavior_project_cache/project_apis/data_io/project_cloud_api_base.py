...
from allensdk.api.cloud_cache.cloud_cache import S3CloudCache, LocalCache, StaticLocalCache

from allensdk.brain_observatory.behavior.behavior_project_cache.utils import version_check
...
class ProjectCloudApiBase(object):
    ...
    def load_manifest(self, manifest_name: Optional[str] = None):
        ...
        if self.cache._manifest.metadata_file_names is None:
            raise RuntimeError(
                f"{type(self.cache)} object has no metadata "
                f"file names. Check contents of the loaded "
                f"manifest file: {self.cache._manifest_name}"
            )

        if not self.skip_version_check:
            data_sdk_version = [i for i in self.cache._manifest._data_pipeline if i["name"] == "AllenSDK"][0]["version"]
            version_check(
    ...
    @classmethod
    def from_s3_cache(
        cls, cache_dir: Union[str, Path], bucket_name: str, project_name: str, ui_class_name: str
    ) -> "ProjectCloudApiBase":
        ...
        """
        cache = S3CloudCache(cache_dir, bucket_name, project_name, ui_class_name=ui_class_name)
        return cls(cache)

    @classmethod
    def from_local_cache(
        cls, cache_dir: Union[str, Path], project_name: str, ui_class_name: str, use_static_cache: bool = False
    ) -> "ProjectCloudApiBase":
        ...
        """
        if use_static_cache:
            cache = StaticLocalCache(cache_dir, project_name, ui_class_name=ui_class_name)
        else:
            cache = LocalCache(cache_dir, project_name, ui_class_name=ui_class_name)
    ...
    def _get_local_path(self, fname: Optional[str] = None, file_id: Optional[str] = None):
        ...
        if not exists:
            raise FileNotFoundError(
                f"You started a cache without a connection to s3 and {local_path} is not already on your system"
            )
        return local_path
