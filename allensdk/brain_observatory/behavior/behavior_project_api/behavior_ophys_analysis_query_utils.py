...
class Database(object):
    ...
    def __init__(self, database_server='visual_behavior_data', ):
...
def clean_and_timestamp(entry):
    entry = {k: (int(v) if is_int(v) else v) for k, v in entry.items()}
    entry = {k: (float(v) if is_float(v) else v) for k, v in entry.items()}
    entry.update({'entry_time_utc':str(datetime.datetime.utcnow())})
    return entry

def write_to_manifest_collection(manifest,overwrite = False):
    '''
    * single table
    ...
    * make a convenience function to retrieve the entire manifest
    ...
    for idx,row in manifest.iterrows():
        entry = row.to_dict()
        res = vb['ophys_data']['manifest'].find_one({'ophys_experiment_id':entry['ophys_experiment_id']})
        if res is None:
...
def write_to_dff_traces_collection(session,overwrite = False):
    '''
    * each row will be a document
    * index by:
     * cell_specimen_id
    ...
    for idx,row in dff_traces.iterrows():
        entry = row.to_dict()
        res = vb['ophys_data']['dff_traces'].find_one({'cell_roi_id':entry['cell_roi_id']})
        if res is None:
            # cast to simple int or float
            entry = clean_and_timestamp(entry)
...
def write_stimulus_response_to_collection(session):
    '''
    * each row will be a document
    ...
    * index by:  
    ...
    res = vb['ophys_data']['stimulus_response'].find_one({'ophys_experiment_id':int(session.ophys_experiment_id)})

    if res is None:
        df = session.get_stimulus_response_df().drop(['dff_trace','dff_trace_timestamps'],axis=1)
...
def get_stimulus_response(query={}):
    '''
    ...
    cols = ['stimulus_presentations_id','cell_specimen_id','mean_response', 'baseline_response', 'p_value']
    dfs = []
    for s in res:
        df = pd.DataFrame()
        for col in cols:
...
def write_stimulus_presentations_to_collection(session):
    '''
    * index by:  
     * ophys_experiment_id
     * ophys_session_id (not implemented)
    ...
    * each session will be one document
    ...
    res = vb['ophys_data']['stimulus_presentations'].find_one({'ophys_experiment_id':int(session.ophys_experiment_id)})

    if res is None:
        df = session.stimulus_presentations.drop(['licks','rewards'],axis=1).reset_index()
...
def get_stimulus_presentations(query={}):
    '''
    returns stimulus_response table
    pass query in the form:
        {KEY:value}
...
def write_metrics_to_collection(metrics_df):
    '''
