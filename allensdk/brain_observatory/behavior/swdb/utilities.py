...
def annotate_mean_df_with_pref_stim(mean_df):
    ...
    for cell in mdf.reset_index()["cell_specimen_id"].unique():
        if image_name == "image_name":
            assert (
                len(mdf.reset_index().set_index("cell_specimen_id").loc[cell].query("pref_stim").image_name.unique())
                == 1
            )
        else:
            assert (
...
def get_xticks_xticklabels(trace, ophys_frame_rate=31.0, interval_sec=1, window=[-4, 8]):
    """
...
def plot_flashes_on_trace(
    ax, window=[-4, 8], go_trials_only=False, omitted=False, flashes=False, alpha=0.25, facecolor="gray"
):
    """
...
def create_multi_session_mean_df(
    cache, experiment_ids, conditions=["cell_specimen_id", "change_image_name"], flashes=False
):
    """
    ...
    for experiment_id in experiment_ids:
        ...
        mdf["retake_number"] = manifest[
            manifest.ophys_experiment_id == session.metadata["ophys_experiment_id"]
        ].retake_number.values[0]
