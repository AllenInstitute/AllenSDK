...
def add_p_vals_tr(tr, response_window=[4, 4.5]):
    ...
    # Set up empty column
    tr["p_value"] = 1.0
    ophys_frame_rate = 31.0  # Shouldn't hard code this PROBLEM
...
def annotate_trial_response_df_with_pref_stim(trial_response_df):
    ...
    for cell in rdf.reset_index()["cell_specimen_id"].unique():
        assert (
            len(rdf.reset_index().set_index("cell_specimen_id").loc[cell].query("pref_stim").change_image_name.unique())
            == 1
        )
...
def get_trial_response_df(session, response_analysis_params):
    ...
    """
    frame_rate = 31.0  # PROBLEM, shouldnt hard code this here
