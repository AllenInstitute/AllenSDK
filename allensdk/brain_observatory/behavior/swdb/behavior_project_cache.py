...
cache_path_example = "/allen/programs/braintv/workgroups/nc-ophys/visual_behavior/SWDB_2019/cache_20190813"  # noqa: E501


@deprecated(
    "swdb.behavior_project_cache.BehaviorProjectCache is deprecated "
...
class BehaviorProjectCache(object):
    def __init__(self, cache_base):
        """
        A cache-level object for the behavior/ophys data. Provides access
        to the manifest of ophys/behavior containers, as well as
        pre-computed analysis files for each experiment.

        Args:
            cache_base (str): Path to the directory containing
            the cached behavior/ophys data
        ...
        Methods:
            get_session(ophys_experiment_id):
                Returns an extended BehaviorOphysSession object,
                including trial_response_df and
                flash_response_df

            get_container_sessions(container_id):
                Returns a dictionary with behavior stages as keys
        ...
        self.cache_paths = {
            "manifest_path": os.path.join(
                cache_base, "visual_behavior_data_manifest.csv"
            ),
            "nwb_base_dir": os.path.join(cache_base, "nwb_files"),
            "analysis_files_base_dir": os.path.join(
                cache_base, "analysis_files"
            ),
            "analysis_files_metadata_path": os.path.join(
                cache_base, "analysis_files_metadata.json"
            ),
        }

        self.experiment_table = csv_io["reader"](
            self.cache_paths["manifest_path"]
        )

        self.experiment_table["cre_line"] = self.experiment_table[
            "full_genotype"
        ].apply(parse_cre_line)
        self.experiment_table["passive_session"] = self.experiment_table[
            "stage_name"
        ].apply(parse_passive)
        self.experiment_table["image_set"] = self.experiment_table[
            "stage_name"
        ].apply(parse_image_set)
        ...
        self.analysis_files_base_dir = self.cache_paths[
    ...
    def get_session(self, experiment_id):
        ...
        extended_stim_df_path = self.get_extended_stimulus_presentations_df(
            experiment_id
        )
        api = ExtendedNwbApi(
            nwb_path,
...
def parse_cre_line(full_genotype):
    """
    Args:
        full_genotype (str): formatted from LIMS, e.g.
...
def parse_passive(behavior_stage):
    """
    Args:
        behavior_stage (str): the stage string, e.g.
        OPHYS_1_images_A or OPHYS_1_images_A_passive
    Returns:
...
class ExtendedNwbApi(BehaviorOphysNwbApi):
    def __init__(
        ...
        """
        super(ExtendedNwbApi, self).__init__(
            path=nwb_path, filter_invalid_rois=True
        )
    ...
    def get_flash_response_df(self):
        ...
        fdf = fdf.join(
            self.get_stimulus_presentations(), on="flash_id", how="left"
        )
        return fdf

    def get_extended_stimulus_presentations_df(self):
        return pd.read_hdf(
            self.extended_stimulus_presentations_df_path, key="df"
        )
    ...
    def get_task_parameters(self):
        ...
        We need to hard-code the omitted flash fraction and stimulus
        duration here.
        """
    ...
    def get_metadata(self):
        ...
        # For SWDB only
        # metadata should not include 'behavior_session_uuid' because it is
        # not useful to students and confusing
    ...
    def get_running_speed(self):
        # We want the running speed attribute to be a dataframe
        # (like licks, rewards, etc.) instead of a
        # RunningSpeed object. This will improve consistency for students.
        # For SWDB we have also opted to
        # have columns for both 'timestamps' and 'values' of things,
        # since this is more intuitive for students
    ...
    def get_trials(self, filter_aborted_trials=True):
        ...
        stimulus_presentations = super(
            ExtendedNwbApi, self
        ).get_stimulus_presentations()

        # Note: everything between dashed lines is a patch to deal with
        # timing issues in the AllenSDK
        # This should be removed in the future after issues
        # 876 and #802 are fixed.
        # -----------------------

        # gets start_time of next stimulus after timestamp in
        ...
        # This method can lead to a NaN change time for any trials at the
        # end of the session. However, aborted trials at the end of the
        # session also don't have change times. The safest method seems
        # like just droping any trials that aren't covered by the
        # stimulus_presentations
        ...
        trials = trials[
            np.logical_not(trials["stop_time"] > last_stimulus_presentation)
        ]

        # recalculates response latency based on corrected change time
        # and first lick time
        ...
        trials["response_latency"] = trials.apply(
            recalculate_response_latency, axis=1
        )
        # -------------------------------------------------------------------------------

        # asserts that every change time exists in the
        # stimulus_presentations table
        for change_time in trials[trials["change_time"].notna()][
            "change_time"
        ]:
        ...
        # Response_binary is just whether or not they responded
        # - e.g. true for hit or FA.
    ...
    def get_stimulus_presentations(self):
        ...
        extended_stimulus_presentations = (
            self.get_extended_stimulus_presentations_df()
        )
        extended_stimulus_presentations = extended_stimulus_presentations.drop(
            columns=["omitted"]
        )
        stimulus_presentations = stimulus_presentations.join(
            extended_stimulus_presentations
        )
        ...
        stimulus_presentations = stimulus_presentations.rename(
            columns={
                "index": "absolute_flash_number",
                "running_speed": "mean_running_speed",
            }
        )
        # Replace image set with A/B
        stimulus_presentations["image_set"] = self.get_task_parameters()[
            "stage"
        ][15]
    ...
    def get_stimulus_templates(self):
        # super stim templates is a dict with one annoyingly-long key,
        # so pop the val out
        stimulus_templates = super(
            ExtendedNwbApi, self
        ).get_stimulus_templates()
        stimulus_template_array = stimulus_templates[
            list(stimulus_templates.keys())[0]
        ]
    ...
    def get_licks(self):
        # Licks column 'time' should be 'timestamps' to be consistent
    ...
    def get_rewards(self):
        # Rewards has timestamps in the index which is confusing and not
        # consistent with the rest of the session. Use a normal index and
        # have timestamps as a column
        rewards = super(ExtendedNwbApi, self).get_rewards()
        rewards = rewards.reset_index()
        return rewards

    def get_dff_traces(self):
        # We want to drop the 'cell_roi_id' column from the dff traces
        # dataframe. This is just for Friday Harbor, not for eventual inclusion
        # in the LIMS api.
    ...
    def get_image_index_names(self):
        image_index_names = (
            ...
            .groupby("image_index")
            .apply(lambda group: one(group["image_name"].unique()))
        )
...
class ExtendedBehaviorSession(BehaviorOphysSession):
    ...
    LazyProperty attributes access the data only on the first demand, and
    then memoize the result for reuse.

    Attributes:
        ophys_experiment_id : int (LazyProperty)
            Unique identifier for this experimental session
        max_projection : allensdk.brain_observatory.behavior.image_api.Image
            (LazyProperty) 2D max projection image
        ...
        dff_traces : pandas.DataFrame (LazyProperty)
            The traces of dff organized into a dataframe; index is the
            cell roi ids
        segmentation_mask_image:
           allensdk.brain_observatory.behavior.image_api.Image (LazyProperty)
            An image with pixel value 1 if that pixel was included in an ROI,
            and 0 otherwise
        roi_masks: dict (LazyProperty)
            A dictionary with individual ROI masks for each cell specimen ID.
            Keys are cell specimen IDs, values are 2D numpy arrays.
        ...
        running_speed : pandas.DataFrame (LazyProperty)
            A dataframe containing the running_speed in cm/s and the timestamps
            of each data point
        ...
        stimulus_templates : dict (LazyProperty)
            ...
            session. Keys are image names, values are 2D numpy arrays.
        ...
        corrected_fluorescence_traces : pandas.DataFrame (LazyProperty)
            The motion-corrected fluorescence traces organized into a
            dataframe; index is the cell roi ids
        average_projection :
            allensdk.brain_observatory.behavior.image_api.Image (LazyProperty)
            2D image of the microscope field of view, averaged across
            the experiment
...
if __name__ == "__main__":
    ...
    session = cache.get_session(
        cache.experiment_table.iloc[0]["ophys_experiment_id"]
    )
