...
from allensdk.brain_observatory.behavior.data_objects.trials.trials import calculate_reward_rate
from allensdk.deprecated import deprecated

csv_io = {"reader": lambda path: pd.read_csv(path, index_col="Unnamed: 0"), "writer": lambda path, df: df.to_csv(path)}
...
class BehaviorProjectCache(object):
    def __init__(self, cache_base):
        ...
        self.experiment_table["cre_line"] = self.experiment_table["full_genotype"].apply(
    ...
    def get_extended_stimulus_presentations_df(self, experiment_id):
        return os.path.join(
            self.analysis_files_base_dir, "extended_stimulus_presentations_df_{}.h5".format(experiment_id)
        )
    ...
    def get_container_sessions(self, container_id):
        ...
        for ind_row, row in container_experiments.iterrows():
            container_stages.update({row["stage_name"]: self.get_session(row["ophys_experiment_id"])})
...
class ExtendedNwbApi(BehaviorOphysNwbApi):
    def __init__(
        self, nwb_path, trial_response_df_path, flash_response_df_path, extended_stimulus_presentations_df_path
    ):
        """
    ...
    def get_running_speed(self):
        ...
        return pd.DataFrame({"speed": running_speed.speed, "timestamps": running_speed.timestamps})
    ...
    def get_stimulus_presentations(self):
        ...
        stimulus_presentations = stimulus_presentations.rename(
            columns={"index": "absolute_flash_number", "running_speed": "mean_running_speed"}
        )
