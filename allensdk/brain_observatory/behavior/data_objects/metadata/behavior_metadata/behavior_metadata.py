...
from allensdk.brain_observatory.behavior.data_objects.metadata.behavior_metadata.equipment import Equipment
from allensdk.brain_observatory.behavior.data_objects.metadata.behavior_metadata.foraging_id import ForagingId
from allensdk.brain_observatory.behavior.data_objects.metadata.behavior_metadata.project_code import ProjectCode
from allensdk.brain_observatory.behavior.data_objects.metadata.behavior_metadata.session_type import SessionType
...
def get_expt_description(session_type: str) -> str:
    ...
    if len(match) != 1:
        emsg = (
            f"session type should match one and only one possible pattern "
...
def get_task_parameters(data: Dict) -> Dict:
    ...
    task_parameters["blank_duration_sec"] = [float(x) for x in doc["blank_duration_range"]]
    ...
    task_parameters["omitted_flash_fraction"] = behavior["params"].get("flash_omit_probability", float("nan"))
    task_parameters["response_window_sec"] = [float(x) for x in doc["response_window"]]
...
class BehaviorMetadata(
    DataObject, LimsReadableInterface, JsonReadableInterface, NwbReadableInterface, NwbWritableInterface
):
    ...
    def __init__(
        ...
        super().__init__(name="behavior_metadata", value=None, is_value_self=True)
    ...
    def from_lims(
        ...
        subject_metadata = SubjectMetadata.from_lims(behavior_session_id=behavior_session_id, lims_db=lims_db)
        equipment = Equipment.from_lims(behavior_session_id=behavior_session_id.value, lims_db=lims_db)

        stimulus_file = BehaviorStimulusFile.from_lims(
            db=lims_db, behavior_session_id=behavior_session_id.value
        ).validate()
        date_of_acquisition = DateOfAcquisition.from_stimulus_file(stimulus_file=stimulus_file)

        stimulus_frame_rate = StimulusFrameRate.from_stimulus_file(stimulus_file=stimulus_file)
        session_type = SessionType.from_stimulus_file(stimulus_file=stimulus_file)

        foraging_id = ForagingId.from_lims(behavior_session_id=behavior_session_id.value, lims_db=lims_db)
        behavior_session_uuid = BehaviorSessionUUID.from_stimulus_file(stimulus_file=stimulus_file).validate(
            behavior_session_id=behavior_session_id.value, foraging_id=foraging_id.value, stimulus_file=stimulus_file
        )
    ...
    def from_json(cls, dict_repr: dict) -> "BehaviorMetadata":
        ...
        stimulus_frame_rate = StimulusFrameRate.from_stimulus_file(stimulus_file=stimulus_file)
        session_type = SessionType.from_stimulus_file(stimulus_file=stimulus_file)
        session_uuid = BehaviorSessionUUID.from_stimulus_file(stimulus_file=stimulus_file)
    ...
    def to_nwb(self, nwbfile: NWBFile) -> NWBFile:
        ...
        extension = load_pynwb_extension(BehaviorMetadataSchema, "ndx-aibs-behavior-ophys")
