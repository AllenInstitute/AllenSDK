...
class BehaviorSessionId(
    ...
    def from_lims(cls, db: PostgresQueryMixin, ophys_experiment_id: int) -> "BehaviorSessionId":
        query = f"""
            SELECT bs.id
    ...
    @classmethod
    def from_ecephys_session_id(cls, db: PostgresQueryMixin, ecephys_session_id: int) -> "BehaviorSessionId":
        query = f"""
            SELECT bs.id
