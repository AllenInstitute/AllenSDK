...
from allensdk.brain_observatory.behavior.data_objects.timestamps.util import calc_frame_rate
from allensdk.internal.api import PostgresQueryMixin


class ImagingPlane(DataObject, LimsReadableInterface, JsonReadableInterface, NwbReadableInterface):
    def __init__(
        self, ophys_frame_rate: float, targeted_structure: str, excitation_lambda: float, indicator: Optional[str]
    ):
        super().__init__(name="imaging_plane", value=None, is_value_self=True)
    ...
    def from_lims(
        ...
        behavior_session_id = BehaviorSessionId.from_lims(db=lims_db, ophys_experiment_id=ophys_experiment_id)
    ...
    @classmethod
    def from_json(cls, dict_repr: dict, ophys_timestamps: OphysTimestamps, excitation_lambda=910.0) -> "ImagingPlane":
    ...
    @staticmethod
    def _get_targeted_structure_from_lims(ophys_experiment_id: int, lims_db: PostgresQueryMixin) -> str:
        query = """
                SELECT st.acronym
