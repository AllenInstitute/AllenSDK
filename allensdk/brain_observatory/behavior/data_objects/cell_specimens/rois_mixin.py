...
class RoisMixin:
    ...
    def filter_and_reorder(self, roi_ids: np.ndarray, raise_if_rois_missing=True):
        ...
        def handle_rois_in_input_not_in_dataframe():
            msg = f"Input contains roi ids not in {type(self).__name__}."
        ...
        if original_index_name != "cell_roi_id":
            self._value = self._value.reset_index().set_index("cell_roi_id")
        ...
        if original_index_name != "cell_roi_id":
            ...
            self._value = self._value.reset_index().set_index(original_index_name)
