...
class Events(DataObject, RoisMixin, DataFileReadableInterface, NwbReadableInterface, NwbWritableInterface):
    """Events
    columns:
        events: np.array
        lambda: float
    ...
    def __init__(
        ...
        frame_rate_hz
            ...
            unitless filter_scale for filter_events_array
            is filter_scale_seconds*frame_rate_hz
        filter_scale_seconds
        ...
        filtered_events = filter_events_array(
            arr=events, scale=filter_scale_seconds * frame_rate_hz, n_time_steps=filter_n_time_steps
        )
    ...
    def from_data_file(
        ...
        return cls(
            ...
            filter_scale_seconds=filter_scale_seconds,
    ...
    def to_nwb(self, nwbfile: NWBFile) -> NWBFile:
        ...
        cell_specimen_table = seg_interface.plane_segmentations["cell_specimen_table"]
        cell_specimen_df = cell_specimen_table.to_dataframe()

        # We only want to store the subset of rois that have events data
        rois_with_events_indices = [cell_specimen_df.index.get_loc(label) for label in events.index]
        roi_table_region = cell_specimen_table.create_roi_table_region(
            description="Cells with detected events", region=rois_with_events_indices
        )
