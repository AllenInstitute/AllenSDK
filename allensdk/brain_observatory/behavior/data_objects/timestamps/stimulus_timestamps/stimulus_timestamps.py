...
from allensdk.core import LimsReadableInterface, NwbReadableInterface, JsonReadableInterface
from allensdk.brain_observatory.behavior.data_files.sync_file import SyncFileReadableInterface
...
class StimulusTimestamps(
    ...
    def __init__(
        ...
        super().__init__(name="stimulus_timestamps", value=timestamps + monitor_delay)
        self._stimulus_file = stimulus_file
        self._sync_file = sync_file
        self._monitor_delay = monitor_delay

    def update_timestamps(self, timestamps: np.ndarray) -> "StimulusTimestamps":
        """
    ...
    def subtract_monitor_delay(self) -> "StimulusTimestamps":
        ...
        return StimulusTimestamps(timestamps=new_value, monitor_delay=0.0)
    ...
    @classmethod
    def from_stimulus_file(cls, stimulus_file: BehaviorStimulusFile, monitor_delay: float) -> "StimulusTimestamps":
        stimulus_timestamps = get_behavior_stimulus_timestamps(stimulus_pkl=stimulus_file.data)

        return cls(timestamps=stimulus_timestamps, monitor_delay=monitor_delay, stimulus_file=stimulus_file)

    @classmethod
    def from_sync_file(cls, sync_file: SyncFile, monitor_delay: float) -> "StimulusTimestamps":
        stimulus_timestamps = get_ophys_stimulus_timestamps(sync_path=sync_file.filepath)
        return cls(timestamps=stimulus_timestamps, monitor_delay=monitor_delay, sync_file=sync_file)

    @classmethod
    def from_json(cls, dict_repr: dict, monitor_delay=0.0) -> "StimulusTimestamps":
        """
        ...
        if "sync_file" in dict_repr:
            ...
            return cls.from_sync_file(sync_file=sync_file, monitor_delay=monitor_delay)
        else:
            stim_file = BehaviorStimulusFile.from_json(dict_repr=dict_repr)
            return cls.from_stimulus_file(stimulus_file=stim_file, monitor_delay=monitor_delay)

    @classmethod
    def from_multiple_stimulus_blocks(
        cls,
        sync_file: SyncFile,
        list_of_stims: List[Union[BehaviorStimulusFile, MappingStimulusFile, ReplayStimulusFile]],
        ...
        frame_count_tolerance: float = 0.0,
    ) -> "StimulusTimestamps":
        """
        ...
        frame_count_tolerance: float
            ...
            equal
        """
        behavior_stimulus_files = [x for x in list_of_stims if isinstance(x, BehaviorStimulusFile)]
        if len(behavior_stimulus_files) == 0:
            raise ValueError("One of the values in `list_of_stims` must be a `BehaviorStimulusFile`")
        elif len(behavior_stimulus_files) > 1:
            raise ValueError("You passed multiple `BehaviorStimulusFile` to `list_of_stims`. Please pass only 1.")
        ...
        if stims_of_interest:
            ...
            if any([x >= len(list_of_stims) for x in stims_of_interest]):
                raise ValueError(
                    "stims_of_interest contains an index greater than the number of elements in list_of_stims"
                )
        ...
        to_concatenate = (
            ...
            if stims_of_interest is None
            else [stimulus_times["timestamps"][idx] for idx in stims_of_interest]
        )
    ...
    def from_lims(
        ...
        if ophys_experiment_id:
            sync_file = SyncFile.from_lims(db=db, ophys_experiment_id=ophys_experiment_id)
            return cls.from_sync_file(sync_file=sync_file, monitor_delay=monitor_delay)
        else:
            return cls.from_stimulus_file(stimulus_file=stimulus_file, monitor_delay=monitor_delay)
    ...
    def from_nwb(cls, nwbfile: NWBFile) -> "StimulusTimestamps":
        ...
        return cls(timestamps=stim_timestamps, monitor_delay=0.0)

    def to_nwb(self, nwbfile: NWBFile) -> NWBFile:
        stimulus_ts = TimeSeries(data=self._value, name="timestamps", timestamps=self._value, unit="s")
