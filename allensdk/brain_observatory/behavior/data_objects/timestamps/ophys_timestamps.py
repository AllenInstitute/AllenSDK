...
class OphysTimestamps(DataObject, SyncFileReadableInterface, NwbReadableInterface):
    ...
    def from_nwb(cls, nwbfile: NWBFile) -> "OphysTimestamps":
        ts = nwbfile.processing["ophys"].get_data_interface("dff").roi_response_series["traces"].timestamps[:]
    ...
    def validate(self, number_of_frames: int) -> "OphysTimestamps":
        ...
        if number_of_frames < num_of_timestamps:
            ...
            raise RuntimeError(
                f"dff_frames (len={number_of_frames}) is longer than timestamps (len={num_of_timestamps})."
            )
        return self
...
class OphysTimestampsMultiplane(OphysTimestamps):
    ...
    @classmethod
    def from_sync_file(cls, sync_file: SyncFile, group_count: int, plane_group: int) -> "OphysTimestampsMultiplane":
        if group_count == 0:
