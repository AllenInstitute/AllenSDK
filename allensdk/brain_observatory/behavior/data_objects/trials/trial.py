...
class Trial:
    def __init__(
        ...
        self._trial = trial
        self._start = start
        self._end = self._calculate_trial_end(trial_end=end, behavior_stimulus_file=behavior_stimulus_file)
        self._index = index
        self._stimulus_timestamps = stimulus_timestamps
        self._sync_file = sync_file
        self._data = self._match_to_sync_timestamps(
            raw_stimulus_timestamps=stimulus_timestamps, licks=licks, rewards=rewards, stimuli=stimuli
        )
    ...
    def _match_to_sync_timestamps(
        self, raw_stimulus_timestamps: StimulusTimestamps, licks: Licks, rewards: Rewards, stimuli: dict
    ) -> Dict[str, Any]:
        """
        ...
        event_dict = {
            (e[0], e[1]): {"timestamp": stimulus_timestamps.value[e[3]], "frame": e[3]} for e in self._trial["events"]
        }
        ...
        if self._end > 0:
            valid_idx = np.where(np.logical_and(lick_frames > self._start, lick_frames <= self._end))
        else:
        ...
        tr_data["reward_time"] = self._get_reward_time(
            reward_times, event_dict[("trial_start", "")]["timestamp"], event_dict[("trial_end", "")]["timestamp"]
        )
        tr_data.update(self._get_trial_data())
        tr_data.update(
            self._get_trial_timing(
                event_dict,
                tr_data["lick_times"],
                tr_data["go"],
                tr_data["catch"],
    ...
    @staticmethod
    def _get_reward_time(rebased_reward_times, start_time, stop_time) -> float:
        """extract reward times in time range"""
        reward_times = rebased_reward_times[
            np.where(np.logical_and(rebased_reward_times >= start_time, rebased_reward_times <= stop_time))
        ]
        return float("nan") if len(reward_times) == 0 else one(reward_times)

    @staticmethod
    def _calculate_trial_end(trial_end, behavior_stimulus_file: BehaviorStimulusFile) -> int:
        if trial_end < 0:
    ...
    def _get_trial_data(self) -> Dict[str, Any]:
        ...
        * auto_rewarded (reward was automatically delivered following the
        ...
        return {
            "reward_volume": sum([r[0] for r in self._trial.get("rewards", [])]),
            "hit": hit,
    ...
    def _get_trial_timing(
        ...
        assert not (hit and false_alarm), (
            "both `hit` and `false_alarm` cannot be True, they are mutually exclusive categories"
        )
        assert not (go and catch), "both `go` and `catch` cannot be True, they are mutually exclusive categories"
        assert not (go and auto_rewarded), (
            "both `go` and `auto_rewarded` cannot be True, they are mutually exclusive categories"
        )
        ...
        change_frame = self.calculate_change_frame(
            event_dict=event_dict, go=go, catch=catch, auto_rewarded=auto_rewarded
        )
    ...
    def _get_trial_image_names(self, stimuli) -> Dict[str, str]:
        ...
        initial_image_category_name, _, initial_image_name = self._resolve_initial_image(stimuli, trial_start_frame)
        if len(self._trial["stimulus_changes"]) == 0:
            change_image_name = initial_image_name
        else:
            ((from_set, from_name), (to_set, to_name), _, _) = self._trial["stimulus_changes"][0]
        ...
        return {"initial_image_name": initial_image_name, "change_image_name": change_image_name}
