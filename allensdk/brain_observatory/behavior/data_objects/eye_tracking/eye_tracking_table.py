...
from allensdk.brain_observatory.behavior.data_files.eye_tracking_video import EyeTrackingVideo
from allensdk.brain_observatory.behavior.data_objects import StimulusTimestamps
from allensdk.brain_observatory.behavior.data_files.eye_tracking_file import EyeTrackingFile
from allensdk.brain_observatory.behavior.data_files.eye_tracking_metadata_file import EyeTrackingMetadataFile
from allensdk.core import DataObject
from allensdk.core import NwbReadableInterface, DataFileReadableInterface
...
from allensdk.brain_observatory.nwb.eye_tracking.ndx_ellipse_eye_tracking import EllipseSeries, EllipseEyeTracking


class EyeTrackingTable(DataObject, DataFileReadableInterface, NwbReadableInterface, NwbWritableInterface):
    ...
    def _get_empty_df(cls) -> pd.DataFrame:
        ...
        eye_tracking_data = pd.DataFrame(empty_data, index=pd.Index([], name="frame"))
        return eye_tracking_data

    @classmethod
    def from_nwb(
        cls, nwbfile: NWBFile, z_threshold: float = 3.0, dilation_frames: int = 2
    ) -> Optional["EyeTrackingTable"]:
        """
        Parameters
        -----------
        nwbfile
        ...
        try:
            ...
            warnings.warn(
                f"This nwb file with identifier '{int(nwbfile.identifier)}' has no eye tracking data. (NWB error: {e})"
            )
        ...
        corneal_reflection_tracking = eye_tracking_acquisition.corneal_reflection_tracking
    ...
    def from_data_file(
        ...
        """
        cls._logger.info(
            f"Getting eye_tracking_data with 'z_threshold={z_threshold}', 'dilation_frames={dilation_frames}'"
        )
        ...
        is_metadata_frame_present = (
            _is_metadata_frame_present(metadata_file=metadata_file, video=video)
            if metadata_file is not None or video is not None
            else False
        )

        try:
            frames, stimulus_timestamps = cls._validate_frame_time_alignment(
                frames=data_file.data.index.values,
                times=stimulus_timestamps,
            ...
            eye_tracking_data = process_eye_tracking_data(
                eye_data, stimulus_timestamps.value, z_threshold, dilation_frames
            )
    ...
    def _validate_frame_time_alignment(
        cls, frames: np.ndarray, times: StimulusTimestamps, is_metadata_frame_present: bool = False
        ...
        if len(times) > len(frames):
            ...
            times = times.update_timestamps(timestamps=times.value[: len(frames)])
...
def get_lost_frames(eye_tracking_metadata: EyeTrackingMetadataFile) -> np.ndarray:
    """
...
def _is_metadata_frame_present(
    metadata_file: Optional[EyeTrackingMetadataFile] = None, video: Optional[EyeTrackingVideo] = None
) -> bool:
    ...
    if metadata_file is not None:
        video_file_name = metadata_file.data["RecordingReport"]["VideoOutputFileName"].lower()
    ...
    return video_file_name.suffix == ".mp4" or "mvr" in video_file_name.name
