...
from allensdk.brain_observatory.behavior.stimulus_processing import get_stimulus_templates
...
class Templates(DataObject, StimulusFileReadableInterface, NwbReadableInterface, NwbWritableInterface):
    def __init__(self, templates: Dict[str, StimulusTemplate]):
        ...
        image_template_keys = [key for key in templates.keys() if "movie" not in key.lower()]
        ...
        if len(image_template_keys) == 1:
            ...
            error_message += f"Found multiple image StimulusTemplates {image_template_keys}. "

        movie_template_keys = [key for key in templates.keys() if "movie" in key.lower()]
        ...
        if len(movie_template_keys) == 1:
            ...
            error_message += f"Found multiple fingerprint movie StimulusTemplates {movie_template_keys}. "
    ...
    def from_stimulus_file(
        ...
        gratings_dir = os.path.join(gratings_dir, "prod5/project_VisualBehavior")
        grating_images_dict = {
            "gratings_0.0": {
                "warped": np.asarray(
                    imageio.imread(safe_system_path(os.path.join(gratings_dir, "warped_grating_0.png")))
                ),
                "unwarped": np.asarray(
                    imageio.imread(safe_system_path(os.path.join(gratings_dir, "masked_unwarped_grating_0.png")))
                ),
            },
            "gratings_90.0": {
                "warped": np.asarray(
                    imageio.imread(safe_system_path(os.path.join(gratings_dir, "warped_grating_90.png")))
                ),
                "unwarped": np.asarray(
                    imageio.imread(safe_system_path(os.path.join(gratings_dir, "masked_unwarped_grating_90.png")))
                ),
            },
            "gratings_180.0": {
                "warped": np.asarray(
                    imageio.imread(safe_system_path(os.path.join(gratings_dir, "warped_grating_180.png")))
                ),
                "unwarped": np.asarray(
                    imageio.imread(safe_system_path(os.path.join(gratings_dir, "masked_unwarped_grating_180.png")))
                ),
            },
            "gratings_270.0": {
                "warped": np.asarray(
                    imageio.imread(safe_system_path(os.path.join(gratings_dir, "warped_grating_270.png")))
                ),
                "unwarped": np.asarray(
                    imageio.imread(safe_system_path(os.path.join(gratings_dir, "masked_unwarped_grating_270.png")))
                ),
            },
        }
        ...
        stim_template = get_stimulus_templates(
            pkl=pkl, grating_images_dict=grating_images_dict, limit_to_images=limit_to_images
        )
        t = {stim_template.image_set_name: stim_template}

        has_fingerprint_stimulus = "fingerprint" in pkl["items"]["behavior"]["items"]
        if has_fingerprint_stimulus and load_stimulus_movie:
            movie_data = np.load(
                Path(pkl["items"]["behavior"]["items"]["fingerprint"]["static_stimulus"]["movie_path"])
            )
    ...
    def from_nwb(cls, nwbfile: NWBFile) -> "Templates":
        ...
        for image_set_name, image_data in nwbfile.stimulus_template.items():
            image_attributes = [{"image_name": image_name} for image_name in image_data.control_description]
    ...
    def to_nwb(self, nwbfile: NWBFile, stimulus_presentations: Presentations) -> NWBFile:
        for key, stimulus_templates in self.value.items():
            ...
            visual_stimulus_image_series = StimulusTemplateExtension(
        ...
        if "image_index" in stimulus_presentations.value and self._image_template_key is not None:
            nwbfile = self._add_image_index_to_nwb(nwbfile=nwbfile, presentations=stimulus_presentations)

        return nwbfile

    def _add_image_index_to_nwb(self, nwbfile: NWBFile, presentations: Presentations):
        ...
        stimulus_name = "image_set" if "image_set" in presentations else "stimulus_name"
        stimulus_index = presentations[presentations[stimulus_name] == nwb_template.name]
