...
class FingerprintStimulus:
    ...
    def from_stimulus_file(
        ...
        """
        fingerprint_stim = stimulus_file.data["items"]["behavior"]["items"]["fingerprint"]["static_stimulus"]
        ...
        stimulus_session_frame_indices = np.array(
            stimulus_file.data["items"]["behavior"]["items"]["fingerprint"]["frame_indices"]
        )
        ...
        for repeat in range(n_repeats):
            for frame in range(movie_length):
                ...
                # movie
                stimulus_frame_indices = np.array(fingerprint_stim["sweep_frames"][frame + (repeat * movie_length)])
                start_frame, end_frame = stimulus_session_frame_indices[stimulus_frame_indices + movie_start_index]
                start_time, stop_time = stimulus_timestamps.value[
                    [
                        start_frame,
                        ...
                        min(end_frame + 1, len(stimulus_timestamps.value) - 1),
                    ]
                ]
                res.append(
                    {
        ...
        table["stimulus_block"] = (
            stimulus_presentations["stimulus_block"].max() + 2
