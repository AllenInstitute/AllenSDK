...
class Rewards(DataObject, StimulusFileReadableInterface, NwbReadableInterface, NwbWritableInterface):
    def __init__(self, rewards: pd.DataFrame):
        super().__init__(name="rewards", value=rewards)

    @classmethod
    def from_stimulus_file(
        cls, stimulus_file: BehaviorStimulusFile, stimulus_timestamps: StimulusTimestamps
    ) -> "Rewards":
        ...
        if not np.isclose(stimulus_timestamps.monitor_delay, 0.0):
            msg = (
                "Instantiating rewards with monitor_delay = "
        ...
        for idx, trial in trial_df.iterrows():
            ...
            if rewards:
                ...
                rewards_dict["timestamps"].append(stimulus_timestamps.value[rewards[0][2]])
    ...
    def from_nwb(cls, nwbfile: NWBFile) -> Optional["Rewards"]:
        ...
        df = pd.DataFrame({"volume": volume, "timestamps": time, "auto_rewarded": autorewarded})
        return cls(rewards=df)
    ...
    def to_nwb(self, nwbfile: NWBFile) -> NWBFile:
        ...
        reward_volume_ts = TimeSeries(
            name="volume", data=self.value["volume"].values, timestamps=self.value["timestamps"].values, unit="mL"
        )
