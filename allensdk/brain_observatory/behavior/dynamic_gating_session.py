...
class DynamicGatingSession(
    ...
    def from_json(
        ...
        if running_speed_load_from_multiple_stimulus_files:
            running_acquisition = (
                RunningAcquisition.from_dual_stimulus_files(
            ...
            raw_running_speed = RunningSpeed.from_dual_stimulus_files(
                behavior_stimulus_file=(
                    BehaviorStimulusFile.from_json(dict_repr=session_data)
                ),
                mapping_stimulus_file=MappingStimulusFile.from_json(
            ...
            running_speed = RunningSpeed.from_dual_stimulus_files(
                behavior_stimulus_file=(
                    BehaviorStimulusFile.from_json(dict_repr=session_data)
                ),
                mapping_stimulus_file=MappingStimulusFile.from_json(
        ...
        print(task_parameters.no_reward)
    ...
    def from_nwb_path(cls, nwb_path: str, **kwargs) -> "DynamicGatingSession":
        ...
        Returns
        -------
        An instantiation of a `DynamicGatingSession`
        """
    ...
    def _read_licks(
        ...
        stimulus_timestamps = StimulusTimestamps.from_dual_stimulus_blocks(sync_file, [BehaviorStimulusFile(stimulus_file_lookup.behavior_stimulus_file.filepath), 
                                                                                      MappingStimulusFile(stimulus_file_lookup.mapping_stimulus_file.filepath)],
                                                                                      monitor_delay=0.0)
    ...
    def _read_trials(
        ...
        """
        Construct the DynamicGatingTrials data object for this session
        """
    ...
    @classmethod
    def extract_lost_frames_from_json(cls, cam_json):
    ...
    @classmethod
    def read_json(cls, jsonfilepath):
    ...
    @classmethod
    def get_frame_exposure_times(cls, sync_dataset, cam_json):
    
        if isinstance(cam_json, str):
            cam_json = cls.read_json(cam_json)
        ...
        lost_frames = cls.extract_lost_frames_from_json(cam_json)
    ...
    def _read_eye_tracking_table(
        ...
        with open(sync_path) as sync:
            sync_dataset = SyncDataset(sync_path)

        frame_times = cls.get_frame_exposure_times(sync_dataset, eye_tracking_metadata_file.filepath)
        ...
        stimulus_timestamps = StimulusTimestamps(
            timestamps=frame_times, monitor_delay=0.0
        )
    ...
    @classmethod
    def _trials_class(cls) -> Type[DynamicGatingTrials]:
        return DynamicGatingTrials
