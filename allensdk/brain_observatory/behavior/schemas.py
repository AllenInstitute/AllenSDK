...
class BehaviorMetadataSchema(RaisingSchema):
    ...
    behavior_session_id = fields.Int(doc="The unique ID for the behavior session", required=True)
...
class OphysMetadataSchema(NwbOphysMetadataSchema):
    ...
    ophys_experiment_id = fields.Int(doc="Unique ID for the ophys experiment (aka imaging plane)", required=True)
    ophys_session_id = fields.Int(doc="Unique ID for the ophys session", required=True)
    ...
    imaging_plane_group = fields.Int(
        doc=(
            "A numeric index which indicates the order that an imaging plane "
    ...
    imaging_plane_group_count = fields.Int(
        doc=(
            "The total number of plane groups collected in a session "
...
class BehaviorTaskParametersSchema(RaisingSchema):
    ...
    blank_duration_sec = fields.List(
        fields.Float,
        doc=(
            "The lower and upper bound (in seconds) for a randomly chosen inter-stimulus interval duration for a trial"
        ),
        required=True,
        shape=(2,),
    )
    stimulus_duration_sec = fields.Float(
        doc="Duration of each stimulus presentation in seconds", required=True, allow_nan=True
    )
    ...
    response_window_sec = fields.List(
        fields.Float,
        doc=(
            "The lower and upper bound (in seconds) for a randomly chosen "
    ...
    stimulus_name = fields.String(doc="Name of the stimulus file set presented", required=False)


class EyeTrackingRigGeometry(RaisingSchema):
    """Eye tracking rig geometry"""

    values = fields.Float(doc="position/rotation with respect to (x, y, z)", required=True, shape=(3,))
    unit_of_measurement = fields.Str(doc="Unit of measurement for the data", required=True)
...
class OphysEyeTrackingRigMetadataSchema(RaisingSchema):
    ...
    equipment = fields.Str(doc="Description of rig", required=True)
    monitor_position = fields.Nested(EyeTrackingRigGeometry, doc="position of monitor (x, y, z)", required=True)
    camera_position = fields.Nested(EyeTrackingRigGeometry, doc="position of camera (x, y, z)", required=True)
    led_position = fields.Nested(EyeTrackingRigGeometry, doc="position of LED (x, y, z)", required=True)
    monitor_rotation = fields.Nested(EyeTrackingRigGeometry, doc="rotation of monitor (x, y, z)", required=True)
    camera_rotation = fields.Nested(EyeTrackingRigGeometry, doc="rotation of camera (x, y, z)", required=True)
