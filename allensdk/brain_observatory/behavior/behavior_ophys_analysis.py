...
def plot_example_traces_and_behavior(
    dataset, cell_roi_ids, xmin_seconds, length_mins, save_dir=None, include_running=False, cell_label=False
):
    suffix = ""
    if include_running:
        n = 2
    ...
    def plot_example_traces_and_behavior(self, N=10):
        ...
        for xmin_seconds in np.arange(0, 5000, length_mins * 60):
            plot_example_traces_and_behavior(
                self.session, active_cell_roi_ids, xmin_seconds, length_mins, cell_label=False, include_running=True
            )
