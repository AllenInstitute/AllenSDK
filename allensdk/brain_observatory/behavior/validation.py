...
def validate_ophys_timestamps(ophys_experiment_id, number_of_frames):
    ...
    if raw_data_shape[0] != ophys_timestamps_shape[0]:
        raise ValidationError(
            'ophys_timestamp length does not match raw data length')


def validate_last_trial_ends_adjacent_to_flash(ophys_experiment_id, api=None,
                                               verbose=False):
    """
    ...
    The last trial has to have ended somewhere between the two carrots where:
      ...
      should have started, after accounting for the possibility of the
      session ending on an omitted flash
    ...
    N_final_omitted_flashes = session.stimulus_presentations.index.max() - \
        session.stimulus_presentations.query(
        'omitted == False').index.max()

    # get the start/end time of the last valid (non-omitted) flash
    last_flash_start = \
        session.stimulus_presentations.query('omitted == False')[
            'start_time'].iloc[-1]
    last_flash_end = \
        session.stimulus_presentations.query('omitted == False')['stop_time']\
        .iloc[-1]

    # calculate when the next stimulus should have flashed, after accounting
    # for any omitted flashes at the end of the session
    next_flash_would_have_started = \
        last_flash_end + max_blank_duration + \
        N_final_omitted_flashes * (max_flash_duration + max_blank_duration)
    ...
    if not last_flash_start <= last_trial_end <= next_flash_would_have_started:
        raise ValidationError(
            'The last trial does not end between the start of the last flash '
            'and the expected start time of the next flash')


def main():
    ophys_experiment_id_list = [775614751, 778644591, 787461073, 782675436,
                                783928214, 783927872,
                                787501821, 787498309, 788490510, 788488596,
                                788489531, 789359614,
                                790149413, 790709081, 791119849, 791453282,
                                791980891, 792813858,
                                792812544, 792816531, 792815735, 794381992,
                                794378505, 795076128,
                                795073741, 795952471, 795952488, 795953296,
                                795948257, 796106850,
                                796106321, 796108483, 796105823, 796308505,
                                797255551, 795075034,
                                798403387, 798404219, 799366517, 799368904,
                                799368262, 803736273,
                                805100431, 805784331, 805784313, 806456687,
                                806455766, 806989729,
                                807753318, 807752719, 807753334, 807753920,
                                796105304, 784482326,
