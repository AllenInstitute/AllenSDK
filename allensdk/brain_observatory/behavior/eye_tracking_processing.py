...
def load_eye_tracking_hdf(eye_tracking_file: Path) -> pd.DataFrame:
    ...
    for field_name in eye_tracking_fields:
        ...
        new_col_name_map = {col_name: f"{field_name}_{col_name}" for col_name in field_data.columns}
...
def determine_likely_blinks(
    eye_areas: pd.Series, pupil_areas: pd.Series, outliers: pd.Series, dilation_frames: int = 2
) -> pd.Series:
...
def process_eye_tracking_data(
    eye_data: pd.DataFrame, frame_times: pd.Series, z_threshold: float = 3.0, dilation_frames: int = 2
) -> pd.DataFrame:
    ...
    if n_sync != n_eye_frames:
        raise EyeTrackingError(
            f"Error! The number of sync file frame times "
            f"({len(frame_times)}) does not match the "
            f"number of eye tracking frames "
            f"({len(eye_data.index)})!"
        )

    cr_areas = eye_data[["cr_width", "cr_height"]].apply(compute_elliptical_area, axis=1)
    eye_areas = eye_data[["eye_width", "eye_height"]].apply(compute_elliptical_area, axis=1)
    pupil_areas = eye_data[["pupil_width", "pupil_height"]].apply(compute_circular_area, axis=1)
    ...
    likely_blinks = determine_likely_blinks(eye_areas, pupil_areas, outliers, dilation_frames=dilation_frames)
