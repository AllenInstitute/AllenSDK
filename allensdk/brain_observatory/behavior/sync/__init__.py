...
def get_raw_stimulus_frames(dataset: SyncDataset, permissive: bool = False) -> np.ndarray:
    ...
    """
    try:
        return dataset.get_edges(kind="falling", keys=["vsync_stim", "stim_vsync"], units="seconds")
    except KeyError:
        if not permissive:
            raise
        return
...
def get_ophys_frames(dataset: SyncDataset, permissive: bool = False) -> np.ndarray:
    ...
    """
    try:
        return dataset.get_edges(kind="rising", keys=["vsync_2p", "2p_vsync"], units="seconds")
    except KeyError:
        if not permissive:
            raise
        return
...
def get_lick_times(dataset: SyncDataset, permissive: bool = False) -> Optional[np.ndarray]:
    ...
    """
    return dataset.get_edges("rising", ["lick_times", "lick_sensor"], "seconds", permissive)


def get_stim_photodiode(dataset: SyncDataset, permissive: bool = False) -> Optional[List[float]]:
    ...
    """
    return dataset.get_edges("all", ["stim_photodiode", "photodiode"], "seconds", permissive)


def get_trigger(dataset: SyncDataset, permissive: bool = False) -> Optional[np.ndarray]:
    ...
    """
    keys = ["2p_trigger", "acq_trigger", "2p_acq_trigger", "2p_acquiring", "stim_running"]
    return dataset.get_edges(kind="rising", keys=keys, units="seconds", permissive=permissive)


def get_eye_tracking(dataset: SyncDataset, permissive: bool = False) -> Optional[np.ndarray]:
...
def get_behavior_monitoring(dataset: SyncDataset, permissive: bool = False) -> Optional[np.ndarray]:
...
def get_sync_data(sync_path: str, permissive: bool = False) -> Dict[str, Union[List, np.ndarray, None]]:
