    ...
    if rule == "clean":
        ...
        subprocess.check_call(
            [
                ...
                "build",
                "--build-arg",
                "MODELFILE={modelfile}".format(modelfile="modelfile.zip"),
                "-t",
                "dlc-eye-tracking:stage-1",
                "-f",
                DOCKERFILE_STAGE_1,
                "stage_1",
            ]
        ...
        subprocess.check_call(
            ["docker", "build", "-t", "dlc-eye-tracking:stage-2", "-f", DOCKERFILE_STAGE_2, "stage_2"]
        )

    elif rule == "build:stage-4":
        subprocess.check_call(
            ["docker", "build", "-t", "dlc-eye-tracking:stage-4", "-f", DOCKERFILE_STAGE_4, "stage_4"]
        )
        ...
        subprocess.check_call(
            [
                ...
                "--runtime=nvidia",
                "-e",
                "VIDEO_INPUT_FILE={}".format(video_input_file),
                "dlc-eye-tracking:{}".format(stage),
            ]
        )
        ...
        subprocess.check_call(
            [
                ...
                "tag",
                "dlc-eye-tracking:{}".format(stage),
                "us.gcr.io/aibs-pilot/dlc-eye-tracking:{}".format(stage),
            ]
        )
        ...
        subprocess.check_call(
            [
                ...
                "tag",
                "dlc-eye-tracking:{}".format(stage),
                "docker.aibs-artifactory.corp.alleninstitute.org/dlc-eye-tracking:{}".format(stage),
            ]
        )
        ...
        stage = rule.split(":")[-1]
        subprocess.check_call(
            ["docker", "push", "docker.aibs-artifactory.corp.alleninstitute.org/dlc-eye-tracking:{}".format(stage)]
        )
