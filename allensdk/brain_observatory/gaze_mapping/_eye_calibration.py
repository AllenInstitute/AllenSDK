...
def compute_ellipse_areas(ellipse_params) -> pd.Series:
    """Compute circular area of a pupil or eye ellipse using major axis.
    ...
    Args:
        ellipse_params (pandas.DataFrame): A table of pupil/eye parameters
            consisting of 5 columns ("center_x", "center_y", "height",
            "phi", "width") and n-row timepoints.
    ...
    # assume that it is the pupil/eye circle diameter.
    diameter = ellipse_params[["height", "width"]].max(axis=1)
    radii = diameter / 2
...
def project_to_plane(plane_normal, plane_point, points):
    ...
    '''
    factors = np.dot(plane_point, plane_normal) / np.dot(points, plane_normal)

    return (factors * points.T).T
