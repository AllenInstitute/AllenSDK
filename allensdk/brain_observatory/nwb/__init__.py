...
CELL_SPECIMEN_COL_DESCRIPTIONS = {
    ...
    "mask_image_plane": "Which image plane an ROI resides on. Overlapping ROIs "  # noqa: E501
...
def check_nwbfile_version(
    nwbfile_path: str, desired_minimum_version: str, warning_msg: str
):
...
def read_eye_gaze_mappings(input_path: Path) -> dict:
    ...
    eye_gaze_data["raw_eye_areas"] = pd.read_hdf(
        input_path, key="raw_eye_areas"
    )
    eye_gaze_data["raw_pupil_areas"] = pd.read_hdf(
        input_path, key="raw_pupil_areas"
    )
    eye_gaze_data["raw_screen_coordinates"] = pd.read_hdf(
        input_path, key="raw_screen_coordinates"
    )
    eye_gaze_data["raw_screen_coordinates_spherical"] = pd.read_hdf(
        input_path, key="raw_screen_coordinates_spherical"
    )

    eye_gaze_data["new_eye_areas"] = pd.read_hdf(
        input_path, key="new_eye_areas"
    )
    eye_gaze_data["new_pupil_areas"] = pd.read_hdf(
        input_path, key="new_pupil_areas"
    )
    eye_gaze_data["new_screen_coordinates"] = pd.read_hdf(
        input_path, key="new_screen_coordinates"
    )
    eye_gaze_data["new_screen_coordinates_spherical"] = pd.read_hdf(
        input_path, key="new_screen_coordinates_spherical"
    )

    eye_gaze_data["synced_frame_timestamps"] = pd.read_hdf(
        input_path, key="synced_frame_timestamps"
    )
    ...
    eye_gaze_mapping_df = pd.DataFrame(
        {
            ...
            "raw_screen_coordinates_x_cm": eye_gaze_data[
                "raw_screen_coordinates"
            ]["x_pos_cm"].values,
            "raw_screen_coordinates_y_cm": eye_gaze_data[
                "raw_screen_coordinates"
            ]["y_pos_cm"].values,
            "raw_screen_coordinates_spherical_x_deg": eye_gaze_data[
                "raw_screen_coordinates_spherical"
            ]["x_pos_deg"].values,
            "raw_screen_coordinates_spherical_y_deg": eye_gaze_data[
                "raw_screen_coordinates_spherical"
            ]["y_pos_deg"].values,
            "filtered_eye_area": eye_gaze_data["new_eye_areas"].values,
            "filtered_pupil_area": eye_gaze_data["new_pupil_areas"].values,
            "filtered_screen_coordinates_x_cm": eye_gaze_data[
                "new_screen_coordinates"
            ]["x_pos_cm"].values,
            "filtered_screen_coordinates_y_cm": eye_gaze_data[
                "new_screen_coordinates"
            ]["y_pos_cm"].values,
            "filtered_screen_coordinates_spherical_x_deg": eye_gaze_data[
                "new_screen_coordinates_spherical"
            ]["x_pos_deg"].values,
            "filtered_screen_coordinates_spherical_y_deg": eye_gaze_data[
                "new_screen_coordinates_spherical"
            ]["y_pos_deg"].values,
        },
...
def eye_tracking_data_is_valid(
...
def create_eye_tracking_nwb_processing_module(
    ...
    eye_tracking_mod = pynwb.ProcessingModule(
        name="eye_tracking", description="Eye tracking processing module"
    )
    ...
    cr_params = pynwb.core.DynamicTable.from_dataframe(
        df=cr_fits, name="cr_ellipse_fits"
    )
...
def add_eye_gaze_mapping_data_to_nwbfile(
    nwbfile: pynwb.NWBFile, eye_gaze_data: dict
) -> pynwb.NWBFile:
    (
        raw_gaze_mapping_mod,
        filt_gaze_mapping_mod,
...
def add_running_speed_to_nwbfile(
    nwbfile, running_speed, name="speed", unit="cm/s", from_dataframe=False
):
    ...
    running_speed_series = pynwb.base.TimeSeries(
        name=name, data=data, timestamps=timestamps, unit=unit
    )

    if "running" in nwbfile.processing:
        running_mod = nwbfile.processing["running"]
    else:
        running_mod = ProcessingModule(
...
def add_stimulus_template(nwbfile, image_data, name):
    ...
    visual_stimulus_image_series = ImageSeries(
        name=name,
        data=image_data,
        unit="NA",
        format="raw",
...
def create_stimulus_presentation_time_interval(
    ...
    column_descriptions = {
        ...
        "stimulus_block": "Index of contiguous presentations of one stimulus type",  # noqa: E501
...
def add_stimulus_presentations(
    nwbfile, stimulus_table, tag="stimulus_time_interval"
):
    ...
    stimulus_table: pd.DataFrame
        Each row corresponds to an interval of time. Columns define
        the interval
        (start and stop time) and its characteristics.
        Nans in columns with string data will be replaced with
        the empty strings.
    ...
    stimulus_name_column = get_column_name(
        stimulus_table.columns, possible_names
    )
...
def setup_table_for_invalid_times(invalid_epochs):
    ...
    if invalid_epochs:
        ...
        tags = [
            [_type, str(_id), label]
            for _type, _id, label in zip(df["type"], df["id"], df["label"])
        ]
...
def setup_table_for_epochs(table, timeseries, tag):
    ...
    indices = np.searchsorted(
        timeseries.timestamps[:], table["start_time"].values
    )
    if len(indices > 0):
        diffs = np.concatenate(
            [np.diff(indices), [table.shape[0] - indices[-1]]]
        )
    else:
        diffs = []
    ...
    table["timeseries"] = [
        [[indices[ii], diffs[ii], timeseries]] for ii in range(table.shape[0])
    ]
    return table


def add_stimulus_timestamps(
    nwbfile, stimulus_timestamps, module_name="stimulus"
):
...
def add_trials(nwbfile, trials, description_dict={}):
    ...
    for c in [
        c for c in trials.columns if c not in ["start_time", "stop_time"]
    ]:
        ...
        if not len(data) == len(order):
            ...
            nwbfile.add_trial_column(
                name=c,
                description=description_dict.get(c, "NOT IMPLEMENTED: %s" % c),
                data=data,
                index=index,
            )
        else:
...
def add_licks(nwbfile, licks):
    ...
    licks_mod = ProcessingModule(
        "licking", "Licking behavior processing module"
    )
...
def add_rewards(nwbfile, rewards_df):
    ...
    rewards_mod = ProcessingModule(
        "rewards", "Licking behavior processing module"
    )
...
def add_image(
    ...
    if isinstance(image_data, sitk.Image):
        ...
        raise ValueError(
            "Not a supported image_data type: {}".format(type(image_data))
        )
...
def add_segmentation_mask_image(
    nwbfile, segmentation_mask_image, image_api=None
):
...
def add_metadata(nwbfile, metadata: dict, behavior_only: bool):
    ...
    subject_metadata = {
        k: v for k, v in metadata_clean.items() if k in subject_fields
    }
    for subject_key in subject_metadata.keys():
        metadata_clean.pop(subject_key, None)

    BehaviorSubject = load_pynwb_extension(
        SubjectMetadataSchema, "ndx-aibs-behavior-ophys"
    )
    ...
    if behavior_only:
        BehaviorMetadata = load_pynwb_extension(
            BehaviorMetadataSchema, "ndx-aibs-behavior-ophys"
        )
        ...
        nwb_metadata = OphysBehaviorMetadata(
            name="metadata", **new_metadata_dict
        )
...
def add_cell_specimen_table(
    nwbfile: NWBFile, cell_specimen_table: pd.DataFrame, session_metadata: dict
):
    """
    ...
    nwbfile.create_device(
        device_name, "Allen Brain Observatory - Scientifica 2P Rig"
    )
    ...
    for col_name in cell_roi_table.columns:
        # the columns 'roi_mask', 'pixel_mask', and 'voxel_mask' are already
        # defined in the nwb.ophys::PlaneSegmentation Object
        if col_name not in [
            "id",
            "mask_matrix",
            "roi_mask",
            ...
            # This builds the columns with name of column and description of
            # column both equal to the column name in the cell_roi_table
...
def add_corrected_fluorescence_traces(nwbfile, corrected_fluorescence_traces):
    corrected_fluorescence_traces = (
        corrected_fluorescence_traces.reset_index().set_index("cell_roi_id")[
