...
CELL_SPECIMEN_COL_DESCRIPTIONS = {
    "cell_specimen_id": "Unified id of segmented cell across experiments (after cell matching)",
    ...
    "max_correction_right": "Max motion correction in right direction in pixels",
    "valid_roi": "Indicates if cell classification found the ROI to be a cell or not",
    "x": "x position of ROI in Image Plane in pixels (top left corner)",
    "y": "y position of ROI in Image Plane in pixels (top left corner)",
}


def check_nwbfile_version(nwbfile_path: str, desired_minimum_version: str, warning_msg: str):
    ...
    if nwb_version is None:
        warnings.warn(
            f"'{nwbfile_path}' doesn't appear to be a valid "
    ...
    eye_gaze_mapping_df = pd.DataFrame(
        {
            ...
            "raw_screen_coordinates_spherical_x_deg": eye_gaze_data["raw_screen_coordinates_spherical"][
                "x_pos_deg"
            ].values,
            "raw_screen_coordinates_spherical_y_deg": eye_gaze_data["raw_screen_coordinates_spherical"][
                "y_pos_deg"
            ].values,
            ...
            "filtered_screen_coordinates_spherical_x_deg": eye_gaze_data["new_screen_coordinates_spherical"][
                "x_pos_deg"
            ].values,
            "filtered_screen_coordinates_spherical_y_deg": eye_gaze_data["new_screen_coordinates_spherical"][
                "y_pos_deg"
            ].values,
        },
...
def eye_tracking_data_is_valid(eye_dlc_tracking_data: dict, synced_timestamps: pd.Series) -> bool:
    is_valid = True
...
def create_eye_tracking_nwb_processing_module(
    eye_dlc_tracking_data: dict, synced_timestamps: pd.Series
) -> pynwb.ProcessingModule:
    # Top level container for eye tracking processed data
    eye_tracking_mod = pynwb.ProcessingModule(name="eye_tracking", description="Eye tracking processing module")

    # Data interfaces of dlc_fits_container
    pupil_fits = eye_dlc_tracking_data["pupil_params"].assign(timestamps=synced_timestamps)
    pupil_params = pynwb.core.DynamicTable.from_dataframe(df=pupil_fits, name="pupil_ellipse_fits")

    cr_fits = eye_dlc_tracking_data["cr_params"].assign(timestamps=synced_timestamps)
    cr_params = pynwb.core.DynamicTable.from_dataframe(df=cr_fits, name="cr_ellipse_fits")

    eye_fits = eye_dlc_tracking_data["eye_params"].assign(timestamps=synced_timestamps)
    eye_params = pynwb.core.DynamicTable.from_dataframe(df=eye_fits, name="eye_ellipse_fits")
...
def add_eye_gaze_data_interfaces(
    pynwb_container: pynwb.NWBContainer,
    ...
    pupil_area_ts = pynwb.base.TimeSeries(
        name="pupil_area", data=pupil_areas.values, timestamps=synced_timestamps.values, unit="Pixels ^ 2"
    )

    eye_area_ts = pynwb.base.TimeSeries(
        name="eye_area", data=eye_areas.values, timestamps=synced_timestamps.values, unit="Pixels ^ 2"
    )
...
def create_gaze_mapping_nwb_processing_modules(eye_gaze_data: dict):
    ...
    raw_gaze_mapping_mod = pynwb.ProcessingModule(
        name="raw_gaze_mapping", description="Gaze mapping processing module raw outputs"
    )
    ...
    filt_gaze_mapping_mod = pynwb.ProcessingModule(
        name="filtered_gaze_mapping", description="Gaze mapping processing module filtered outputs"
    )
...
def add_eye_tracking_ellipse_fit_data_to_nwbfile(
    nwbfile: pynwb.NWBFile, eye_dlc_tracking_data: dict, synced_timestamps: pd.Series
) -> pynwb.NWBFile:
    eye_tracking_mod = create_eye_tracking_nwb_processing_module(eye_dlc_tracking_data, synced_timestamps)
    nwbfile.add_processing_module(eye_tracking_mod)

    return nwbfile


def add_eye_gaze_mapping_data_to_nwbfile(nwbfile: pynwb.NWBFile, eye_gaze_data: dict) -> pynwb.NWBFile:
...
def add_running_acquisition_to_nwbfile(nwbfile, running_acquisition_df: pd.DataFrame):
...
def add_running_speed_to_nwbfile(nwbfile, running_speed, name="speed", unit="cm/s", from_dataframe=False):
    ...
    running_speed_series = pynwb.base.TimeSeries(name=name, data=data, timestamps=timestamps, unit=unit)
...
def create_stimulus_presentation_time_interval(
    name: str, description: str, columns_to_add: Iterable
    ...
    interval = pynwb.epoch.TimeIntervals(name=name, description=description)
...
def setup_table_for_invalid_times(invalid_epochs):
    ...
    if invalid_epochs:
        ...
        tags = [[_type, str(_id), label] for _type, _id, label in zip(df["type"], df["id"], df["label"])]

        table = pd.DataFrame({"start_time": start_time, "stop_time": stop_time, "tags": tags})
        table.index.name = "id"
...
def add_stimulus_timestamps(nwbfile, stimulus_timestamps, module_name="stimulus"):
    stimulus_ts = TimeSeries(data=stimulus_timestamps, name="timestamps", timestamps=stimulus_timestamps, unit="s")
    ...
    for c in trials.columns:
        ...
        if not len(data) == len(order):
            ...
            nwbfile.add_trial_column(
                name=c, description=description_dict.get(c, "NOT IMPLEMENTED: %s" % c), data=data, index=index
            )
        else:
...
def add_rewards(nwbfile, rewards_df):
    reward_volume_ts = TimeSeries(
        name="volume", data=rewards_df.volume.values, timestamps=rewards_df["timestamps"].values, unit="mL"
    )

    autorewarded_ts = TimeSeries(
        name="autorewarded", data=rewards_df.autorewarded.values, timestamps=reward_volume_ts.timestamps, unit="mL"
    )
...
def add_image(nwbfile, image_data, image_name, module_name, module_description, image_api=None):
    ...
    if isinstance(image_data, sitk.Image):
        ...
        raise ValueError(f"Not a supported image_data type: {type(image_data)}")
...
def add_metadata(nwbfile, metadata: dict, behavior_only: bool):
    ...
    subject_fields = {"age_in_days", "driver_line", "genotype", "subject_id", "reporter_line", "sex"}
    subject_metadata = {k: v for k, v in metadata_clean.items() if k in subject_fields}
    for subject_key in subject_metadata.keys():
        metadata_clean.pop(subject_key, None)

    BehaviorSubject = load_pynwb_extension(SubjectMetadataSchema, "ndx-aibs-behavior-ophys")
    ...
    if behavior_only:
        BehaviorMetadata = load_pynwb_extension(BehaviorMetadataSchema, "ndx-aibs-behavior-ophys")
        nwb_metadata = BehaviorMetadata(name="metadata", **new_metadata_dict)
    else:
        OphysBehaviorMetadata = load_pynwb_extension(OphysBehaviorMetadataSchema, "ndx-aibs-behavior-ophys")
...
def add_task_parameters(nwbfile, task_parameters):
    OphysBehaviorTaskParameters = load_pynwb_extension(BehaviorTaskParametersSchema, "ndx-aibs-behavior-ophys")
...
def add_cell_specimen_table(nwbfile: NWBFile, cell_specimen_table: pd.DataFrame, session_metadata: dict):
    ...
    """
    cell_specimen_metadata = NwbOphysMetadataSchema().load(session_metadata, unknown=marshmallow.EXCLUDE)
    ...
    if device_name.startswith("MESO"):
        device_config = {"name": device_name, "description": "Allen Brain Observatory - Mesoscope 2P Rig"}
    else:
        device_config = {
            "name": device_name,
    ...
    optical_channel = OpticalChannel(
        name="channel_1", description="2P Optical Channel", emission_lambda=cell_specimen_metadata["emission_lambda"]
    )
    ...
    plane_segmentation = image_segmentation.create_plane_segmentation(
        name="cell_specimen_table", description="Segmented rois", imaging_plane=imaging_plane
    )
    ...
    for col_name in cell_roi_table.columns:
        ...
        if col_name not in ["id", "mask_matrix", "roi_mask", "pixel_mask", "voxel_mask"]:
            ...
            plane_segmentation.add_column(
                col_name, CELL_SPECIMEN_COL_DESCRIPTIONS.get(col_name, "No Description Available")
            )
...
def add_dff_traces(nwbfile, dff_traces, ophys_timestamps):
    ...
    trace_data = np.array([dff_traces.loc[cell_roi_id].dff for cell_roi_id in dff_traces.index.values])

    cell_specimen_table = (
        nwbfile.processing["ophys"].data_interfaces["image_segmentation"].plane_segmentations["cell_specimen_table"]
    )  # noqa: E501
    roi_table_region = cell_specimen_table.create_roi_table_region(
        description="segmented cells labeled by cell_specimen_id", region=slice(len(dff_traces))
    )
...
def add_motion_correction(nwbfile, motion_correction):
    ...
    t1 = TimeSeries(
        name="ophys_motion_correction_x", data=motion_correction["x"].values, timestamps=ophys_timestamps, unit="pixels"
    )

    t2 = TimeSeries(
        name="ophys_motion_correction_y", data=motion_correction["y"].values, timestamps=ophys_timestamps, unit="pixels"
    )
