...
def append_well_known_file(wkfs, path, wkf_type_id=None, content_type=None):
    record = {"filename": os.path.basename(path), "storage_directory": os.path.dirname(path)}
...
def get_input_json(object_id, object_class, strategy_class, host="lims2", **kwargs):
    query_string = ("http://{}/InputJsons?strategy_class={}&object_class={}&object_id={}").format(
        host, strategy_class, object_class, object_id
    )
