...
class PostgresQueryMixin(object):
    ...
    def get_connection(self):
        return psycopg2.connect(
            dbname=self.dbname, user=self.user, host=self.host, password=self.password, port=self.port
        )
    ...
    def select(self, query):
        return psycopg2_select(
            query, database=self.dbname, host=self.host, port=self.port, username=self.user, password=self.password
        )
...
def db_connection_creator(
    ...
    if credentials:
        ...
        db_conn = credential_injector(fallback_credentials)(PostgresQueryMixin)()
    else:
        raise RuntimeError(
            "Must provide either credentials or fallback credentials in order to create a db connection!"
        )
