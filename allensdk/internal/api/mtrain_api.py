...
from allensdk.core.auth_config import MTRAIN_DB_CREDENTIAL_MAP, LIMS_DB_CREDENTIAL_MAP
...
class MtrainApi:
    ...
    def get_page(self, table_name, get_obj=None, filters=[], **kwargs):
        ...
        for ii in itertools.count(1):
            ...
            uri = "/".join(
                [self.api_base, 'api/v1/%s?page=%i&q={"filters":%s}' % (table_name, ii, json.dumps(filters))]
            )
    ...
    def get_session(self, behavior_session_uuid=None, behavior_session_id=None):
        assert not all(v is None for v in [behavior_session_uuid, behavior_session_id]), (
            "must enter either a behavior_session_uuid or a behavior_session_id"
        )
        if behavior_session_id is not None:
            ...
            def _get_behavior_metadata():
                ...
                behavior_session_id_ = BehaviorSessionId(behavior_session_id=behavior_session_id)
                bm = BehaviorMetadata.from_lims(behavior_session_id=behavior_session_id_, lims_db=lims_db)
                return bm
            ...
            if behavior_session_uuid is not None:
                ...
                assert behavior_session_uuid == str(bm.behavior_session_uuid), (
                    "behavior_session {} does not match behavior_session_id {}".format(
        ...
        regimen_df = (
            self.get_df("regimens")
            .rename(columns={"id": "regimen_id", "name": "regimen_name"})
        ...
        trials_df = (
            self.get_df("trials", filters=filters)
            .sort_values("index")
            .drop(["id", "behavior_session"], axis=1)
    ...
    def get_behavior_training_df(self, LabTracks_ID=None):
        ...
        stage_df = (
            self.get_df("stages").rename(columns={"id": "stage_id", "name": "stage_name"}).drop(["states"], axis=1)
        )
    ...
    def get_current_stage(self, LabTracks_ID):
        ...
        state_response = sess.get(
            os.path.join(self.api_base, "get_script/"), data=json.dumps({"LabTracks_ID": LabTracks_ID})
        )  #
        # .json()#['objects']).keys()
...
class MtrainSqlApi:
    def __init__(self, dbname=None, user=None, host=None, password=None, port=None):
        if any(map(lambda x: x is None, [dbname, user, host, password, port])):
            # Currying is equivalent to decorator syntactic sugar
            self.mtrain_db = credential_injector(MTRAIN_DB_CREDENTIAL_MAP)(PostgresQueryMixin)()
        else:
            self.mtrain_db = PostgresQueryMixin(dbname=dbname, user=user, host=host, password=password, port=port)
    ...
    def get_behavior_training_df(self, LabTracks_ID):
        ...
        dataframe = pd.read_sql(
            """SELECT stages.name as stage_name, regimens.name as
            regimen_name, bs.date, bs.id as behavior_session_id
