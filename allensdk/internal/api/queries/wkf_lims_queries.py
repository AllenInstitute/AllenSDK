...
def wkf_path_from_attachable(
    lims_connection: PostgresQueryMixin, wkf_type_name: List[str], attachable_type: str, attachable_id: int
) -> Dict[str, str]:
    """
    ...
    query += build_in_list_selector_query(col="wkft.name", valid_list=wkf_type_name, operator="WHERE", valid=True)
    ...
    for type_name, filepath in zip(query_result.type_name, query_result.filepath):
        if type_name in wkf_path_lookup:
            raise OneResultExpectedError(f"More than one result returned for {type_name}")
