...
class BiophysicalModuleReader(object):
    ...
    def fit_parameters_path(self):
        ...
        fit_parameter_path = os.path.join(file_entry["storage_directory"], file_entry["filename"])
    ...
    def update_well_known_file(self, path, well_known_file_type_id=None):
        if well_known_file_type_id is None:
            well_known_file_type_id = lims_utilities.NWB_UNCOMPRESSED_FILE_TYPE_ID
        well_known_files = self.lims_data["well_known_files"]
        (dirname, filename) = os.path.split(os.path.abspath(path))

        def get_nwb_file_id(f):
            if (
                "well_known_file_type_id" in f
                and f["well_known_file_type_id"] == well_known_file_type_id
                and os.path.normpath(f["storage_directory"]) == os.path.normpath(dirname)
        ...
        try:
            existing_file_id = next(wkf_id for wkf_id in (get_nwb_file_id(f2) for f2 in well_known_files) if wkf_id)

            # existing parameter file found
            self.lims_update_data["well_known_files"] = [f for f in well_known_files if not_nwb_file(f)]
    ...
    def to_manifest(self, manifest_path=None):
        ...
        nwb_file_name, extension = os.path.splitext(os.path.basename(self.stimulus_path()))
        b.add_path(
            "output_path",
            "%d_virtual_experiment%s" % (neuronal_model_run_id, extension),
            typename="file",
            parent_key="WORKDIR",
            format="NWB",
        )
        ...
        b.add_section(
            "biophys",
            {"biophys": [{"model_file": [manifest_path, self.fit_parameters_path()], "model_type": self.model_type()}]},
        )
