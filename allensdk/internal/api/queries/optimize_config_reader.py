...
class OptimizeConfigReader(object):
    ...
    def stimulus_path(self):
        ...
        stimulus_path = os.path.join(file_entry["storage_directory"], file_entry["filename"])
    ...
    def morphology_path(self):
        ...
        morphology_path = os.path.join(file_entry["storage_directory"], file_entry["filename"])
    ...
    def sweep_numbers(self):
        ...
        sweeps = [
            sweep_entry["sweep_number"]
            for sweep_entry in sweep_entries
            if sweep_entry["workflow_state"] == "auto_passed" or sweep_entry["workflow_state"] == "manual_passed"
        ]
    ...
    def mod_file_paths(self):
        ...
        for file_entry in file_entries:
            mod_path = os.path.join(file_entry["storage_directory"], file_entry["filename"])
            OptimizeConfigReader._log.info(mod_path)
            mod_file_paths.append(mod_path)

        return mod_file_paths

    def update_well_known_file(self, path, well_known_file_type_id=None):
        ...
        def get_model_parameter_file_id(f):
            if "well_known_file_type_id" in f and f["well_known_file_type_id"] == well_known_file_type_id:
                return f["id"]
            else:
                return None
        ...
        try:
            existing_file_id = next(
                wkf_id for wkf_id in (get_model_parameter_file_id(f2) for f2 in well_known_files) if wkf_id
            )
    ...
    def build_manifest(self, manifest_path=None):
        ...
        for modfile in self.mod_file_entries():
            b.add_path(
                "MOD_FILE_%s" % (os.path.splitext(modfile["filename"])[0]),
                os.path.join(modfile["storage_directory"], modfile["filename"]),
                typename="file",
                format="MODFILE",
            )

        b.add_path("stimulus_path", self.stimulus_path(), typename="file", format="NWB")

        b.add_path("manifest", os.path.join(os.path.realpath(os.curdir), manifest_path), typename="file")

        b.add_path(
            "output", os.path.basename(self.stimulus_path()), typename="file", parent_key="WORKDIR", format="NWB"
        )

        b.add_path("neuronal_model_data", self.lims_path, typename="file")

        b.add_path("upfile", "upbase.dat", typename="file", parent_key="WORKDIR")
        b.add_path("downfile", "downbase.dat", typename="file", parent_key="WORKDIR")
        b.add_path("passive_fit_data", "passive_fit_data.json", typename="file", parent_key="WORKDIR")
        b.add_path("stage_1_jobs", "stage_1_jobs.json", typename="file", parent_key="WORKDIR")
        b.add_path("fit_1_file", "fit_1_data.json", typename="file", parent_key="WORKDIR")
        b.add_path("fit_2_file", "fit_2_data.json", typename="file", parent_key="WORKDIR")
        b.add_path("fit_3_file", "fit_3_data.json", typename="file", parent_key="WORKDIR")
        b.add_path("fit_type_path", typename="file", spec="%s", parent_key="WORKDIR")
        b.add_path("target_path", typename="file", spec="target.json", parent_key="WORKDIR")
        b.add_path("fit_config_json", typename="file", spec="%s/config.json", parent_key="WORKDIR")
        b.add_path("final_hof_fit", typename="file", spec="%s/s%d/final_hof_fit.txt", parent_key="WORKDIR")
        b.add_path("final_hof", typename="file", spec="%s/s%d/final_hof.txt", parent_key="WORKDIR")
        b.add_path("output_fit_file", typename="file", spec="fit_%s_%s.json")

        b.add_section("biophys", {"biophys": [{"model_file": [manifest_path]}]})

        b.add_section(
            "stimulus_conf", {"runs": [{"sweeps": self.sweep_numbers(), "specimen_id": self.lims_data["specimen_id"]}]}
        )

        b.add_section("hoc_conf", {"neuron": [{"hoc": ["stdgui.hoc", "import3d.hoc", "cell.hoc"]}]})
