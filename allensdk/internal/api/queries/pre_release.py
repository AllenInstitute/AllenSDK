...
class BrainObservatoryApiPreRelease(BrainObservatoryApi):
    ...
    def get_experiment_containers(self):
        ...
        for q in query_result:
            ...
            if q["donor_tags"] == "Epileptiform Events":
                c["specimen"]["donor"]["conditions"] = [collections.defaultdict(collections.defaultdict)]
                c["specimen"]["donor"]["conditions"][0]["name"] = "Epileptiform Events"
            elif q["donor_tags"] == "":
                pass
            else:
                raise
        ...
        for q in query_result:
            ...
            for suffix in ["a", "b", "c"]:
                if q["crar%s_data" % suffix] is not None:
                    c["reliability_nm1_%s" % suffix] = q["crar%s_data" % suffix]["roi_cell_metrics"].get(
