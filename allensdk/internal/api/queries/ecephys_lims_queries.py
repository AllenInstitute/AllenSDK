...
def donor_id_lookup_from_ecephys_session_ids(
    lims_connection: PostgresQueryMixin, session_id_list: List[int]
) -> pd.DataFrame:
    """
    ...
    {build_in_list_selector_query(col="ecephys_sessions.id", valid_list=session_id_list)}
    """
...
def donor_id_list_from_ecephys_session_ids(
    ...
    """
    lookup = donor_id_lookup_from_ecephys_session_ids(lims_connection=lims_connection, session_id_list=session_id_list)
