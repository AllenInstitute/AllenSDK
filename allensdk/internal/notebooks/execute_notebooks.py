...
parser.add_argument("--notebooks_dir", required=True, help="Path to notebooks to execute")
parser.add_argument("--skip_notebooks", nargs="+", help="List of notebook names to skip", default=[])
...
NOTEBOOK_ARGS = {
    "brain_observatory_analysis.ipynb": {"RUN_LOCALLY_SPARSE_NOISE": False},
    "ecephys_data_access.ipynb": {"DOWNLOAD_COMPLETE_DATASET": False},
    "visual_behavior_neuropixels_data_access.ipynb": {"DOWNLOAD_COMPLETE_DATASET": False},
    "visual_behavior_ophys_data_access.ipynb": {"DOWNLOAD_COMPLETE_DATASET": False},
    "ecephys_session.ipynb": {"DOWNLOAD_LFP": False},
}
...
class NotebookRunner:
    ...
    def __init__(self, notebooks_dir: str):
        ...
        notebooks_dir
            Path to notebooks
        """
        notebook_paths = [Path(x) for x in glob.glob(os.path.join(notebooks_dir, "*.ipynb"))]
        self._notebook_paths = [x for x in notebook_paths if x.name not in args.skip_notebooks]
            ...
            with tempfile.TemporaryDirectory() as tmp_dir:
                ...
                try:
                    papermill.execute_notebook(
                        ...
                        parameters={
                            **{"output_dir": tmp_dir, "resources_dir": str(Path(__file__).parent / "resources")},
                            **NOTEBOOK_ARGS.get(notebook_path.name, {}),
                        },
                        kernel_name="python3",
                    )
                    self._remove_injected_parameters_cell(notebook_path=tmp_nb_path)

                    logging.info(
                        "Executing notebook succeeded. "
    ...
    def _remove_injected_parameters_cell(notebook_path):
        ...
        output = NotebookExporter(config=c).from_filename(notebook_path)
