...
def plot_single_ap_values(nwb_file, sweep_numbers, rheo_features, sweep_features, cell_features, type_name):
    ...
    yvals = [
        float(rheo_features[k + "_v_" + type_name])
        for k in gen_features
        if rheo_features[k + "_v_" + type_name] is not None
    ]
    ...
    plt.figure(figs[1].number)
    yvals = [
        float(rheo_features[k + "_t_" + type_name])
        for k in gen_features
        if rheo_features[k + "_t_" + type_name] is not None
    ]
    xvals = range(len(yvals))
    plt.scatter(xvals, yvals, color="blue", marker="_", s=40, zorder=100)
    plt.xticks(xvals, ["thr", "pk", "tr", "ftr", "str"])
    ...
    if rheo_features["upstroke_downstroke_ratio_" + type_name] is not None:
        plt.scatter(
            [0],
            [float(rheo_features["upstroke_downstroke_ratio_" + type_name])],
            color="blue",
            marker="_",
            s=40,
            zorder=100,
    ...
    for index, sn in enumerate(sweep_numbers):
        ...
        if nspikes:
            plt.plot(
                ...
                [spikes[0]["upstroke_v"] - delta_v, spikes[0]["upstroke_v"] + delta_v],
                color="red",
            )
            ...
            plt.plot(
                ...
                [spikes[0]["downstroke_v"] - delta_v, spikes[0]["downstroke_v"] + delta_v],
                color="red",
            )
...
def plot_subthreshold_long_square_figures(
    nwb_file, cell_features, rheo_features, sweep_features, image_dir, sizes, cell_image_files
):
    ...
    plt.plot(
        [i.min(), i.max()],
        [cell_features["long_squares"]["tau"], cell_features["long_squares"]["tau"]],
        color="red",
        linewidth=2,
    )
    plt.xlabel("pA")
...
def plot_short_square_figures(
    ...
    if repeat_amp is not None:
        ...
        figs = plot_single_ap_values(
            nwb_file, short_square_sweep_nums, rheo_features, sweep_features, cell_features, "short_square"
        )

        for index, fig in enumerate(figs):
            save_figure(fig, "short_squares_%d" % index, "short_squares", image_dir, sizes, cell_image_files)

        fig = plot_instantaneous_threshold_thumbnail(
            nwb_file, short_square_sweep_nums, cell_features, rheo_features, sweep_features
        )
...
def plot_instantaneous_threshold_thumbnail(
    nwb_file, sweep_numbers, cell_features, rheo_features, sweep_features, color="red"
):
...
def plot_ramp_figures(
    nwb_file, sweep_info, cell_features, rheo_features, sweep_features, image_dir, sizes, cell_image_files
):
    sweeps = sweep_info
    ramps_sweeps = [
        s["sweep_number"]
        for s in sweeps
        if s["workflow_state"].endswith("passed") and s["ephys_stimulus"]["description"][:10] == "C1RP25PR1S"
    ]
...
def plot_long_square_summary(nwb_file, cell_features, rheo_features, sweep_features):
    ...
    thumbnail_summary_fig = plot_sweep_set_summary(
        nwb_file, int(rheo_features["thumbnail_sweep_num"]), long_square_sweep_numbers
    )
...
def plot_fi_curve_figures(nwb_file, cell_features, rheo_features, sweep_features, image_dir, sizes, cell_image_files):
    ...
    plt.plot(
        x[last_zero_idx:],
        cell_features["long_squares"]["fi_fit_slope"] * (np.array(x[last_zero_idx:]) - x[last_zero_idx]),
        color="red",
    )
    plt.xlabel("pA")
...
def plot_sweep_value_figures(sweep_info, image_dir, sizes, cell_image_files):
    ...
    data = np.array(
        [
            [s["pre_vm_mv"], s["sweep_number"]]
            for s in sweeps
            if not s["ephys_stimulus"]["description"].startswith("EXTPBLWOUT")
        ]
    ).T
    mask_nulls(data)
...
def plot_cell_figures(nwb_file, cell_features, sweep_features, rheo_features, image_dir, sweep_info, sizes):
    cell_image_files = {}
...
def plot_sweep_set_summary(
    nwb_file, highlight_sweep_number, sweep_numbers, highlight_color="#0779BE", background_color="#dddddd"
):
    fig = plt.figure(frameon=False)
    ax = plt.Axes(fig, [0.0, 0.0, 1.0, 1.0])
    ax.set_axis_off()
...
def make_sweep_html(sweep_files, file_name):
    ...
    if "test_pulses" in sweep_files:
        for small_img, large_img in zip(sweep_files["test_pulses"]["small"], sweep_files["test_pulses"]["large"]):
            html += "<a href='%s' target='_blank'><img src='%s'></img></a>" % (
                os.path.basename(large_img),
    ...
    if "experiments" in sweep_files:
        for small_img, large_img in zip(sweep_files["experiments"]["small"], sweep_files["experiments"]["large"]):
            html += "<a href='%s' target='_blank'><img src='%s'></img></a>" % (
                os.path.basename(large_img),
                os.path.basename(small_img),
            )
...
def make_sweep_page(nwb_file, working_dir, sweep_data):
    ...
    sweep_files = plot_sweep_figures(nwb_file=nwb_file, sweep_data=sweep_data, image_dir=working_dir, sizes=sizes)

    make_sweep_html(sweep_files, os.path.join(working_dir, "sweep.html"))
...
def make_cell_page(
    ...
    make_cell_html(
        cell_files, os.path.join(working_dir, "index.html"), relative_sweep_link, specimen_info, fields_to_show
    )
