...
def filter_sweeps(sweeps, types=None, passed_only=True, iclamp_only=True):
    ...
    if types:
        sweeps = [s for s in sweeps for t in types if s["ephys_stimulus"]["description"].startswith(t)]
...
def find_coarse_long_square_amp_delta(sweeps, decimals=0):
    ...
    if len(deltas) != 1:
        logging.warning(
            "Found multiple coarse long square amplitude step differences: %s.  Using: %f" % (str(deltas), delta)
        )
...
def extract_data(data, nwb_file):
    ...
    for s in iclamp_sweep_list:
        try:
            ...
            raise Exception(
                ...
                % (s["sweep_number"], json.dumps(s, indent=3, default=json_handler))
            )
    ...
    logging.info("%d of %d sweeps passed QC", num_passed_sweeps, len(iclamp_sweep_list))
