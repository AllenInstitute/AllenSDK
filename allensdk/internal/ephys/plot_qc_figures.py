...
def plot_single_ap_values(nwb_file, sweep_numbers, lims_features, sweep_features, cell_features, type_name):
    ...
    yvals = [
        float(lims_features[k + "_v_" + type_name])
    ...
    plt.figure(figs[1].number)
    yvals = [
        float(lims_features[k + "_t_" + type_name])
        for k in gen_features
...
def plot_sweep_figures(nwb_file, ephys_roi_result, image_dir, sizes):
    ...
    vclamp_sweep_numbers = sorted(
        [s["sweep_number"] for s in sweeps if s["stimulus_units"] == "Amps" or s["stimulus_units"] == "pA"]
    )
...
def plot_subthreshold_long_square_figures(
    nwb_file, cell_features, lims_features, sweep_features, image_dir, sizes, cell_image_files
):
...
def plot_short_square_figures(
    ...
    if repeat_amp is not None:
        ...
        figs = plot_single_ap_values(
            nwb_file, short_square_sweep_nums, lims_features, sweep_features, cell_features, "short_square"
        )
        ...
        fig = plot_instantaneous_threshold_thumbnail(
            nwb_file, short_square_sweep_nums, cell_features, lims_features, sweep_features
        )
...
def plot_instantaneous_threshold_thumbnail(
    nwb_file, sweep_numbers, cell_features, lims_features, sweep_features, color="red"
):
...
def plot_ramp_figures(
    nwb_file, cell_specimen, cell_features, lims_features, sweep_features, image_dir, sizes, cell_image_files
):
...
def plot_long_square_summary(nwb_file, cell_features, lims_features, sweep_features):
    ...
    thumbnail_summary_fig = plot_sweep_set_summary(
        nwb_file, int(lims_features["thumbnail_sweep_num"]), long_square_sweep_numbers
    )
...
def make_cell_page(nwb_file, ephys_roi_result, working_dir, save_cell_plots=True):
    ...
    make_cell_html(cell_files, ephys_roi_result, os.path.join(working_dir, "index.html"), relative_sweep_link)
