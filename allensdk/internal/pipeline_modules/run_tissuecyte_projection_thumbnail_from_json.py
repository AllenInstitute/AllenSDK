...
def main():
    ...
    colormap = pd.read_csv(
        input_data["colormap_path"], header=None, names=["red", "green", "blue"], delim_whitespace=True
    )
    ...
    for rot in input_data["rotations"]:
        rot["write_depth_sheet"] = functools.partial(
            write_depth_image, path=str(os.path.join(output_dir, rot["depth_path"]))
        )
    ...
    run(volume, input_data["min_threshold"], input_data["max_threshold"], input_data["rotations"], colormap)
