...
LARGE_FONT = 12
PLOT_CONFIGS = {
    "small": dict(height_px=SMALL_HEIGHT, pattern="%s_small.png", font_size=SMALL_FONT),
...
PLOT_TYPES = ["dg", "sg", "ns", "lsn_on", "lsn_off", "rf", "nm1", "nm2", "nm3", "sp", "corr", "eye"]
...
def debug(experiment_id, plots=None, local=False):
    ...
    run_module(
        ...
        sdk_path=SDK_PATH,
        pbs=dict(vmem=32, job_name="bobthumbs_%d" % experiment_id, walltime="10:00:00"),
        local=local,
...
def build_receptive_field(lsna, configs, output_dir):
    lsn_movie, lsn_mask = lsna.data_set.get_locally_sparse_noise_stimulus_template(lsna.stimulus, mask_off_screen=False)
    ...
    csids = lsna.data_set.get_cell_specimen_ids()
    for file_name, csid, i in build_cell_plots(
        csids, "receptive_field_on", 1.754, [configs["large"]], output_dir, transparent=True
    ):
        lsna.plot_cell_receptive_field(
            True, cell_specimen_id=csid, clim=clim, mask=lsn_mask, cell_index=i, scalebar=True
        )
        oplots.finalize_no_axes()

    for file_name, csid, i in build_cell_plots(
        csids, "receptive_field_on", 1.754, [configs["small"]], output_dir, transparent=True
    ):
        lsna.plot_cell_receptive_field(
            True, cell_specimen_id=csid, clim=clim, mask=lsn_mask, cell_index=i, scalebar=False
        )
        oplots.finalize_no_axes()

    for file_name, csid, i in build_cell_plots(
        csids, "receptive_field_off", 1.754, [configs["large"]], output_dir, transparent=True
    ):
        lsna.plot_cell_receptive_field(
            False, cell_specimen_id=csid, clim=clim, mask=lsn_mask, cell_index=i, scalebar=True
        )
        oplots.finalize_no_axes()

    for file_name, csid, i in build_cell_plots(
        csids, "receptive_field_off", 1.754, [configs["small"]], output_dir, transparent=True
    ):
        lsna.plot_cell_receptive_field(
            False, cell_specimen_id=csid, clim=clim, mask=lsn_mask, cell_index=i, scalebar=False
        )
        oplots.finalize_no_axes()
...
def build_correlation_plots(data_set, analysis_file, configs, output_dir):
        ...
        if hasattr(dg, "representational_similarity"):
            ...
            for cfgs, show_labels in setups:
                for fn in build_plots("drifting_gratings_representational_similarity", 1.0, cfgs, output_dir):
                    oplots.plot_representational_similarity(
                        dg.representational_similarity,
        ...
        if hasattr(sg, "representational_similarity"):
            ...
            for cfgs, show_labels in setups:
                for fn in build_plots("static_gratings_representational_similarity", 1.0, cfgs, output_dir):
                    oplots.plot_representational_similarity(
                        sg.representational_similarity,
    ...
    if len(ans):
        ...
        for fn, csid, i in build_cell_plots(csids, "signal_correlation", 1.0, [configs["large"]], output_dir):
            ...
            oplots.plot_cell_correlation([np.delete(sig_corr[row], i) for sig_corr in sig_corrs], labels, colors)
            oplots.finalize_with_axes()
...
def lsna_check_hvas(data_set, data_file):
    ...
    if stim is None:
        raise MissingStimulusException(
            "Could not find appropriate LSN stimulus for session %s", data_set.get_session_type()
        )
    else:
...
def build_eye_tracking_plots(data_set, configs, output_dir):
    try:
        ...
        if len(xy_deg) == 0:
            logging.debug("Eye tracking had no finite data, should have been failed")
            return
        elif len(xy_deg) < 3:
        ...
        for fn in build_plots("eye_tracking_gaze_axes", 1.0, [configs["large"], configs["svg"]], output_dir):
            oplots.plot_pupil_location(xy_deg, c=c, include_labels=True)
            oplots.finalize_with_axes()

        for fn in build_plots("eye_tracking_gaze", 1.0, [configs["small"]], output_dir):
...
def build_experiment_thumbnails(nwb_file, analysis_file, output_directory, types=None, threads=4):
    if types is None:
        types = PLOT_TYPES
...
def main():
    ...
    build_experiment_thumbnails(nwb_file, analysis_file, output_directory, args.types, args.threads)
