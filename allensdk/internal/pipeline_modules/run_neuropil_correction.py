...
def debug(experiment_id, local=False):
    ...
    run_module(
        ...
        exp_dir,
        sdk_path=SDK_PATH,
        pbs=dict(vmem=160, job_name="np_%d" % experiment_id, walltime="36:00:00"),
        local=local,
...
def main():
    ...
    for n in range(num_traces):
        ...
        debug_plot(
            os.path.join(plot_dir, "initial_%04d.png" % n), roi, neuropil, fc, r, results["r_vals"], results["err_vals"]
        )
    ...
    for n in range(num_traces):
        ...
        # save a debug plot
        debug_plot(os.path.join(plot_dir, "final_%04d.png" % n), roi, neuropil, corrected[n, :], r_list[n])
