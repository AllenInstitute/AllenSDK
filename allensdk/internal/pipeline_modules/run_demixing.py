...
def debug(experiment_id, local=False):
    ...
    sd = lu.query("select storage_directory from ophys_experiments where id = %d" % experiment_id)[0][
        "storage_directory"
    ]
...
"""
    ...
    run_module(
        ...
        exp_dir,
        sdk_path=SDK_PATH,
        pbs=dict(vmem=160, job_name="demix_%d" % experiment_id, walltime="36:00:00"),
        local=local,
...
def main():
    ...
    nt_inds = demixer.plot_negative_transients(
        demix_traces, demixed_traces, valid[valid_idxs], demix_masks, trace_ids[valid_idxs], plot_dir
    )

    logging.debug("rois with negative transients: %s", str(trace_ids[valid_idxs][nt_inds]))

    nb_inds = demixer.plot_negative_baselines(
        demix_traces, demixed_traces, demix_masks, trace_ids[valid_idxs], plot_dir
    )
