...
def debug(container_id, local=False, plots=None):
    ...
    for exp in get_container_info(container_id):
        ...
        input_data.append(
            dict(input_json=exp_input_json, output_json=os.path.join(exp_data["output_directory"], "output.json"))
        )
    ...
    run_module(
        ...
        container_dir,
        sdk_path=SDK_PATH,
        pbs=dict(vmem=32, job_name="cthumbs_%d" % container_id, walltime="10:00:00"),
        local=local,
...
def main():
    ...
    for input_file in data:
        ...
        robsth.build_experiment_thumbnails(nwb_file, analysis_file, output_directory, types, mod.args.threads)
