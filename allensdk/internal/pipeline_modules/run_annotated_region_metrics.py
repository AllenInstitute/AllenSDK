...
SCRIPT_PATH = (
    "/data/informatics/CAM/isi_metrics/allensdk/allensdk/internal/pipeline_modules/run_annotated_region_metrics.py"
)


def debug(region_id, storage_directory="./", local=True, sdk_path=SDK_PATH, script_path=SCRIPT_PATH, lims_host="lims2"):
    strategy_class = "AnnotatedRegionMetricsStrategy"
    object_class = "AnnotatedRegion"
    input_json = get_input_json(region_id, object_class, strategy_class, lims_host)
    ...
    run_module(
        ...
        exp_dir,
        sdk_path=sdk_path,
        pbs=dict(vmem=4, job_name="isi_metrics_{}".format(region_id), walltime="1:00:00"),
        local=local,
    )
