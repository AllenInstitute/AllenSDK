...
def debug(experiment_id, num_frames=None, threshold_factor=None, local=False):
    ...
    run_module(
        ...
        experiment_dir,
        sdk_path=SDK_PATH,
        pbs=dict(vmem=160, job_name="itrack_%d" % experiment_id, walltime="10:00:00"),
        local=local,
...
def main():
    ...
    output_data = dict(
        pupil_file=tracker.pupil_file, corneal_reflection_file=tracker.cr_file, mean_frame_file=tracker.mean_frame_file
    )
