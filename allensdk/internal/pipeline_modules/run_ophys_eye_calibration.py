...
def debug(experiment_id, local=False):
    ...
    SCRIPT = (
        "/data/informatics/CAM/eye_calibration/allensdk/allensdk/internal/pipeline_modules/run_ophys_eye_calibration.py"
    )
...
""".format(experiment_id)
    ...
    input_data = {
        ...
        "output_file": os.path.join(exp_dir, "eye_tracking_to_screen_mapping.h5"),
        "monitor_position_x_mm": exp_info.get("screen_center_x_mm", eye_calibration.MONITOR_POSITION_OLD[0] * 10),
        "monitor_position_y_mm": exp_info.get("screen_center_y_mm", eye_calibration.MONITOR_POSITION_OLD[1] * 10),
        "monitor_position_z_mm": exp_info.get("screen_center_z_mm", eye_calibration.MONITOR_POSITION_OLD[2] * 10),
        ...
        "camera_position_x_mm": exp_info.get("camera_center_x_mm", eye_calibration.CAMERA_POSITION_OLD[0] * 10),
        "camera_position_y_mm": exp_info.get("camera_center_y_mm", eye_calibration.CAMERA_POSITION_OLD[1] * 10),
        "camera_position_z_mm": exp_info.get("camera_center_z_mm", eye_calibration.CAMERA_POSITION_OLD[2] * 10),
        "camera_rotation_x_deg": exp_info.get(
            "camera_rotation_x_deg", eye_calibration.CAMERA_ROTATIONS_OLD[0] * 180 / np.pi
        ),
        "camera_rotation_y_deg": exp_info.get(
            "camera_rotation_y_deg", eye_calibration.CAMERA_ROTATIONS_OLD[1] * 180 / np.pi
        ),
        "camera_rotation_z_deg": exp_info.get(
            "camera_rotation_z_deg", eye_calibration.CAMERA_ROTATIONS_OLD[2] * 180 / np.pi
        ),
        "led_position_x_mm": exp_info.get("led_center_x_mm", eye_calibration.LED_POSITION_ORIGINAL[0] * 10),
        "led_position_y_mm": exp_info.get("led_center_y_mm", eye_calibration.LED_POSITION_ORIGINAL[1] * 10),
        "led_position_z_mm": exp_info.get("led_center_z_mm", eye_calibration.LED_POSITION_ORIGINAL[2] * 10),
    }

    # TEMPORARY HACKS TO DEAL WITH BAD DATA IN LIMS
    # TODO: REMOVE WHEN DATAFIXES DONE
    if input_data["monitor_position_x_mm"] == -86.2:
        input_data["monitor_position_x_mm"] = eye_calibration.MONITOR_POSITION_NEW[0] * 10
        input_data["monitor_position_y_mm"] = eye_calibration.MONITOR_POSITION_NEW[1] * 10
        input_data["monitor_position_z_mm"] = eye_calibration.MONITOR_POSITION_NEW[2] * 10
...
def write_output(filename, position_degrees, position_cm, areas):
    with h5py.File(filename, "w") as f:
        ...
        f.create_dataset("screen_coordinates_spherical", data=position_degrees)
...
def main():
    ...
    pupil_on_monitor_deg = calibrator.pupil_position_on_monitor_in_degrees(pupil_params, cr_params)
    pupil_on_monitor_cm = calibrator.pupil_position_on_monitor_in_cm(pupil_params, cr_params)
    ...
    write_output(outfile, pupil_on_monitor_deg, pupil_on_monitor_cm, pupil_areas)
