...
def debug(
    ...
    for eid in experiment_ids:
        ...
        input_data[eid] = dict(
            nwb_file=get_experiment_nwb_file(eid),
    ...
    run_module(
        ...
        python=python,
        sdk_path=SDK_PATH,
        pbs=dict(vmem=32, job_name="bobanalysis_%d" % eid, walltime=walltime, queue=queue),
        local=local,
    )
...
def main():
    ...
    for ident, experiment in jin.items():
        ...
        results[ident] = run_session_analysis(nwb_file, output_file, save_flag=True, plot_flag=False)
