...
def construct_affine(theta):
    tr_rot = [np.cos(theta), np.sin(theta), 0, -np.sin(theta), np.cos(theta), 0, 0, 0, 1, 0, 0, 0]
    return tr_rot
...
def project_to_polyline(boundary, soma):
    ...
    points = zip(x, y)
    dists_projs = [dist_proj_point_lineseg(soma, np.array(q1), np.array(q2)) for q1, q2 in zip(points[:-1], points[1:])]
