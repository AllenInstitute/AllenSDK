...
def create_transcripts_for_genes(analysis_run_gene_file):
    ...
    transcripts_for_genes = np.genfromtxt(
        analysis_run_gene_file["analysis_run_gene_path"], usecols=[0, 1], skip_header=1, dtype="str"
    ).tolist()
...
def create_genes_for_transcripts(analysis_run_transcript_file):
    ...
    genes_for_transcripts = np.genfromtxt(
        analysis_run_transcript_file["analysis_run_transcript_path"], usecols=[0, 1], skip_header=1, dtype="str"
    ).tolist()
...
def create_gene_fpkm_table(analysis_run_records):
    ...
    for record in analysis_run_records:
        gene_fpkm.append(np.genfromtxt(record["analysis_run_gene_path"], usecols=[-1], skip_header=1, dtype="str"))
        rna_well_ids.append(record["rna_well_id"])

    entrez_ids = np.genfromtxt(
        analysis_run_records[0]["analysis_run_gene_path"], usecols=[0], skip_header=1, dtype="str"
    ).tolist()
...
def create_transcript_fpkm_table(analysis_run_records):
    ...
    for record in analysis_run_records:
        transcript_fpkm.append(
            np.genfromtxt(record["analysis_run_transcript_path"], usecols=[-1], skip_header=1, dtype="str")
        )
        rna_well_ids.append(record["rna_well_id"])

    transcript_ids = np.genfromtxt(
        analysis_run_records[0]["analysis_run_transcript_path"], usecols=[0], skip_header=1, dtype="str"
    ).tolist()
...
def main():
    ...
    create_transcripts_for_genes(analysis_run_records["analysis_run_records"][0]).to_csv(
        transcripts_for_genes_output, index=False, header=False
    )
    create_genes_for_transcripts(analysis_run_records["analysis_run_records"][0]).to_csv(
        genes_for_transcripts_output, index=False, header=False
    )
