...
def create_fake_metadata(
    ...
    width=DEBUG_WIDTH,
    height=DEBUG_HEIGHT,
    itemsize=DEBUG_ITEMSIZE,
    n_planes=DEBUG_N_PLANES,
):
    metadata = []
    ...
    for plane in range(n_planes):
        ...
        for i, channel in enumerate(channels):
            byte_offset = width * height * itemsize * (plane * len(channels) + i)
            strides = [width * height * itemsize * n_planes * len(channels), width * itemsize, itemsize]
            frame_meta.append(
                {
        ...
        metadata.append({"output_file": outfile, "experiment_id": experiment_id, "frame_metadata": frame_meta})
    return metadata
...
def debug(experiment_id, local=False, raw_path=None):
    ...
    SCRIPT = (
        "/data/informatics/CAM/ophys_decomp/allensdk/allensdk/"
    ...
    if raw_path is not None:
        ...
        input_data = {"raw_filename": raw_path, "frame_metadata": conversion_definitions}
    else:
    ...
    run_module(
        ...
        exp_dir,
        sdk_path=SDK_PATH,
        pbs=dict(vmem=160, job_name="ophys_decomp_%d" % experiment_id, walltime="36:00:00"),
        local=local,
    )
...
def convert_frame(conversion_definition):
    ...
    osd.export_frame_to_hdf5(raw_filename, ophys_hdf5_filename, auxiliary_hdf5_filename, frame_metadata)
...
def main():
    ...
    for eid, ophys_file, auxiliary_file in output:
        output_data[eid] = {"ophys_data": ophys_file, "auxiliary_data": auxiliary_file}
