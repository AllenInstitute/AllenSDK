...
def calc_spike_cut_and_v_reset_via_expvar_residuals(
    all_current_list,
    all_voltage_list,
    dt,
    El_reference,
    ...
    if max_spike_cut_time and max_spike_cut_time < 0.010:
        expVarIndRangeAfterSpike = range(
            int(0.001 / dt), int(max_spike_cut_time / dt)
        )  # NOTE: THIS IS USED IN REFERENCE TO SPIKE TIME

    else:
        expVarIndRangeAfterSpike = range(
            int(0.001 / dt), int(0.010 / dt)
        )  # NOTE: THIS IS USED IN REFERENCE TO SPIKE TIME
    ...
    for jj, vectorOfIndAcrossWaves in enumerate(
        list_of_endPointArrays
    ):  # these indices should be in terms of the spike waveforms
        #        print('jj', jj)
        # TODO: Teeter get rid of the nonblipness
        v_at_specificEndPoint = [
            all_v_spike_shape_list[ii][index] for ii, index in enumerate(vectorOfIndAcrossWaves)
        ]  # this is calculating variance at certain time points
    ...
    all_v_at_min_expVar_list = [
        v[list_of_endPointArrays[vectorIndex_of_max_explained_var][ii]] for ii, v in enumerate(all_v_spike_shape_list)
    ]
    ...
    if MAKE_PLOT:
        ...
        if PUBLICATION_PLOT:
            ...
            for ii in range(0, len(all_v_spike_shape_list)):
                ...
                # plt.plot(truncatedTime[aligned_peakInd[ii]],spikewave[aligned_peakInd[ii]], '.k'
                plt.plot(
                    truncatedTime[all_thresholdInd[ii]] * 1000,
                    temp_v_spike_shape_list[ii][all_thresholdInd[ii]] * 1e3,
                    ".k",
                    ms=10,
                )
                ...
                #                plt.plot(truncatedTime, all_v_spike_shape_list[ii])
                plt.plot(
                    time_at_minExpVar * 1000,
                    (np.array(all_v_at_min_expVar_list) + El_reference + deltaV) * 1.0e3,
                    ".k",
                    ms=10,
                )
    ...
    if MAKE_PLOT:
        plt.figure(figsize=(20, 10))
        plt.plot(
            all_v_spike_init_list, all_v_at_min_expVar_list, "b.", ms=16, label="noise"
        )  # list of voltage traces for blip
    ...
    (
        ...
        r_value_at_min_expVar_list,
        p_value_at_min_expVar_list,
        std_err_at_min_expVar_list,
    ...
    spike_cut_length = (list_of_endPointArrays[vectorIndex_of_max_explained_var][0]) - int(
        ALIGN_CUT_WINDOW[0] / dt
    )  # note this is dangerous if they arent' all at the same ind
    ...
    if MAKE_PLOT:
        xlim = np.array([min(all_v_spike_init_list), max(all_v_spike_init_list)])
        plotLineRegressRed(
            slope_at_each_time_end[vectorIndex_of_max_explained_var],
            intercept_at_each_time_end[vectorIndex_of_max_explained_var],
            np.nan,
            xlim,
        )
        plt.legend(loc=2, fontsize=20)
    ...
    if PUBLICATION_PLOT:
        ...
        plt.plot(
            np.array(all_v_spike_init_list) * 1e3, np.array(all_v_at_min_expVar_list) * 1e3, "b.", ms=16
        )  # list of voltage traces for blip
...
def plotLineRegress1(slope, intercept, r, xlim):
    ...
    plt.plot(
        xlim, y, "-k", lw=4, label="slope=" + "%.2f" % slope + ", intercept=" + "%.3f" % intercept + ", r=" + "%.2f" % r
    )
...
def plotLineRegressRed(slope, intercept, r, xlim):
    ...
    print("slope=", slope, "intercept=", intercept, "xlim", xlim)
    plt.plot(
        xlim, y, "-r", lw=4, label="slope=" + "%.2f" % slope + ", intercept=" + "%.3f" % intercept + ", r=" + "%.2f" % r
    )
