...
def ASGLM_pairwise(
    ...
    taus_int = [1000.0 / kk for kk in ks_int]  # converting to ms
    taus_filter = [f * j for j in taus_int]  # convert ms time to filter time units 1/(dt*ks)
    ks_list = [
        (1.0 / (i)) for i in taus_filter
    ]  # use peak positions of rcos bumps as time-scales for exponential bases
    ncos = 2  # no of bases!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    for ks_ind, (ks_fit_units, ks_SI_units) in enumerate(zip(ks_pairs, ks_pairs_in_SI_units)):
        ...
        for rr in range(len(I_stim)):  # loop over repeats
            ...
            basis_IPSP, gg0 = GLM.create_basis_IPSP(
        ...
        if MAKE_PLOT:
            plt.annotate(
                "ASGLM (fit asc and R): AScurrent basis",
                xy=(0.4, 0.985),
        ...
        llh_for_each_sweep = []
        for kkk, (v_spike_deleted, dv_spike_deleted, i_spike_deleted, b_ipsp_spikes_deleted) in enumerate(
            zip(v_all_swps_list, dv_all_swps_list, i_all_swps_list, b_ipsp_all_swps_list)
        ):
            ...
            if MAKE_PLOT:
                ...
                plt.subplot(5, 2, ks_ind + 1)
                plot_inds = np.arange(
                    int(t0_list[0][0]) - tst, int(t0_list[0][0]) - tst + nkt
                )  # plot just after first spike
                tvec = dt * plot_inds
        ...
        if MAKE_PLOT:
            ...
            plt.tight_layout()
            plt.annotate(
                "ASGLM (fit asc and R): Sum fit after spike currents",
                xy=(0.3, 0.985),
                xycoords="figure fraction",
                horizontalalignment="left",
                verticalalignment="top",
            ...
            plt.tight_layout()
            plt.annotate(
                "ASGLM (fit asc and R): Individual Currents",
                xy=(0.3, 0.985),
                xycoords="figure fraction",
                horizontalalignment="left",
                verticalalignment="top",
