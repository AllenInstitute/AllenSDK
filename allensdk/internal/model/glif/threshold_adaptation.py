...
def calc_spike_component_of_threshold_from_multiblip(
    multi_SS, dt, dv_cutoff, thresh_frac, MAKE_PLOT=False, SHOW_PLOT=False, BLOCK=False, PUBLICATION_PLOT=False
):
    ...
    for k in range(0, len(multi_SS_v)):
        ...
        if (
            thresh != [] and len(thresh) > 1
        ):  # there needs to be more than one spike so that we can find the time difference
            thresh_first_spike.append(
                thresh[0]
            )  # Note that this finds the first spike (it might not be at the first stimulus blip)
    ...
    try:  # this try here because sometimes even though have the traces there isnt more than one trace with two spikes
        ...
        p0_force = [0.002, -100.0]
        p0_fit = [0.002, -100.0, thresh_inf]

        # TODO: THIS WOULD BE BETTER IF IT CALLED THE ACTUAL FUNCTION IN THE NEURON METHODS THAT WAY THEY WOULD HAVE TO BE THE SAME
        (popt_force, pcov_force) = curve_fit(
            exp_force_c, (time_previous_spike, thresh_inf), threshold, p0=p0_force, maxfev=100000
        )
        ...
        if MAKE_PLOT:
            plt.plot(
                ...
                "r",
                lw=4,
                label="exp fit (force const to thesh first spike)\n  k=%.3g, amp=%.3g" % (popt_force[1], popt_force[0]),
            )
            plt.plot(
                time_previous_spike,
                fit_fit,
            ...
            if PUBLICATION_PLOT:
                ...
                for k in range(0, len(multi_SS_v)):
                    ...
                    ax1.plot(np.arange(0, len(multi_SS_i[k])) * dt, multi_SS_i[k] * 1.0e12, lw=2)
                    ax1.set_ylabel("Current (pA)", fontsize=16)
                    ax1.set_xlim([2.0, 2.12])
                    ax1.axes.xaxis.set_ticklabels([])
                    # ax1.set_title('Triple Short Square', fontsize=20)

                    ax2.plot(np.arange(0, len(multi_SS_v[k])) * dt, multi_SS_v[k] * 1.0e3, lw=2)
                    ax2.plot(spike_ind[k] * dt, np.array(thresh) * 1.0e3, ".k", ms=16)
                    ax2.set_ylabel("Voltage (mV)", fontsize=16)
                    ax2.set_xlabel("Time (ms)", fontsize=16)
                    ax2.set_xlim([2.0, 2.12])

                ax3.plot(time_previous_spike, np.array(threshold) * 1.0e3, ".k", ms=16)
                ...
                ax3.plot(
                    time_previous_spike, fit_force * 1.0e3, "r", lw=4
                )  # , label="exp fit: k=%.3g, amp=%.3g" % (popt_force[1], popt_force[0]))
                ax3.legend()
                plt.tight_layout()
                plt.show()
...
def fit_avoltage_bvoltage(
    x, v_trace_list, El_list, spike_cut_length, all_spikeInd_list, th_inf, dt, a_spike, b_spike, fake=False
):
    ...
    * current injection during the spike is not taken into account.  This seems reasonable as the
    ...
    for v_trace, El, all_spikeInd in zip(v_trace_list, El_list, all_spikeInd_list):
        ...
        for spike_number in range(
            1, len(all_spikeInd)
        ):  # skipping first spike since no residual spike component of threshold
            integration_length = (
                all_spikeInd[spike_number] - all_spikeInd[spike_number - 1] + 1
            )  # this is the amount of time that needs to be integrated over note that it is one longer than the interval because of the last value is added to the aspike for the next ISI
            local_spike_comp_of_threshold = spike_component_of_threshold_exact(
                a_spike + left_over, b_spike, np.arange(integration_length) * dt
            )
        ...
        if fake:
            v_comp_of_th_at_each_spike_via_data = (
                v_trace[all_spikeInd - 1] - internal_sp_comp_array[all_spikeInd - 1] - th_inf
        ...
        for spike_number in range(1, len(all_spikeInd)):  # loop over all ISI's in data
            v_start_ind = all_spikeInd[spike_number - 1] + int(
                spike_cut_length
            )  # dont want to use voltage during a spike
            ...
            theta0 = v_comp_of_th_at_each_spike_via_data[
                spike_number - 1
            ]  # this assumes that the voltage component of the threshold does not change over the time period of the spike
            ...
            model = +theta0 * np.exp(-b_voltage * dt * (end_ind - v_start_ind)) + a_voltage * np.exp(
                -b_voltage * tvec[-1]
            ) * np.sum(dt * (v_in_ISI - El) * np.exp(b_voltage * tvec))
            err = (theta1 - model) ** 2
            if ~np.isnan(err):
                sq_err.append(err)
...
def fit_avoltage_bvoltage_th(
    x, v_trace_list, El_list, spike_cut_length, all_spikeInd_list, dt, a_spike, b_spike, fake=False
):
...
def get_peaks(voltage, aboveValue=0):
    ...
    peakInd = np.intersect1d(
        np.intersect1d(IndShiftL[0], IndShiftR[0]), greatThanThresh[0]
    )  # find the indicies of the peak
    return peakInd
