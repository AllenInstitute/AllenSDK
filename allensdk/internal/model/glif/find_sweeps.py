...
def get_sweeps_by_name(sweeps, sweep_type):
    if isinstance(sweeps, dict):
        return [s for sn, s in sweeps.items() if s["ephys_stimulus"]["ephys_stimulus_type"]["name"] == sweep_type]
    else:
        return [s for s in sweeps if s["ephys_stimulus"]["ephys_stimulus_type"]["name"] == sweep_type]
...
def find_noise_sweeps(sweeps):
    ...
    out = {"all": all_noise_sweeps, "noise1": noise1["all"], "noise2": noise2["all"]}
