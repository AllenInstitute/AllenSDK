...
def MLIN(voltage, current, res, cap, dt, MAKE_PLOT=False, SHOW_PLOT=False, BLOCK=False, PUBLICATION_PLOT=False):
    ...
    distribution_start_ind = start_idx + int(0.5 / dt)
    ...
    if MAKE_PLOT:
        ...
        data_grid = np.arange(min(v_section), max(v_section), abs(min(v_section) - max(v_section)) / 100.0)
        plt.plot(
            data_grid,
            subthreshold_long_square_voltage_distribution.pdf(data_grid),
            "r",
            label="gauss with\nmeasured var",
        )
        ...
        plt.plot(
            data_grid,
            subthreshold_long_square_voltage_distribution.cdf(data_grid),
            "r",
            label="gauss with\nmeasured var",
        )
        ...
        if PUBLICATION_PLOT:
            ...
            plt.plot(times[distribution_start_ind:distribution_end_ind], v_section * 1.0e3)
            plt.title("Voltage for histogram", fontsize=16)

            plt.subplot2grid((3, 3), (1, 0), rowspan=2)
            plt.hist(v_section * 1.0e3, bins=50, normed=True, label="data")
            data_grid = np.arange(min(v_section), max(v_section), abs(min(v_section) - max(v_section)) / 100.0)
            #                    plt.plot(data_grid, subthreshold_long_square_voltage_distribution.pdf(data_grid), 'r', label='gauss with\nmeasured var')
            plt.plot(data_grid * 1.0e3, 1.0e-3 * expsymm_pdf(data_grid, sv_for_expsymm), "m", lw=3, label="expsymm ")
            ...
            plt.plot(data_grid * 1.0e3, expsymm_cdf(data_grid, sv_for_expsymm), "m", lw=3, label="expsymm")
            plt.title("Normalized cumulative sum", fontsize=16)
            plt.xlabel("V-mean(V) (mV)", fontsize=16)
            plt.legend(loc=2, fontsize=14)

            plt.subplot2grid((3, 3), (1, 2), rowspan=2)
            plt.plot(ACtime, AC * 1.0e3, label="data")
            plt.xlabel("shift (s)", fontsize=14)
            plt.title("Auto correlation", fontsize=16)
            #                    plt.plot(ACtime, exp_decay(ACtime, AC[0], tau_4AC), label='RC')
            plt.plot(ACtime, exp_decay(ACtime, popt[0] * 1.0e3, tau_from_AC), lw=3, label="fit")
            plt.legend(loc=1)
            plt.tight_layout()
...
def expsymm_pdf(v, dv):
    return 1.0 / (2.0 * dv) * np.exp(-np.absolute(v) / dv)


def expsymm_cdf(v, dv):
    return 1.0 / 2.0 + (v * (1 - np.exp(-np.absolute(v) / dv))) / (2.0 * np.absolute(v))
