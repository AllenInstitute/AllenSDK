...
def checkPreprocess(
    originalStim_list, processedStim_list, originalVoltage_list, processedVoltage_list, config, blockME=False
):
    ...
    if "subSample" in config.dictOfPreprocessMethods.keys():
        timeProcessed = (
            np.arange(len(np.concatenate(processedStim_list)))
            * config.dictOfPreprocessMethods["subSample"]["desired_time_step"]
        )
    else:
    ...
    plt.annotate(
        config.cellName + ": View result of preprocessing",
        xy=(0.4, 0.975),
...
def plotSpikes(voltage_list, spike_ind_list, dt, blockME=False, method=False):
    ...
    plt.plot(
        subsampled_time,
        [np.concatenate(voltage_list)[ii] for ii in np.concatenate(converted_spike_ind_list)],
        "r.",
        ms=16,
    )
    if not method:
...
def checkSpikeCutting(
    originalStim_list,
    cutStim_list,
    originalVoltage_list,
    cutVoltage_list,
    ...
    if (
        len(originalStim_list) != len(cutStim_list)
        or len(originalStim_list) != len(originalVoltage_list)
        or len(originalStim_list) != len(cutVoltage_list)
        or len(originalStim_list) != len(allindOfNonSpiking_list)
    ):
    ...
    plt.annotate(
        config.cellName + ": check spike cutting",
        xy=(0.4, 0.975),
        xycoords="figure fraction",
        horizontalalignment="left",
        verticalalignment="top",
