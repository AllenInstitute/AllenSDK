...
class GlifOptimizer(object):
    def __init__(
        ...
        if self.init_params is None:
            ...
            self.init_params = np.ones(expected_param_count)
            logging.warning(
                "optimizer init_params has wrong length (given %d, expected %d). settings to all ones"
                % (len(self.init_params), expected_param_count)
            )
    ...
    def run_many(self, iteration_finished_callback=None, seed=None):
        ...
        for outer in range(0, self.outer_iterations):  # outerloop
            for inner in range(0, self.inner_iterations):  # innerloop
                ...
                logging.info(
                    "fmin took %f secs, %f mins, %f hours"
                    % (
                        time.time() - iteration_start_time,
                        (time.time() - iteration_start_time) / 60,
                        (time.time() - iteration_start_time) / 60 / 60,
                    )
                )
