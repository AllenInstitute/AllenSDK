...
def specify_parameter_groups(dictionary, dict_specifer, neuron_type):
    ...
    output_dict = {
        "El_reference": dictionary["El"]["El_noise"]["measured"]["mean"],
        "El": 0.0,
        ...
        "asc_tau_array": (1.0 / np.array(dictionary["asc"]["k"])).tolist(),
        "th_inf": dictionary["th_inf"]["via_Vmeasure"]["from_zero"],
        "deltaV": None,
        "threshold_adaptation": {
            "a_spike_component_of_threshold": dictionary["threshold_adaptation"]["a_spike_component_of_threshold"],
...
def validate_method_requirements(method_config_name, has_mss):
    ...
    if method_config_name not in valid_configs:
        raise ModelConfigurationException(
            "Model type %s cannot be configured due to missing data (mss: %s)" % (method_config_name, str(has_mss))
        )
...
def configure_model(method_config, preprocessor_values):
    ...
    if (
        ...
        and preprocessor_values["threshold_adaptation"]["a_spike_component_of_threshold"]
    ):
        has_mss = True
    else:
        has_mss = False
...
def configure_method_parameters(
    neuron_config,
    optimizer_config,
    v_reset_slope,
    v_reset_intercept,
    ...
    if method_config.get("params", None) is None:
        if method_config["name"] == "zero":
            ...
            method_config["params"] = {"a": v_reset_slope, "b": v_reset_intercept}

        elif method_config["name"] == "i_v_before":
            method_config["params"] = {"a": 1, "b": 2, "c": 3}
    ...
    if method_config.get("params", None) is None:
        if method_config["name"] == "quadratic_i_of_v":
            raise ModelConfigurationException(
                "quadraticIofV of voltage_dynamics_method preprocessing is not yet implemented"
            )
    ...
    if method_config.get("params", None) is None:
        ...
        if method_config["name"] == "vector":
            method_config["params"] = {"vector": [1, 2, 3]}
    ...
    if method_config.get("params", None) is None:
        if method_config["name"] == "sum":
            method_config["params"] = {"r": np.ones(len(neuron_config["asc_tau_array"]))}
        elif method_config["name"] == "none":
            method_config["params"] = {}
        else:
