...
def MLIN_list_error(param_guess, experiment, input_data):
    ...
    spike_bin_size_time = 0.005  # TODO: PLAY WITH THIS VALUE. 1 MS, 2, 4, 8
    ...
    try:
        ...
        logging.error(
            "voltage STARTS above threshold: setting error to be large.Difference between thresh and voltage is: %f"
            % e.dv
        )
        raise Exception()
    except GlifBadResetException as e:
        logging.error(
            "THIS REALLY SHOULDNT HAPPEN WITH NEW INITIALIZATION EXCEPTION: voltage is above threshold at reset: setting error to be large.  Difference between thresh and voltage is: %f"
            % e.dv
        )
        raise Exception()
    ...
    for stim_list_index in range(0, len(experiment.stim_list)):
        ...
        if len(experiment.spike_time_steps[stim_list_index]) == 0:
            ...
            for ii in range(0, len(bio_spike_ind) - 1):
                between_spike_edges_list.append(
                    [bio_spike_ind[ii] + spike_length, bio_spike_ind[ii + 1] - spike_bin_size_ind]
                )
            ...
            for bin_edges in non_spike_bin_ind_edges:
                ...
                the_ind = bin_ind[
                    min_indicies[-1]
                ]  # this is here just incase there is more than one value at max voltage in a bin
                th_in_bin = th_model[the_ind]
                v_in_bin = v_model[the_ind]
                diffV = th_model[the_ind] - v_model[the_ind]
            ...
            # ---OPTION THREE
            noSpike_negDiff = (-np.log(2.0) + np.array(noSpike_bins["v_th_diff"]) / sv)[
                np.array(noSpike_bins["v_th_diff"]) <= 0.0
            ]
            noSpike_posDiff = (np.log(1.0 - 0.5 * np.exp(-np.array(noSpike_bins["v_th_diff"]) / sv)))[
                np.array(noSpike_bins["v_th_diff"]) > 0.0
            ]
            ...
            spike_posDiff = (-np.log(2.0) - np.array(spike_bins["v_th_diff"]) / sv)[
                np.array(spike_bins["v_th_diff"]) > 0.0
            ]
