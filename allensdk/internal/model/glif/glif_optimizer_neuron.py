...
class GlifOptimizerNeuron(glif_neuron.GlifNeuron):
    ...
    def to_dict(self):
        ...
        curr_dict.update(
            {
                "extrapolation_method_name": self.extrapolation_method_name,
    ...
    def run_with_biological_spikes(self, stimulus, response, bio_spike_time_steps):
        ...
        if voltage_t0 > threshold_t0:
            raise GlifBadInitializationException(
                "Voltage STARTS above threshold: voltage_t0 (%f) threshold_t0 (%f)" % (voltage_t0, threshold_t0)
            )
        ...
        try:
            ...
            if num_spikes == 0:
                ...
                run_data = self.run_until_biological_spike(
                    voltage_t0, threshold_t0, AScurrents_t0, stimulus, response, start_index, end_index, []
                )
                ...
                for spike_num in range(num_spikes):
                    ...
                    assert start_index < end_index, Exception(
                        "start_index > end_index: this is probably because spike_cut_length is longer than the previous inter-spike interval"
                    )
                    ...
                    interpolated_model_spike_times[spike_num] = (
                        run_data["interpolated_model_spike_time"] + start_index * self.dt
                    )
            ...
            if (
                len(interpolated_model_spike_times) != num_spikes
                or len(grid_model_spike_times) != num_spikes
                or len(grid_ISI) != num_spikes
                or len(interpolated_ISI) != num_spikes
                or len(grid_bio_spike_model_voltage) != num_spikes
                or len(grid_bio_spike_model_threshold) != num_spikes
            ):
    ...
    def run_until_biological_spike(
        self,
        voltage_t0,
        threshold_t0,
        AScurrents_t0,
        ...
        local_coarse_indicies = np.append(
            np.arange(num_time_steps_fine)[:: self.dt_multiplier], after_end_index - start_index
        )  # the last indicie in this array is still one longer than the last simulated index
        ...
        t_coarse_grid = (
            np.arange(num_time_steps_coarse - 1) * self.dt
        )  # subtracting the one off here because appending the actual last time that is not the same dt.
        t_coarse_grid = np.append(t_coarse_grid, t_fine_grid[-1])
        dt_vector = (
            t_coarse_grid[1:] - t_coarse_grid[:-1]
        )  # note that this vector is one index shorter than the t_course_grid
        ...
        stimulus_coarse = [
            stimulus[global_coarse_indicies[ii] : global_coarse_indicies[ii + 1]].mean()
            for ii in range(len(global_coarse_indicies) - 1)
        ]

        # step though time steps and calculate voltage values
        for time_step in range(
            len(local_coarse_indicies) - 1
        ):  # minus 1 is needed to match vector sizes because initial inputs are recorded in output vectors.
            ...
            if (
                np.isnan(voltage_t0)
                or np.isinf(voltage_t0)
                or np.isnan(threshold_t0)
                or np.isinf(threshold_t0)
                or any(np.isnan(AScurrents_t0))
                or any(np.isinf(AScurrents_t0))
            ):
                logging.error(self)
                ...
                fv = spi.interp1d(
                    t_coarse_grid[:time_step],
                    voltage_out_coarse_grid[:time_step],
                    assume_sorted=True,
                    bounds_error=False,
                    fill_value=voltage_out_coarse_grid[-1],
                )
                ft = spi.interp1d(
                    t_coarse_grid[:time_step],
                    threshold_out_coarse_grid[:time_step],
                    assume_sorted=True,
                    bounds_error=False,
                    fill_value=threshold_out_coarse_grid[-1],
                )
                ...
                for ii in range(len(AScurrents_t0)):
                    curr_fASc = spi.interp1d(
                        t_coarse_grid[:time_step],
                        AScurrent_matrix_coarse_grid[:time_step, ii],
                        assume_sorted=True,
                        bounds_error=False,
                        fill_value=AScurrent_matrix_coarse_grid[-1, ii],
                    )
                    temp_asc = curr_fASc(temp_fine_grid_for_intp)
                    AScurrent_matrix[: len(temp_asc), ii] = temp_asc

                raise GlifNeuronException(
                    "Invalid threshold, voltage, or after-spike current encountered.",
                    {
            ...
            (voltage_t1, threshold_t1, AScurrents_t1) = self.dynamics(
                voltage_t0,
                threshold_t0,
                AScurrents_t0,
                stimulus_coarse[time_step],
                time_step + start_index,
                bio_spike_time_steps,
            )  # TODO fix list versus array
        ...
        for ii in range(len(AScurrents_t0)):
            curr_fASc = spi.interp1d(
                t_coarse_grid,
                AScurrent_matrix_coarse_grid[:, ii],
                assume_sorted=True,
                bounds_error=False,
                fill_value=AScurrent_matrix_coarse_grid[-1, ii],
            )
        ...
        (
            grid_model_spike_time,
            grid_model_spike_voltage,
            interpolated_model_spike_time,
            interpolated_model_spike_voltage,
        ) = find_first_model_spike(voltage_out, threshold_out, voltage_t1, threshold_t1, self.dt)
        ...
        if grid_model_spike_time is None:
            (
                ...
                interpolated_model_spike_time,
                interpolated_model_spike_voltage,
            ) = self.extrapolation_method(self, voltage_out, threshold_out, voltage_t1, threshold_t1, self.dt)
        ...
        if num_spikes > 0:
            if after_end_index < len(stimulus):
                ...
                (voltage_t0, threshold_t0, AScurrents_t0, bad_reset_flag) = self.reset(
                    voltage_t1, threshold_t1, AScurrents_t1
                )  # USE THIS IF YOU WANT TO USE MODEL VALUES AT TIME OF BIOLOGICAL SPIKE
                # ---------------------------------------------------------------------------------------------------------------------
            else:
...
def find_first_model_spike(voltage, threshold, voltage_t1, threshold_t1, dt):
    ...
    for time_step in range(num_time_steps):
        if voltage[time_step] > threshold[time_step]:
            ...
            interpolated_model_spike_time = glif_neuron.interpolate_spike_time(
                dt,
                time_step - 1,
                threshold[time_step - 1],
                threshold[time_step],
                voltage[time_step - 1],
                voltage[time_step],
            )

            interpolated_model_spike_voltage = interpolate_spike_voltage(
                dt,
                time_step - 1,
                threshold[time_step - 1],
                threshold[time_step],
    ...
    if voltage_t1 > threshold_t1:
        ...
        interpolated_model_spike_time = glif_neuron.interpolate_spike_time(
            dt, num_time_steps - 1, threshold[num_time_steps - 1], threshold_t1, voltage[num_time_steps - 1], voltage_t1
        )
        interpolated_model_spike_voltage = interpolate_spike_voltage(
            dt, num_time_steps, threshold[-1], threshold_t1, voltage[-1], voltage_t1
        )
...
def extrapolate_model_spike_from_endpoints(neuron, voltage, threshold, voltage_t1, threshold_t1, dt):
    ...
    interpolated_model_spike_time = extrapolate_spike_time(
        dt, num_time_steps, threshold[0], threshold_t1, voltage[0], voltage_t1
    )
    interpolated_model_spike_voltage = extrapolate_spike_voltage(
        dt, num_time_steps, threshold[0], threshold_t1, voltage[0], voltage_t1
    )

    grid_model_spike_time = (
        np.ceil(interpolated_model_spike_time / dt) * dt
    )  # grid spike time based off extrapolated spike time
...
def extrapolate_model_spike_from_endpoints_single_tau(neuron, voltage, threshold, voltage_t1, threshold_t1, dt):
    ...
    result = extrapolate_model_spike_from_endpoints(
        neuron, voltage[starting_ind:], threshold[starting_ind:], voltage_t1, threshold_t1, dt
    )
