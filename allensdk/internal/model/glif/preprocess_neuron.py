...
def find_first_spike_voltage(
    voltage, dt, ssq=False, MAKE_PLOT=False, SHOW_PLOT=False, BLOCK=False, dv_cutoff=20.0, thresh_frac=0.05
):
...
def estimate_dv_cutoff(voltage_list, dt, start_t, end_t):
    ...
    dv_cutoff, thresh_frac = ft.estimate_adjusted_detection_parameters(v_set, t_set, start_t, end_t, filter=None)

    return dv_cutoff, thresh_frac


def preprocess_neuron(
    nwb_file, sweep_list, cell_properties=None, dt=None, cut=None, bessel=None, save_figure_path=None
):
    if dt is None:
        dt = DEFAULT_DT
    if cut is None:
    ...
    ssq_dv_cutoff, ssq_thresh_frac = estimate_dv_cutoff(
        all_ssq_data["voltage"], dt, efex.SHORT_SQUARES_WINDOW_START, efex.SHORT_SQUARES_WINDOW_END
    )
    ...
    minimum_suprathreshold_short_square_data = load_sweeps(
        nwb_file, [minimum_suprathreshold_short_square_sweeps[0]], dt, cut, bessel
    )
    ...
    for ss in range(0, len(noise1_data["current"])):
        ...
        subthresh_noise_current_list.append(noise1_data["current"][ss][noise1_data["start_idx"][ss] : int(6.0 / dt)])
        subthresh_noise_voltage_list.append(noise1_data["voltage"][ss][noise1_data["start_idx"][ss] : int(6.0 / dt)])
    ...
    if len(ssq_triple_sweeps):
        ...
        multi_ssq_dv_cutoff, multi_ssq_thresh_frac = estimate_dv_cutoff(
            multi_ssq_data["voltage"], dt, efex.SHORT_SQUARE_TRIPLE_WINDOW_START, efex.SHORT_SQUARE_TRIPLE_WINDOW_END
        )
        print("*************************")
    ...
    maximum_subthreshold_long_square_data = load_sweeps(
        nwb_file, [maximum_subthreshold_long_square_sweeps[0]], dt, cut, bessel
    )
    ...
    for ii in range(len(noise1_ind_wo_test_pulse_removed)):
        ISI_length = np.append(
            ISI_length, noise1_ind_wo_test_pulse_removed[ii][1:] - noise1_ind_wo_test_pulse_removed[ii][:-1]
        )
    for ii in range(len(noise2_ind_wo_test_pulse_removed)):
        ISI_length = np.append(
            ISI_length, noise2_ind_wo_test_pulse_removed[ii][1:] - noise2_ind_wo_test_pulse_removed[ii][:-1]
        )
    ...
    (R_test_list, C_test_list, El_test_list) = least_squares_RCEl_calc_tested(
        subthresh_noise_voltage_list, subthresh_noise_current_list, dt
    )
    ...
    k_asc_possible = np.array([3, 10.0, 30.0, 100.0, 300.0])
    if SHORT_RUN:  # THIS IS JUST FOR DEBUGGING SO THAT YOU DONT HAVE TO WAIT FOR THE ENTIRE MODULE TO RUN
        (best_k_pair_fit_ascR, best_asc_amp_fit_ascR, best_R_fit_ascR, best_llh_fit_ascR) = ASGLM_pairwise(
            k_asc_possible,
            noise1_data["current"],
            noise1_data["voltage"],
            noise1_ind_wo_test_pulse_removed,
            C_test_list_mean,
            C_test_list_mean * R_test_list_mean,
    ...
    try:
        th_inf_via_Vmeasure = find_first_spike_voltage(
            minimum_suprathreshold_short_square_data["voltage"][0][
                minimum_suprathreshold_short_square_data["start_idx"][0] :
            ],
            dt,
            ssq=True,
            MAKE_PLOT=MAKE_PLOT,
        ...
        raise MissingSpikeException(
            "The suprathreshold short square sweep must have a spike, but no spike was detected. This means that feature extraction and GLIF spike detection are inconsistent."
        )
    ...
    if multi_ssq_data:
        (a_spike_component_of_threshold, b_spike_component_of_threshold, mean_voltage_first_spike_of_blip) = (
            calc_spike_component_of_threshold_from_multiblip(
                multi_ssq_data,
                dt,
                multi_ssq_dv_cutoff,
        ...
        if a_spike_component_of_threshold is not None and b_spike_component_of_threshold is not None:
            a_spike_component_of_threshold = spike_component_of_threshold_exact(
                a_spike_component_of_threshold, b_spike_component_of_threshold, spike_cut_length_NODELTAV * dt
            )
        ...
        if a_spike_component_of_threshold is None or b_spike_component_of_threshold is None:
            ...
            fit_ab_vcomp_from_noise = fmin(
                func=fit_avoltage_bvoltage,
                args=(
                    noise1_data["voltage"],
                    noise_El_list,
                    spike_cut_length_NODELTAV,
            ...
            logging.info(
                "spike components %s %s", str(a_spike_component_of_threshold), str(b_spike_component_of_threshold)
            )
            logging.info(
                "voltage components %s %s", str(a_voltage_comp_of_thr_from_fitab), str(b_voltage_comp_of_thr_from_fitab)
            )

            fit_ab_vcomp_th_from_thr_from_noise = fmin(
                func=fit_avoltage_bvoltage_th,
                args=(
                    noise1_data["voltage"],
                    noise_El_list,
                    spike_cut_length_NODELTAV,
            ...
            logging.info(
                "voltage components %s %s %s %s",
                str(a_voltage_comp_of_thr_from_fitabth),
                str(b_voltage_comp_of_thr_from_fitabth),
                "fit threshold",
                str(th_inf_fit_w_v_comp_of_th),
            )
    ...
    STLS_voltage = maximum_subthreshold_long_square_data["voltage"][0][
        maximum_subthreshold_long_square_data["start_idx"][0] :
    ]
    STLS_current = maximum_subthreshold_long_square_data["current"][0][
        maximum_subthreshold_long_square_data["start_idx"][0] :
    ]
    (var_of_section, sv_for_expsymm, tau_from_AC) = MLIN(
        STLS_voltage,
        STLS_current,
        R_test_list_mean,
        C_test_list_mean,
    ...
    for_reference_dict["sweep_properties"] = {
        "noise1": {
    ...
    for_reference_dict["El"] = {
        "El_noise": {"measured": {"mean": El_noise, "list": noise_El_list, "dependencies": None}},
    ...
    for_reference_dict["spike_cut_length"] = {
        "no deltaV shift": {
            "length": spike_cut_length_NODELTAV,
    ...
    for_reference_dict["spike_cutting"] = {
        "NOdeltaV": {
            "cut_length": spike_cut_length_NODELTAV,
            "slope": slope_at_min_expVar_list_NODELTAV,
            "intercept": intercept_at_min_expVar_list_NODELTAV,
            "dependencies": None,
        }
    ...
    for_reference_dict["asc"] = {
        "k": best_k_pair_fit_ascR,
        "amp": asc_amp_from_ASGLM,
        "dependencies": "Cap and res from least squares",
    }

    for_reference_dict["th_inf"] = {
        "via_Vmeasure": {
            "value": th_inf_via_Vmeasure,
            "from_zero": th_inf_via_Vmeasure_from0,
            "dependencies": "measured from suprathreshold blip",
        },
        "fit_with_v_comp_of_th": {"value": th_inf_fit_w_v_comp_of_th, "from_zero": th_inf_fit_w_v_comp_of_th_from0},
    }

    for_reference_dict["threshold_adaptation"] = {
