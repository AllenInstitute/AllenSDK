...
def load_sweep(file_name, sweep_number, desired_dt=None, cut=0, bessel=False):
    ...
    if bessel:
        sample_freq = 1.0 / data["dt"]
        filt_coeff = (bessel["freq"]) / (sample_freq / 2.0)  # filter fraction of Nyquist frequency
    ...
    return {"voltage": data["response"], "current": data["stimulus"], "dt": data["dt"], "start_idx": data["start_idx"]}
