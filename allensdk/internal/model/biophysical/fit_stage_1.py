...
def find_core1_trace(data_set, all_sweeps):
    ...
    for s in sweep_info:
        if (
            sweep_info[s]["quality"]
            and sweep_info[s]["amp"] >= 39.0 + rheobase_amp
            and sweep_info[s]["isi_cv"] < 1.2 * sweep_to_use_isi_cv
        ):
            use_new_sweep = False
            if (
                sweep_to_use_isi_cv > 0.3
                and ((sweep_to_use_isi_cv - sweep_info[s]["isi_cv"]) / sweep_to_use_isi_cv) >= 0.2
            ):
                use_new_sweep = True
...
def is_trace_good_quality(v, i, t):
    ...
    avg_end_isi = (
        (spikes[-1]["threshold_t"] - spikes[-2]["threshold_t"])
        + (spikes[-2]["threshold_t"] - spikes[-3]["threshold_t"])
    ) / 2.0
...
def prepare_stage_1(description, passive_fit_data):
    ...
    ft["slow_trough_delta_time"] = {
        "mean": float(np.mean(sweep_avg_slow_trough_delta_time)),
        "stdev": float(np.std(sweep_avg_slow_trough_delta_time)),
    }
    ft["slow_trough_delta_v"] = {
        "mean": float(np.mean(sweep_avg_slow_trough_delta_v)),
    ...
    target_dict["stimulus"] = [{"amplitude": 1e-3 * stim_amp, "delay": 1000.0, "duration": 1e3 * stim_dur}]
    ...
    config_base_data = ju.read(os.path.join(FIT_BASE_DIR, "config_base.json"))
