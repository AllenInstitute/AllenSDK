...
class Report:
    ...
    def __init__(self, top_level_description, fit_type):
        self.utils = None
        ...
        self.config_path = self.top_level_description.manifest.get_path("fit_config_json", self.fit_type)
    ...
    def make_fit_json_file(self):
        ...
        for i, p in enumerate(self.all_params):
            ...
            json_data["genome"].append(
                {"value": genome_vals[i], "section": p["section"], "name": param_name, "mechanism": p["mechanism"]}
            )

        # write out file
        with open(
            self.top_level_description.manifest.get_path("output_fit_file", self.specimen_id, self.fit_type), "w"
        ) as f:
    ...
    def setup_model(self):
        ...
        self.utils.set_iclamp_params(
            self.stim_params["amplitude"], self.stim_params["delay"], self.stim_params["duration"]
        )
    ...
    def gather_from_seeds(self):
        first_created = False
        for s in self.seeds:
            final_hof_fit_path = self.top_level_description.manifest.get_path("final_hof_fit", self.fit_type, s)
            final_hof_path = self.top_level_description.manifest.get_path("final_hof", self.fit_type, s)
