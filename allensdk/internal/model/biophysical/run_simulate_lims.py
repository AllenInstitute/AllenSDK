...
class RunSimulateLims(RunSimulate):
    ...
    def generate_manifest_rma(self, neuronal_model_run_id, manifest_path, api_url=None):
        """
        Note
        ----
    ...
    def generate_manifest_lims(self, lims_data_path, manifest_path):
        """
        Note
        ----
        Other necessary files are also written.
    ...
    def copy_local(self):
        ...
        shutil.copy(self.manifest.get_path("fit_parameters"), workdir)

        shutil.copyfile(self.manifest.get_path("stimulus_path"), self.manifest.get_path("output_path"))

        shutil.copy(str(files("allensdk.model.biophysical").joinpath("cell.hoc")), os.curdir)
...
def main(command, lims_strategy_json, lims_response_json):
    ...
    """
    rs = RunSimulateLims(lims_strategy_json, lims_response_json)
