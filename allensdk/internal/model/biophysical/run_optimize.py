...
class RunOptimize(object):
    ...
    def copy_local(self):
        ...
        modfiles = [
            self.manifest.get_path(key)
            for key, info in self.manifest.path_info.items()
        ...
        shutil.copy(str(files("allensdk.model.biophysical").joinpath("cell.hoc")), self.manifest.get_path("BASEDIR"))

    def generate_manifest_rma(self, neuronal_model_id, manifest_path, api_url=None):
        """
        Note
        ----
    ...
    def start_specimen(self):
        ...
        self.passive_fit_data = run_passive_fit.run_passive_fit(self.app_config)

        ju.write(self.manifest.get_path("passive_fit_data"), self.passive_fit_data)

        self.stage_1_jobs = fit_stage_1.prepare_stage_1(self.app_config, self.passive_fit_data)

        ju.write(self.manifest.get_path("stage_1_jobs"), self.stage_1_jobs)
    ...
    def make_fit(self):
        ...
        best_fit_type, min_fit_value = reduce(
            lambda a, b: a if (a[1] < b[1]) else b, (i for i in best_fit_values.items() if i[1] is not None)
        )
        best_fit_file = self.manifest.get_path("output_fit_file", specimen_id, best_fit_type)
        ...
        lims_upload_config.update_well_known_file(
            best_fit_file, well_known_file_type_id=OptimizeConfigReader.NEURONAL_MODEL_PARAMETERS
        )
...
def main(command, input_json, output_json):
    ...
    o = RunOptimize(input_json, output_json)
