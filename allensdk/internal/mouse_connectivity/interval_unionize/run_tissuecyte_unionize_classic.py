...
def get_ancestor_id_map(structures):
    tree = SimpleTree(structures, lambda st: int(st["id"]), lambda st: st["parent_structure_id"])
    ancestor_id_map = tree.value_map(lambda st: st["id"], lambda st: tree.ancestor_ids([st["id"]])[0])
...
def run(input_data):
    ...
    signal_arrays = du.get_injection_data(
        input_data["grid_paths"]["injection_fraction"],
        input_data["grid_paths"]["injection_density"],
        input_data["grid_paths"]["injection_energy"],
    )
    signal_arrays.update(
        du.get_projection_data(
            input_data["grid_paths"]["projection_density"],
    ...
    signal_arrays.update(
        du.get_sum_pixel_intensities(
            input_data["grid_paths"]["sum_pixel_intensities"],
    ...
    raw_unionizes = TissuecyteUnionizer.propagate_unionizes(raw_unionizes, ancestor_id_map)

    logging.info("propagating data to bilateral unionizes")
    bilateral = TissuecyteUnionizer.propagate_to_bilateral(raw_unionizes)

    cooked_unionizes = list(
        unionizer.postprocess_unionizes(
            raw_unionizes,
