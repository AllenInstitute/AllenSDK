...
def sitk_get_image_parameters(volume):
    return (np.array(volume.GetSpacing()), np.array(volume.GetSize()), np.array(volume.GetOrigin()))
...
def sitk_paste_into_center(smaller, larger):
    ...
    if not np.allclose(smaller_parities, larger_parities):
        logging.warn(
            "parities differ, result will not be centered : {0}, {1}".format(smaller_parities, larger_parities)
        )
