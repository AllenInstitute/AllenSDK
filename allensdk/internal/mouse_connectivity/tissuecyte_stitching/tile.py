...
class Tile(object):
    def __init__(self, index, image, is_missing, bounds, channel, size, margins, *args, **kwargs):
        ...
        logging.info(
            "tile {index} on channel {channel} starts at ({0}, {1})".format(
                self.bounds["row"]["start"], self.bounds["column"]["start"], index=self.index, channel=self.channel
            )
        )
    ...
    def trim(self, image):
        ...
        return image[
            self.margins["row"] : self.margins["row"] + self.size["row"],
            self.margins["column"] : self.margins["column"] + self.size["column"],
        ]

    def average_tile_is_untrimmed(self, average_tile):
        return average_tile.shape[0] > self.image.shape[0] or average_tile.shape[1] > self.image.shape[1]

    def apply_average_tile(self, average_tile):
        if average_tile is None:
            logging.info(
                "no average tile found for tile with index {index} on channel {channel}".format(**self.__dict__)
            )
            return self.image
        ...
        logging.info(
            "applying flatfield correction to tile with index {index} on channel {channel}".format(**self.__dict__)
        )
    ...
    def get_image_region(self):
        ...
        return slice(row["start"], row["end"]), slice(col["start"], col["end"]), self.channel

    def get_missing_path(self):
        row = self.bounds["row"]
        col = self.bounds["column"]

        path = [row["start"], col["start"], row["end"], col["start"], row["end"], col["end"], row["start"], col["end"]]
