...
def plot_masks(dm, save_dir, movie_file, movie_dataset, window=150, add_background=True):
    ...
    overlap_pairs = [
        (x, y) for (x, y) in zip(*np.where(dm.mask_overlap > 0)) if x > y and x != dm.mask_overlap.shape[0] - 1
    ]
...
def _add_stim_epochs(trace, ax, data_set):
    stim_colors_dict = {
        "locally_sparse_noise": "green",
        "drifting_gratings": "yellow",
        "natural_movie_one": "magenta",
        "natural_movie_two": "magenta",
        "natural_movie_three": "red",
