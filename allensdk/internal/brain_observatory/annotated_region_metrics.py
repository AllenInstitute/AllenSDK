...
def eccentricity(az, alt, az_center, alt_center):
    ...
    daz = az - az_center
    dalt = alt - alt_center
    ecc = np.arctan(np.sqrt(np.square(np.tan(dalt)) + np.square(np.tan(daz)) / np.square(np.cos(dalt))))
    return ecc
...
def get_metrics(
    ...
    region_mask = create_region_mask(altitude.shape, x, y, width, height, mask)
    ...
    az_min, az_max, az_range, az_bias = retinotopy_metric(region_mask, azimuth)
    alt_min, alt_max, alt_range, alt_bias = retinotopy_metric(region_mask, altitude)
