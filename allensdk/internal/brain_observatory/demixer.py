...
@deprecated(
    "The internal demixer module is deprecated and will be removed. "
...
def demix_time_dep_masks(raw_traces, stack, masks):
    ...
    for t in range(T):
        ...
        if np.sum(drop_test == 0):
            ...
            overlap = flat_masks.dot(
                flat_weighted_masks.T
            ).toarray()  # cast to dense numpy array for linear solver because solution is dense
            try:
