...
class FitEllipse(object):
    ...
    def ransac_fit(self, candidate_points):
        ...
        if self.best_params_set:
            return (
                ellipse_center(self.best_params),
                ellipse_angle_of_rotation(self.best_params) * 180.0 / np.pi,
                ellipse_axis_length(self.best_params),
            )
        else:
            return None
...
def fit_ellipse(candidate_points):
    ...
    return ellipse_center(params), ellipse_angle_of_rotation(params) * 180.0 / np.pi, ellipse_axis_length(params)
