...
def open_view_on_binary(file_like, dtype=np.uint8, mode="r", offset=0, shape=None, order="C", strides=None):
    ...
    return np.lib.stride_tricks.as_strided(mapped, shape=shape, strides=strides)
...
def load_frame(raw_filename, json_meta, use_memmap=False):
    ...
    if use_memmap:
        arr = open_view_on_binary(
            raw_filename,
            dtype=json_meta["dtype"],
...
def export_frame_to_hdf5(
    raw_filename, data_hdf5_filename, auxiliary_hdf5_filename, frame_meta, compression="gzip", compression_opts=9
):
