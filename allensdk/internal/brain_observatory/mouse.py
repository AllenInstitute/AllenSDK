...
class Mouse:
    ...
    def get_behavior_sessions(self, exclude_invalid_sessions: bool = True) -> List[BehaviorMetadata]:
        """
        ...
        behavior_sessions = get_session_metadata_multiprocessing(
            behavior_session_ids=behavior_session_ids, lims_engine=lims_db
        )
    ...
    @classmethod
    def from_behavior_session_id(cls, behavior_session_id: int) -> "Mouse":
        """Instantiates `Mouse` from `behavior_session_id`"""
        lims_db = db_connection_creator(fallback_credentials=LIMS_DB_CREDENTIAL_MAP)
        mouse_id = MouseId.from_lims(behavior_session_id=behavior_session_id, lims_db=lims_db)
        return Mouse(mouse_id=mouse_id.value)

    def get_images_shown(self, up_to_behavior_session_id: Optional[int] = None, n_workers: Optional[int] = None):
        ...
        if up_to_behavior_session_id is not None:
            this_date_of_acquisition = [
                x.date_of_acquisition for x in behavior_sessions if x.behavior_session_id == up_to_behavior_session_id
            ][0]

            prior_behavior_session_ids = set(
                [x.behavior_session_id for x in behavior_sessions if x.date_of_acquisition < this_date_of_acquisition]
            )
            behavior_sessions = [x for x in behavior_sessions if x.behavior_session_id in prior_behavior_session_ids]
        images_shown = get_images_shown(
            behavior_session_ids=([x.behavior_session_id for x in behavior_sessions]),
            lims_engine=lims_db,
            n_workers=n_workers,
        )
        return images_shown
