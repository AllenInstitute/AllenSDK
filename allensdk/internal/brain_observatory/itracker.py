...
class iTracker(object):
    ...
    def estimate_bbox_from_mean_frame(self, margin=75, image_type="png"):
        ...
        mean_frame_annotated = self.annotate_frame_with_bbox(
            mean_frame_annotated, pupil_bbox=bbox_pupil, cr_bbox=bbox_cr
        )
        mean_frame_annotated = self.annotate_frame_with_point(
            mean_frame_annotated, pupil=(x_pupil, y_pupil), cr=(x_cr, y_cr)
        )
    ...
    def compute_mean_frame(self, image_file_type="png"):
        ...
        if self.bbox_cr and self.bbox_pupil:
            ...
            mean_frame_annotated = self.annotate_frame_with_bbox(
                mean_frame_annotated, pupil_bbox=self.bbox_pupil, cr_bbox=self.bbox_cr
            )
    ...
    def process_movie(
        self, movie_output_stream=None, output_frames=False, output_annotation_frames=False, image_file_type="jpg"
    ):
        ...
        for i, input_frame in enumerate(self.input_stream):
            ...
            if movie_output_stream or annotation_frame_output_stream:
                annotated_frame = self.annotate_frame(
                    np.dstack([input_frame, input_frame, input_frame]), pupil_params, cr_params
                )
    ...
    def process_image(self, im, bbox_pupil=None, bbox_cr=None):
        ...
        pupil_candidate_points = self.get_candidate_points(
            self.pupil_rays, self.pupil_ray_values, self.threshold_factor, above_threshold=True
        )
