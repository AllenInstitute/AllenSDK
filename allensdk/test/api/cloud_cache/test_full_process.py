...
def test_full_cache_system(tmpdir):
    ...
    metadata2_v1 = pd.DataFrame({"experiment": [5, 6, 7], "file_id": ["data1", "data2", "data3"]})

    metadata1_v2 = pd.DataFrame({"mouse": [8, 9, 0], "sex": ["M", "F", "M"]})

    v1_hashes = {}
    for data, key in zip((data1_v1, data2_v1, data3_v1), ("data1", "data2", "data3")):
        hasher = hashlib.blake2b()
        hasher.update(data)
        v1_hashes[key] = hasher.hexdigest()
        s3_client.put_object(Bucket=test_bucket_name, Key=f"proj/data/{key}", Body=data)

    for df, key in zip((metadata1_v1, metadata2_v1), ("proj/metadata1.csv", "proj/metadata2.csv")):
    ...
    s3_client.put_object(Bucket=test_bucket_name, Key="proj/metadata1.csv", Body=data)
