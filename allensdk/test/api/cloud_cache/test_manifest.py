...
def test_create_file_attributes(meta_json_path):
    ...
    attr = mfest._create_file_attributes("http://my.url.com/path/to/file.txt", "12345", "aaabbbcccddd")
...
def manifest_for_metadata(tmpdir):
    ...
    manifest = {}
    metadata_files = {}
    metadata_files["a.txt"] = {"url": "http://my.url.com/path/to/a.txt", "version_id": "12345", "file_hash": "abcde"}
    metadata_files["b.txt"] = {
        "url": "http://my.other.url.com/different/path/to/b.txt",  # noqa: E501
        "version_id": "67890",
        "file_hash": "fghijk",
    }
...
def manifest_with_data(tmpdir):
    ...
    manifest["data_pipeline"] = "placeholder"
    data_files = {}
    data_files["a"] = {"url": "http://my.url.com/myproject/path/to/a.nwb", "version_id": "12345", "file_hash": "abcde"}
    data_files["b"] = {
        "url": "http://my.other.url.com/different/path/b.nwb",
        "version_id": "67890",
        "file_hash": "fghijk",
    }
...
def test_file_attribute_errors(meta_json_path):
    ...
    """
    mfest = Manifest("/my/cache/dir", meta_json_path)
    with pytest.raises(ValueError, match=r".* not in self.metadata_file_names"):
        mfest.metadata_file_attributes("some_file.txt")

    with pytest.raises(ValueError, match=r".* not a data file listed in manifest"):
