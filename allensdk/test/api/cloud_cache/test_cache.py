...
def test_list_all_manifests(tmpdir):
    ...
    client.put_object(Bucket=test_bucket_name, Key="proj/manifests/manifest_v1.0.0.json", Body=b"123456")
    ...
    assert cache.manifest_file_names == ["manifest_v1.0.0.json", "manifest_v2.0.0.json"]
...
def test_loading_manifest(tmpdir):
    ...
    manifest_1 = {
        ...
        "data_files": {},
        "metadata_files": {
            "a.csv": {"url": "http://www.junk.com", "version_id": "1111", "file_hash": "abcde"},
            "b.csv": {"url": "http://silly.com", "version_id": "2222", "file_hash": "fghijk"},
        },
    }
    ...
    manifest_2 = {
        ...
        "data_files": {},
        "metadata_files": {
            "c.csv": {"url": "http://www.absurd.com", "version_id": "3333", "file_hash": "lmnop"},
            "d.csv": {"url": "http://nonsense.com", "version_id": "4444", "file_hash": "qrstuv"},
        },
    }

    client.put_object(
        Bucket=test_bucket_name, Key="proj/manifests/manifest_v1.0.0.json", Body=bytes(json.dumps(manifest_1), "utf-8")
    )

    client.put_object(
        Bucket=test_bucket_name, Key="proj/manifests/manifest_v2.0.0.json", Body=bytes(json.dumps(manifest_2), "utf-8")
    )
...
def test_file_exists(tmpdir):
    ...
    # should be true
    good_attribute = CacheFileAttributes("http://silly.url.com", "12345", true_checksum, test_file_path)
    assert cache._file_exists(good_attribute)

    # test when file path is wrong
    bad_path = pathlib.Path("definitely/not/a/file.txt")
    bad_attribute = CacheFileAttributes("http://silly.url.com", "12345", true_checksum, bad_path)

    assert not cache._file_exists(bad_attribute)

    # test when path exists but is not a file
    bad_attribute = CacheFileAttributes("http://silly.url.com", "12345", true_checksum, pathlib.Path(tmpdir))
...
def test_download_file(tmpdir):
    ...
    client = boto3.client("s3", region_name="us-east-1")
    client.put_object(Bucket=test_bucket_name, Key="data/data_file.txt", Body=data)
    ...
    good_attributes = CacheFileAttributes(url, version_id, true_checksum, expected_path)
...
def test_download_file_multiple_versions(tmpdir):
    ...
    client = boto3.client("s3", region_name="us-east-1")
    client.put_object(Bucket=test_bucket_name, Key="data/data_file.txt", Body=data_1)

    response = client.list_object_versions(Bucket=test_bucket_name)
    version_id_1 = response["Versions"][0]["VersionId"]

    client = boto3.client("s3", region_name="us-east-1")
    client.put_object(Bucket=test_bucket_name, Key="data/data_file.txt", Body=data_2)
    ...
    good_attributes = CacheFileAttributes(url, version_id_1, true_checksum_1, expected_path)
    ...
    good_attributes = CacheFileAttributes(url, version_id_2, true_checksum_2, expected_path)
...
def test_download_data(tmpdir):
    ...
    data_file = {"url": url, "version_id": version_id, "file_hash": true_checksum}

    manifest["data_files"] = {"only_data_file": data_file}
    manifest["data_pipeline"] = "placeholder"

    client.put_object(
        Bucket=test_bucket_name, Key="proj/manifests/manifest_v1.0.0.json", Body=bytes(json.dumps(manifest), "utf-8")
    )
...
def test_download_metadata(tmpdir):
    ...
    client = boto3.client("s3", region_name="us-east-1")
    meta_version = client.put_object(Bucket=test_bucket_name, Key="metadata_file.csv", Body=data)["VersionId"]
    ...
    metadata_file = {"url": url, "version_id": version_id, "file_hash": true_checksum}
    ...
    def response_fun(Bucket, Prefix):
        ...
        return {"Versions": [{"VersionId": meta_version, "Key": "metadata_file.csv", "Size": 12}]}
...
def test_metadata(tmpdir):
    ...
    client = boto3.client("s3", region_name="us-east-1")
    client.put_object(Bucket=test_bucket_name, Key="metadata_file.csv", Body=data)
...
def test_outdated_manifest_warning(tmpdir, example_datasets_with_metadata):
    ...
    create_bucket(bucket_name, example_datasets_with_metadata["data"], metadatasets=metadatasets)
...
def test_list_all_downloaded(tmpdir, example_datasets_with_metadata):
    ...
    expected = {"project-x_manifest_v5.0.0.json", "project-x_manifest_v2.0.0.json", "project-x_manifest_v3.0.0.json"}
...
def test_load_last_manifest(tmpdir, example_datasets_with_metadata):
    ...
    with pytest.warns(OutdatedManifestWarning, match=expected):
        cache.load_last_manifest()
