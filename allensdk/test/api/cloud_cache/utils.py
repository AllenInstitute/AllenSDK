...
def load_dataset(
    data_blobs: dict, metadata_blobs: Union[dict, None], manifest_version: str, bucket_name: str, client: boto3.client
) -> None:
    """
    ...
    for fname in data_blobs:
        client.put_object(Bucket=bucket_name, Key=f"project-x/data/{fname}", Body=data_blobs[fname]["data"])

    if metadata_blobs is not None:
        for fname in metadata_blobs:
            client.put_object(Bucket=bucket_name, Key=f"project-x/project_metadata/{fname}", Body=metadata_blobs[fname])
    ...
    for fname in data_blobs:
        ...
        data_file = {"url": url, "version_id": fname_to_version[fname], "file_hash": checksum}
    ...
    if metadata_blobs is not None:
        ...
        for fname in metadata_blobs:
            ...
            metadata_dict[fname] = {"url": url, "file_hash": hasher.hexdigest(), "version_id": fname_to_version[fname]}
    ...
    client.put_object(Bucket=bucket_name, Key=manifest_k, Body=bytes(json.dumps(manifest), "utf-8"))

    return None


def create_bucket(test_bucket_name: str, datasets: dict, metadatasets: Optional[dict] = None) -> None:
    """
    ...
    for v in datasets.keys():
        if metadatasets is not None:
            ...
            m = None
        load_dataset(datasets[v], m, v, test_bucket_name, client)
