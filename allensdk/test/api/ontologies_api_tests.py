...
class OntologiesApiTests(unittest.TestCase):
    ...
    def test_build_structure_query(self):
        expected = "http://api.brain-map.org/api/v2/data/query.json?q=model::Structure,rma::include,[graph_id$eq1],rma::options[tabular$eqstructures.id,structures.acronym,structures.graph_order,structures.color_hex_triplet,structures.structure_id_path,structures.name][num_rows$eq'all'][order$eqstructures.graph_order]"
