...
def test_get_experiment_container_metric(bo_api):
    ...
    bo_api.get_experiment_container_metric(id)
    bo_api.json_msg_query.assert_called_once_with(expected)


def test_get_experiment_container(bo_api):
    id = 511510753
    bo_api.get_experiment_container(id)
    expected = "http://testwarehouse:9000/api/v2/data/query.json?q=model::ExperimentContainer,rma::criteria,[id$in511510753],rma::include,ophys_experiments,isi_experiment,specimen,targeted_structure,rma::options[num_rows$eq'all'][count$eqfalse]"
    bo_api.json_msg_query.assert_called_once_with(expected)
...
def test_get_stimulus_mapping_no_ids(bo_api):
    bo_api.get_stimulus_mapping()
...
def test_get_stimulus_mapping_one_id(bo_api):
    ids = 15
    bo_api.get_stimulus_mapping(stimulus_mapping_ids=ids)
...
def test_get_stimulus_mapping_two_ids(bo_api):
    ids = [15, 43]
    bo_api.get_stimulus_mapping(stimulus_mapping_ids=ids)
...
def test_get_cell_metric_no_ids(bo_api):
    bo_api.get_cell_metric()
...
def test_get_cell_metric_one_ids(bo_api):
    id = 517394843
    bo_api.get_cell_metric(cell_specimen_ids=id)
...
def test_get_cell_metric_two_ids(bo_api):
    ids = [517394843,517394850]
    bo_api.get_cell_metric(cell_specimen_ids=ids)
