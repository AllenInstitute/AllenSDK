...
from allensdk.test_utilities.custom_comparators import stimulus_pickle_equivalence
...
def dict_data_fixture():
    ...
    output_data = {"the_first": 2.1, "the_second": "funny", "the_third": "two", "the_fourth": "three"}
...
def nested_dict_data_fixture():
    ...
    input_data = {b"a": [1, b"b", 2, "c"], "c": {b"d": 4, b"e": b"f"}, b"g": b"h"}

    output_data = {"a": [1, "b", 2, "c"], "c": {"d": 4, "e": "f"}, "g": "h"}

    return {"input": input_data, "output": output_data}


@pytest.fixture
def nested_list_data_fixture(list_data_fixture, nested_dict_data_fixture):
    """
    Return a dict.
    ...
    'output' maps to the desired list with bytes cast as strings.
    """
    input_data = [copy.deepcopy(list_data_fixture["input"]), copy.deepcopy(nested_dict_data_fixture["input"])]

    output_data = [copy.deepcopy(list_data_fixture["output"]), copy.deepcopy(nested_dict_data_fixture["output"])]

    return {"input": input_data, "output": output_data}


@pytest.mark.parametrize("nested", [True, False])
def test_sanitize_list(list_data_fixture, nested_list_data_fixture, nested):
    """
...
@pytest.mark.parametrize("nested", [True, False])
def test_sanitize_dict(dict_data_fixture, nested_dict_data_fixture, nested):
    """
...
def test_sanitize_tuple():
    ...
    """
    input_data = (["a", b"b", "c", 2], "cat", b"dog", {b"d": 2, "e": 3, b"f": b"g"})

    expected_data = (["a", "b", "c", 2], "cat", "dog", {"d": 2, "e": 3, "f": "g"})
...
def test_sanitize_list_or_tuple():
    ...
    input_data = [["h", b"i", "j", 2], "frog", b"fly", {b"k": 2, "l": 3, b"m": b"n"}]

    expected_data = [["h", "i", "j", 2], "frog", "fly", {"k": 2, "l": 3, "m": "n"}]
    actual = _sanitize_list_or_tuple(input_data)
    assert actual == expected_data


def test_sanitize_pickle_data(nested_list_data_fixture, nested_dict_data_fixture):
    """
    Test user-facing sanitization method
    """
    actual = _sanitize_pickle_data(copy.deepcopy(nested_list_data_fixture["input"]))
    assert actual == nested_list_data_fixture["output"]

    actual = _sanitize_pickle_data(copy.deepcopy(nested_dict_data_fixture["input"]))
...
def test_local_pickle_equivalence():
    ...
    sanitized_data = load_and_sanitize_pickle(pickle_path=pkl_path)
...
def test_pickle_equivalence():
    ...
    sanitized_data = load_and_sanitize_pickle(pickle_path=file_path)
    assert stimulus_pickle_equivalence(sanitized_data, pd_data)
