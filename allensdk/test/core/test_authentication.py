...
from allensdk.core.authentication import EnvCredentialProvider, credential_injector, set_credential_provider
...
def test_credential_injector(provider, credential_map, expected):
    ...
    assert credential_injector(credential_map, provider)(mock_func)() == expected


@pytest.mark.parametrize(
    "provider,credential_map,expected",
    [
        (EnvCredentialProvider({"LIMS_USER": "user", "LIMS_PASSWORD": "1234"}), {"user": "LIMS_USER"}, ("user")),
    ],
)
def test_credential_injector_only_injects_existing_kwargs(provider, credential_map, expected):
    def mock_func(*, user):
        return user
...
def test_credential_injector_only_injects_mapped_credentials(provider, credential_map):
...
def test_credential_injector_preserves_function_args(provider, credential_map):
    ...
    assert credential_injector(credential_map, provider)(mock_func)("arg1", kwarg1="kwarg1") == (
        "arg1",
        "kwarg1",
        "user",
        "1234",
