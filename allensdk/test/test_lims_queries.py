...
from allensdk.internal.api.queries.utils import build_in_list_selector_query, _sanitize_uuid_list, build_where_clause

from allensdk.internal.api.queries.behavior_lims_queries import foraging_id_map_from_behavior_session_id
...
def test_build_in_list_selector_query(col, valid_list, operator, valid, expected):
    assert expected == build_in_list_selector_query(col=col, valid_list=valid_list, operator=operator, valid=valid)
...
def test_build_in_selector_error():
    ...
    with pytest.raises(ValueError, match="Operator must be"):
        build_in_list_selector_query(col="silly", valid_list=[1, 2, 3], operator="above", valid=True)
...
def test_foraging_id_map(behavior_session_ids, expected):
    assert expected == foraging_id_map_from_behavior_session_id(
        lims_engine=MockQueryEngine(), logger=None, behavior_session_ids=behavior_session_ids
    )
...
def test_sanitize_uuid_list():
    ...
    sanitized = _sanitize_uuid_list(uuid_list=input_list)

    assert sanitized == ["12345678123456781234567812345678", "abcdefab-1234-abcd-0123-0123456789ab"]
...
def test_donor_id_list_from_ecephys_session_ids():
    ...
    class DummyConnection(object):
        def select(self, query=None):
            data = [
                {"ecephys_session_id": 1, "donor_id": 2},
    ...
    actual = donor_id_list_from_ecephys_session_ids(lims_connection=DummyConnection(), session_id_list=[9, 9, 9])
