...
@pytest.fixture(params=[(False, False), (True, True), (False, True), (True, False)])
def segmentation(request):
    ...
    return TestSegmentation(np.array(masks), n_rois, has_duplicates, has_unions)
...
def old_csv(tmpdir_factory, xy_data):
    data = [
        "0,{},154.086,-14.9831,0,0,1,0.0254625".format(xy_data[0]),
...
def new_csv(tmpdir_factory, xy_data):
    data = [
        "framenumber,x,y,correlation,input_x,input_y,estimate",
...
def test_calculate_max_border_all_outliers():
    df = pd.DataFrame(np.ones((100, 9)), columns=["index", "x", "y", "a", "b", "c", "d", "e", "f"])
...
def test_create_feature_array(object_list):
    ...
    with pytest.raises(KeyError):
        roi_filter.create_feature_array(failing_data, object_list, depth, structure_id, drivers, reporters)
    feature_array = roi_filter.create_feature_array(passing_data, object_list, depth, structure_id, drivers, reporters)
    assert np.all(feature_array.columns == passing_data["training_features"].columns)
