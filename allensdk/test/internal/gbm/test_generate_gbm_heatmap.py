...
def test_create_transcripts_for_genes():
    analysis_run_gene_file = {
        "analysis_run_gene_path": TEST_GENE_FILE,
        "analysis_run_transcript_path": TEST_TRANSCRIPT_FILE,
...
def test_create_genes_for_transcripts():
    ...
    d = [["transcript_id", "gene_id"], ["NM_000015_2", "10"], ["NM_130786_3", "1"], ["tRNA-Tyr.100009601.chr14", "100"]]
    expected_data = pd.DataFrame(data=d)
    assert expected_data.equals(data)
...
def test_create_gene_fpkm_table():
    analysis_run_records = [
        ...
        {
            "analysis_run_gene_path": TEST2_GENE_FILE,
            "analysis_run_transcript_path": TEST2_TRANSCRIPT_FILE,
...
def test_create_transcript_fpkm_table():
    ...
    expected_data = pd.DataFrame(
        data=d, columns=[300173630, 300173634], index=["NM_000015_2", "NM_130786_3", "tRNA-Tyr.100009601.chr14"]
    )
    assert expected_data.equals(data)
...
def test_create_sample_metadata():
    ...
    d = [
        [
            ...
            "5d04",
            309780592,
            "Cellular Tumor sampled by reference histology",
            703393,
            "W1-1-2",
        ],
        [
            ...
            "ff330",
            309780906,
            "Microvascular proliferation sampled by reference histology",
            703393,
            "W1-1-2",
        ],
    ]
