...
from allensdk.internal.api.queries.grid_data_api_prerelease import GridDataApiPrerelease, _get_grid_storage_directories


@pytest.fixture
def storage_dirs(fn_temp_dir):
    return {"111": os.path.join(fn_temp_dir, "111"), "222": os.path.join(fn_temp_dir, "222")}


@pytest.fixture
def query_result(fn_temp_dir):
    return [
        {b"id": 111, b"storage_directory": os.path.join(fn_temp_dir, "111")},
...
def test_get_grid_storage_directories(storage_dirs, query_result, fn_temp_dir):
    ...
    with mock.patch("allensdk.internal.core.lims_utilities.query", new=lambda a: query_result):
...
def test_from_file_name(storage_dirs, fn_temp_dir):
    ...
    with mock.patch(
        "allensdk.internal.api.queries.grid_data_api_prerelease._get_grid_storage_directories",
        new=lambda a: storage_dirs,
    ):
        GridDataApiPrerelease.from_file_name(file_name)

    with mock.patch("allensdk.internal.api.queries.grid_data_api_prerelease._get_grid_storage_directories") as ggsd:
        GridDataApiPrerelease.from_file_name(file_name)
...
def test_download_projection_grid_data(grid_data, fn_temp_dir):
    ...
    assert_raises(ValueError, grid_data.download_projection_grid_data, target, 0, "projection_density_100.nrrd")
