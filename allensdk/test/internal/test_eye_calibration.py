...
def cr_params():
    x, y = np.meshgrid(np.array([300, 320, 340]), np.array([220, 240, 260]))
    return np.vstack((x.flatten(), y.flatten())).T


def pupil_params():
    x, y = np.meshgrid(np.array([280, 320, 380]), np.array([200, 240, 280]))
    return np.vstack((x.flatten(), y.flatten())).T
...
def test_cr_position_in_mouse_eye_coordinates(led_position, eye_radius):
    cr = eye_calibration.EyeCalibration.cr_position_in_mouse_eye_coordinates(led_position, eye_radius)
    tol = 0.000000001
    assert np.abs(np.linalg.norm(cr) - 0.5 * eye_radius) < tol
    err = np.abs(cr / np.linalg.norm(cr) - led_position / np.linalg.norm(led_position))
...
def test_pupil_position_in_mouse_eye_coordinates_right(led_position, camera_rotations):
    ...
    pupil = pupil_params()
    cr = cr_params()
    bad = c.pupil_position_in_mouse_eye_coordinates(np.array([[1000, 0], [0, 1000]]), np.array([[0, 0], [0, 0]]))
...
def test_pupil_position_in_mouse_eye_coordinates_front(led_position, camera_rotations):
    CM_PER_PIXEL = 10.2 / 10000
    TOL = 0.0000000001
    c = eye_calibration.EyeCalibration(
        led_position=led_position,
