...
from allensdk.internal.core.mouse_connectivity_cache_prerelease import MouseConnectivityCachePrerelease


@pytest.fixture(scope="function")
def mcc(fn_temp_dir):
    storage_dirs = {"111": os.path.join(fn_temp_dir, "111"), "222": os.path.join(fn_temp_dir, "222")}
    ...
    return MouseConnectivityCachePrerelease(manifest_file=manifest_path, storage_directories_file_name=file_name)
    ...
    return [
        {
            ...
            "injection_structures": [
                {"id": 184, "name": "Frontal pole, cerebral cortex", "abbreviation": "FRP"},
                {"id": 993, "name": "Secondary motor area", "abbreviation": "MOs"},
            ],
        }
    ]
...
def test_get_projection_density(mcc, fn_temp_dir):
    eye = np.eye(100)
    eid = 111
    path = os.path.join(fn_temp_dir, "experiment_{0}".format(eid), "projection_density_25.nrrd")

    with mock.patch(
        "allensdk.internal.api.api_prerelease.ApiPrerelease.retrieve_file_from_storage",
        new=lambda a, b, c: nrrd.write(c, eye),
    ):
        obtained, _ = mcc.get_projection_density(eid)

    with mock.patch.object(mcc.api.grid_data_api.api, "retrieve_file_from_storage") as mock_rtrv:
...
def test_get_injection_density(mcc, fn_temp_dir):
    eye = np.eye(100)
    eid = 111
    path = os.path.join(fn_temp_dir, "experiment_{0}".format(eid), "injection_density_25.nrrd")
...
def test_get_injection_fraction(mcc, fn_temp_dir):
    eye = np.eye(100)
    eid = 111
    path = os.path.join(fn_temp_dir, "experiment_{0}".format(eid), "injection_fraction_25.nrrd")
...
def test_get_data_mask(mcc, fn_temp_dir):
    eye = np.eye(100)
    eid = 111
    path = os.path.join(fn_temp_dir, "experiment_{0}".format(eid), "data_mask_25.nrrd")
...
def test_filter_experiments(mcc, experiments):
    ...
    # test a bunch
    conditions = dict(
        injection_structure_ids=[184, 98],
        age=["10 wKS", "12 wks"],
        gender=["maLE"],
