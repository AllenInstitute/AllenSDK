...
from allensdk.brain_observatory.vbn_2022.input_json_writer.utils import vbn_nwb_config_from_ecephys_session_id_list
...
def running_speed():
    ...
    return RunningSpeed(timestamps=[1.0, 2.0, 3.0], values=[4, 5, 6])


@pytest.fixture
def nwbfile():
    return pynwb.NWBFile(session_description="asession", identifier="afile", session_start_time=datetime.now())
...
def data_object_roundtrip_fixture(tmp_path):
    def f(nwbfile, data_object_cls, **data_object_cls_kwargs):
        ...
        with pynwb.NWBHDF5IO(str(nwb_path), "r") as read_io:
            ...
            data_object_instance = data_object_cls.from_nwb(roundtripped_nwbfile, **data_object_cls_kwargs)
...
def behavior_ecephys_session_config_fixture():
    ...
    session_data_list = vbn_nwb_config_from_ecephys_session_id_list(
        ecephys_session_id_list=[1111216934], probes_to_skip=None
    )
