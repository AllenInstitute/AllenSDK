...
def create_sample_ellipse_hdf(
    output_file: Path, cr_data: pd.DataFrame, eye_data: pd.DataFrame, pupil_data: pd.DataFrame
):
...
def ellipse_fits_fixture(tmp_path, request) -> dict:
    ...
    if request.param["create_good_fits_file"]:
        ...
        pupil = {"center_x": [300], "center_y": [300], "width": [30], "height": [25], "phi": [0]}
    ...
    return {"cr": pd.DataFrame(cr), "eye": pd.DataFrame(eye), "pupil": pd.DataFrame(pupil), "file_path": test_path}


@pytest.mark.parametrize(
    "ellipse_fits_fixture, expect_good_file",
    [({"create_good_fits_file": True}, True), ({"create_good_fits_file": False}, False)],
    indirect=["ellipse_fits_fixture"],
)
def test_load_ellipse_fit_params(ellipse_fits_fixture: dict, expect_good_file: bool):
    expected = {
        "cr_params": pd.DataFrame(ellipse_fits_fixture["cr"]).astype(float),
...
def test_preprocess_input_args(monkeypatch, input_args: dict, expected: dict):
    ...
    monkeypatch.setattr(main, "load_ellipse_fit_params", mock_load_ellipse_fit_params)
...
@pytest.mark.parametrize(
    "pupil_params_rows, expected, expect_fail", [(5, pd.Series([1, 2, 3, 4, 5]), False), (4, None, True)]
)
