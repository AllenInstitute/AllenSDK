...
@pytest.mark.parametrize(
    "version_only_nwbfile_fixture, min_desired_version, warns, warn_msg, invalid_nwb",
    [
...
def test_check_nwbfile_version(version_only_nwbfile_fixture, min_desired_version, warns, warn_msg, invalid_nwb):
    with warnings.catch_warnings(record=True) as w:
        warnings.simplefilter("always")

        check_nwbfile_version(
            nwbfile_path=version_only_nwbfile_fixture, desired_minimum_version=min_desired_version, warning_msg=warn_msg
        )
    if warns:
        if invalid_nwb:
            assert "neither a 'nwb_version' field nor dataset could be found" in str(w[-1].message)
        else:
