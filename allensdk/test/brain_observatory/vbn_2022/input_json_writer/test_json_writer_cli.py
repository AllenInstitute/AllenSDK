...
from allensdk.brain_observatory.ecephys.write_nwb.vbn._schemas import VBNInputSchema


@pytest.mark.requires_bamboo
def test_writer_cli(tmp_path_factory, helper_functions):
    """
    ...
    for session_id in session_list:
        for suffix in "BDF":
            probes_to_skip.append({"session": session_id, "probe": f"probe{suffix}"})
    ...
    helper_functions.windows_safe_cleanup_dir(dir_path=base_dir)
