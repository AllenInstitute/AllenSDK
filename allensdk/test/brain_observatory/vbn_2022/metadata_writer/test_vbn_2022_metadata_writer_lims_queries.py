...
from allensdk.brain_observatory.vbn_2022.metadata_writer.lims_queries import _merge_ecephys_id_and_failed
from allensdk.internal.brain_observatory.mouse import Mouse
from allensdk.internal.brain_observatory.util.multi_session_utils import remove_invalid_sessions
...
class TestLimsQueries(LimsTest):
    ...
    def test_exclude_deceased_mice(self):
        ...
        behavior_session_id = BehaviorSessionId.from_ecephys_session_id(
            db=self.dbconn, ecephys_session_id=ecephys_session_id
        )
        mouse = Mouse.from_behavior_session_id(behavior_session_id=behavior_session_id.value)
        obtained = mouse.get_behavior_sessions(exclude_invalid_sessions=True)
        obtained_all = mouse.get_behavior_sessions(exclude_invalid_sessions=False)

        assert all([x.date_of_acquisition <= x.subject_metadata.get_death_date() for x in obtained])
        assert len([x.subject_metadata.get_death_date() > x.date_of_acquisition for x in obtained_all]) >= 1
...
def test_filter_on_death_date():
    ...
    for is_valid in (True, False):
        sessions += [
            BehaviorMetadata(
                date_of_acquisition=DateOfAcquisition(date_of_acquisition=datetime.datetime(2020, 6, 7)),
                ...
                subject_metadata=SubjectMetadata(
                    ...
                    reporter_line=None,
                    sex=None,
                    death_on=(datetime.datetime(2020, 6, 8) if is_valid else datetime.datetime(2020, 6, 6)),
                ),
            )
            for i in range(2)
        ]
    ...
    expected = [sessions[0], sessions[1]]
...
def test_merge_ecephys_id_and_failed():
    ...
    ecephys_data = [
        {"ecephys_session_id": 4, "donor_id": "100"},
        {"ecephys_session_id": 1, "donor_id": "200"},
        {"ecephys_session_id": 3, "donor_id": "300"},
        {"ecephys_session_id": 2, "donor_id": "400"},
    ]

    failed_data = [
        {"ecephys_session_id": 7, "donor_id": "300"},
        {"ecphys_session_id": 6, "donor_id": "900"},
        {"ecephys_session_id": 5, "donor_id": "200"},
    ]
    ...
    class DummyConnection(object):
        def select(self, query=None):
            if "5" in query:
                ...
                raise RuntimeError(f"cannot mock query={query}")
