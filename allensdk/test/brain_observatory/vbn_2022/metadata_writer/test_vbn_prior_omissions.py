...
from allensdk.brain_observatory.vbn_2022.metadata_writer.dataframe_manipulations import _add_prior_omissions
...
@pytest.fixture
def session_dfs_fixture(session_list_fixture):
    """
    ...
    input_behavior_df = expected_behavior_df.copy().drop(axis="columns", columns=["prior_exposures_to_omissions"])
    input_ecephys_df = expected_ecephys_df.copy().drop(axis="columns", columns=["prior_exposures_to_omissions"])

    return {
        "behavior": input_behavior_df,
...
def test_vbn_add_prior_omissions(session_dfs_fixture):
    """
    ...
    result = _add_prior_omissions(behavior_sessions_df=behavior_df, ecephys_sessions_df=ecephys_df)
    ...
    pd.testing.assert_frame_equal(behavior_df, session_dfs_fixture["expected_behavior"], check_dtype=False)

    pd.testing.assert_frame_equal(ecephys_df, session_dfs_fixture["expected_ecephys"], check_dtype=False)


def test_vbn_date_off_warning(session_dfs_fixture):
    """
    ...
    in_beh = set(
        [beh for beh in behavior_sessions.loc[behavior_sessions.behavior_session_id.notnull()].behavior_session_id]
    )
    ...
    beh_date = behavior_sessions.loc[behavior_sessions.behavior_session_id == chosen_id]
    ece_date = ecephys_sessions.loc[ecephys_sessions.behavior_session_id == chosen_id]
    ...
    behavior_sessions.loc[behavior_sessions.behavior_session_id == chosen_id, "date_of_acquisition"] = (
        datetime.datetime.fromtimestamp(99.0)
    )
    ...
    with pytest.warns(UserWarning, match="disagree on the date of behavior session"):
        _add_prior_omissions(behavior_sessions_df=behavior_sessions, ecephys_sessions_df=ecephys_sessions)


def test_vbn_session_type_off_error(session_dfs_fixture):
    """
    ...
    in_beh = set(
        [
            beh
            for beh in behavior_sessions.loc[
                behavior_sessions.session_type.notnull() & behavior_sessions.behavior_session_id.notnull()
            ].behavior_session_id
        ]
    )
    ...
    beh_type = behavior_sessions.loc[behavior_sessions.behavior_session_id == chosen_id]
    ece_type = ecephys_sessions.loc[ecephys_sessions.behavior_session_id == chosen_id]
    ...
    # disagreement shows up
    behavior_sessions.loc[behavior_sessions.behavior_session_id == chosen_id, "session_type"] = (
        "something_new_and_bizarre"
    )
    ...
    with pytest.raises(RuntimeError, match="disagree on the session type"):
        _add_prior_omissions(behavior_sessions_df=behavior_sessions, ecephys_sessions_df=ecephys_sessions)
