...
@pytest.mark.parametrize("on_missing_file, with_supplement", [("skip", False), ("warn", False), ("warn", True)])
def test_metadata_writer_smoketest(
    smoketest_config_fixture, tmp_path_factory, helper_functions, on_missing_file, with_supplement
):
    """
    ...
    output_names = ("units.csv", "probes.csv", "channels.csv", "ecephys_sessions.csv", "behavior_sessions.csv")

    config = copy.deepcopy(smoketest_config_fixture)

    output_dir = tmp_path_factory.mktemp("vbn_metadata_smoketest")
    output_dir = pathlib.Path(output_dir)
    output_json_path = pathlib.Path(tempfile.mkstemp(dir=output_dir, suffix=".json")[1])
    config["output_dir"] = str(output_dir.resolve().absolute())

    if with_supplement:
        # add suplemental columns to configuration
        supplement = [
            {"ecephys_session_id": 1115077618, "supplementA": "cat", "supplementB": 5},
            {"ecephys_session_id": 1081429294, "supplementA": "frog", "supplementB": 6},
            {"ecephys_session_id": 11111, "supplementA": None, "supplementB": None},
        ]
    ...
    if with_supplement:
        ...
        for expected in supplement[:2]:
            this_row = df.loc[df.ecephys_session_id == expected["ecephys_session_id"]]
    ...
    helper_functions.windows_safe_cleanup_dir(dir_path=output_dir)


@pytest.mark.requires_bamboo
def test_with_failed_sessions(smoketest_with_failed_sessions_config_fixture, tmp_path_factory, helper_functions):
    """
    ...
    for fname in ("behavior_sessions.csv", "ecephys_sessions.csv", "channels.csv", "units.csv", "probes.csv"):
    ...
    for bad_session_id in config["failed_ecephys_session_id_list"]:
        assert bad_session_id not in ecephys_sessions_df.ecephys_session_id.values

    for good_session_id in config["ecephys_session_id_list"]:
        assert good_session_id in ecephys_sessions_df.ecephys_session_id.values
