...
def test_BinaryIntervalSearchTree():
    bist = si.BinaryIntervalSearchTree([(0, 0.9, "A"), (1, 1.9, "B"), (3, 3.9, "D"), (2, 2.9, "C")])
...
@pytest.mark.skipif(not os.path.exists("/projects/neuralcoding"), reason="test NWB file not available")
...
def test_get_spatio_temporal_grating():
    for t, test_val in zip([0, 0.5, 1], [2, 0, 2]):
        data = si.get_spatio_temporal_grating(
            t,
            height=100,
            aspect_ratio=2,
            ori=45,
    ...
    data = si.get_spatio_temporal_grating(
        0, height=100, aspect_ratio=2, ori=45, pix_per_cycle=20, phase=0, p2p_amp=2, baseline=1, temporal_frequency=1
    )
...
def test_map_template_monitor():
    np.testing.assert_almost_equal(
        np.array((500, 250)), si.map_template_coordinate_to_monitor_coordinate((20, 20), (1000, 500), (40, 40))
    )

    np.testing.assert_almost_equal(
        np.array((20, 20)), si.map_monitor_coordinate_to_template_coordinate((500, 250), (1000, 500), (40, 40))
    )
...
def test_lsn_monitor():
    ...
    np.testing.assert_almost_equal(
        np.array(lsn4_monitor_coordinate),
        si.map_stimulus_coordinate_to_monitor_coordinate(
            lsn4_template_coordinate, si.MONITOR_DIMENSIONS, si.LOCALLY_SPARSE_NOISE_4DEG
        ),
    )
    ...
    np.testing.assert_almost_equal(
        np.array(lsn4_monitor_coordinate),
        si.map_stimulus_coordinate_to_monitor_coordinate(
            lsn4_template_coordinate, si.MONITOR_DIMENSIONS, si.LOCALLY_SPARSE_NOISE_8DEG
        ),
    )
    ...
    lsn4_monitor_coordinate = (240, 330)
    np.testing.assert_almost_equal(
        np.array(lsn4_template_coordinate),
        si.monitor_coordinate_to_lsn_coordinate(
            lsn4_monitor_coordinate, si.MONITOR_DIMENSIONS, si.LOCALLY_SPARSE_NOISE_4DEG
        ),
    )
    ...
    np.testing.assert_almost_equal(
        np.array(lsn4_template_coordinate),
        si.monitor_coordinate_to_lsn_coordinate(
            lsn4_monitor_coordinate, si.MONITOR_DIMENSIONS, si.LOCALLY_SPARSE_NOISE_8DEG
        ),
    )
...
def test_natural_scene_monitor():
    ...
    np.testing.assert_almost_equal(
        np.array(monitor_coordinate),
        si.natural_scene_coordinate_to_monitor_coordinate(template_coordinate, si.MONITOR_DIMENSIONS),
    )
    ...
    np.testing.assert_almost_equal(
        ...
        si.map_monitor_coordinate_to_stimulus_coordinate(monitor_coordinate, si.MONITOR_DIMENSIONS, si.NATURAL_SCENES),
    )
...
def test_natural_movie_monitor():
    ...
    np.testing.assert_almost_equal(
        np.array(monitor_coordinate),
        si.natural_movie_coordinate_to_monitor_coordinate(template_coordinate, si.MONITOR_DIMENSIONS),
    )
    ...
    np.testing.assert_almost_equal(
        np.array(template_coordinate),
        si.map_monitor_coordinate_to_stimulus_coordinate(
            monitor_coordinate, si.MONITOR_DIMENSIONS, si.NATURAL_MOVIE_ONE
        ),
    )
...
def test_bijective_all_stimuli():
    for stimulus in si.all_stimuli():
        ...
        monitor_coordinate = si.map_stimulus_coordinate_to_monitor_coordinate(
            template_coordinate, si.MONITOR_DIMENSIONS, stimulus
        )

        new_template_coordinate = si.map_monitor_coordinate_to_stimulus_coordinate(
            monitor_coordinate, si.MONITOR_DIMENSIONS, stimulus
        )
...
def test_monitor_basic_spatial_unit():
    ...
    np.testing.assert_almost_equal(1.0 / m.aspect_ratio, 3.0 / 4)
...
def test_spatial_frequency_to_pix_per_cycle():
    ...
    x1 = m.spatial_frequency_to_pix_per_cycle(0.1, 15.0)
    x2 = m.spatial_frequency_to_pix_per_cycle(0.05, 15.0)
...
def test_map_stimulus():
    ...
    test_list = [
        (0, 0),
        (-5.333333333333333, -7.333333333333333),
        (-5.333333333333333, -7.333333333333333),
        (-2.6666666666666665, -3.6666666666666665),
        (-16.88888888888889, 0.0),
        (-16.88888888888889, 0.0),
        (-16.88888888888889, 0.0),
        (-141.0, -373.0),
        (0, 0),
        ...
        (60.0, 0.0),
        (141.0, 373.0),
        (-2.2, 0.9555555555555556),
        (-2.2, 0.9555555555555556),
        (-1.1, 0.4777777777777778),
        (22.8, 118.11666666666666),
        (22.8, 118.11666666666666),
        (22.8, 118.11666666666666),
        (0.0, 0.0),
        (141.0, 373.0),
    ...
    for source_stimulus in sorted(si.all_stimuli()):
        for target_stimulus in sorted(si.all_stimuli()):
            ...
            np.testing.assert_array_almost_equal(
                m.map_stimulus(tmp, target_stimulus, source_stimulus), np.array([0, 0])
            )
