...
def nm1a(nwb_a, analysis_a):
    return NaturalMovie.from_analysis_file(BODS(nwb_a), analysis_a,
...
def nm1b(nwb_b, analysis_b):
    return NaturalMovie.from_analysis_file(BODS(nwb_b), analysis_b,
                                           si.NATURAL_MOVIE_ONE)
...
def lsn(nwb_c, analysis_c):
    # in order to work around 2/3 unicode compatibility, separate files are
    # specified for python 2 and 3
    # we need to look up a different key depending on python version
    key = si.LOCALLY_SPARSE_NOISE_4DEG if sys.version_info < (3,) else \
...
def nm1c(nwb_c, analysis_c):
    return NaturalMovie.from_analysis_file(BODS(nwb_c), analysis_c,
                                           si.NATURAL_MOVIE_ONE)
...
def test_drifting_gratings(dg, nwb_a, analysis_a_new):
    ...
    assert np.allclose(dg.signal_correlation, dg_new.signal_correlation,
                       equal_nan=True)
    assert np.allclose(dg.representational_similarity,
                       dg_new.representational_similarity, equal_nan=True)


@pytest.mark.nightly
def test_natural_movie_one_a(nm1a, nwb_a, analysis_a_new):
    nm1a_new = NaturalMovie.from_analysis_file(BODS(nwb_a), analysis_a_new,
                                               si.NATURAL_MOVIE_ONE)
...
def test_natural_movie_three(nm3, nwb_a, analysis_a_new):
    # nm3_new = NaturalMovie.from_analysis_file(BODS(nwb_a), analysis_a_new,
    # si.NATURAL_MOVIE_THREE)
...
def test_static_gratings(sg, nwb_b, analysis_b_new):
    ...
    assert np.allclose(sg.signal_correlation, sg_new.signal_correlation,
                       equal_nan=True)
    assert np.allclose(sg.representational_similarity,
                       sg_new.representational_similarity, equal_nan=True)


@pytest.mark.nightly
def test_natural_movie_one_b(nm1b, nwb_b, analysis_b_new):
    nm1b_new = NaturalMovie.from_analysis_file(BODS(nwb_b), analysis_b_new,
                                               si.NATURAL_MOVIE_ONE)
...
def test_natural_scenes(ns, nwb_b, analysis_b_new):
    ...
    assert np.allclose(ns.signal_correlation, ns_new.signal_correlation,
                       equal_nan=True)
    assert np.allclose(ns.representational_similarity,
                       ns_new.representational_similarity, equal_nan=True)
...
def test_locally_sparse_noise(lsn, nwb_c, analysis_c_new):
    ...
    if session_type == si.THREE_SESSION_C:
        lsn_new = LocallySparseNoise.from_analysis_file(
            ds, analysis_c_new,
            si.LOCALLY_SPARSE_NOISE)
