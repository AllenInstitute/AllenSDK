...
def test_drifting_gratings(dg, nwb_a, analysis_a_new):
    ...
    assert np.allclose(dg.response, dg_new.response, equal_nan=True, atol=1e-4, rtol=1e-4)
...
def test_static_gratings(sg, nwb_b, analysis_b_new):
    ...
    assert np.allclose(sg.response, sg_new.response, equal_nan=True, atol=1e-4, rtol=1e-4)
