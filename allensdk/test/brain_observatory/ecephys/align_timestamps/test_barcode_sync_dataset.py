...
@pytest.mark.parametrize('line_labels,expected', [
    [['barcode'], 0],
    [['barcodes'], 0],
    [[], None]
])
...
@pytest.mark.parametrize('sample_frequency,rising_edges,falling_edges,times_exp,codes_exp,table', [
...
def test_extract_barcodes(sample_frequency, rising_edges, falling_edges, times_exp, codes_exp, table):
    ...
    with mock.patch('allensdk.brain_observatory.sync_dataset.Dataset.get_rising_edges', return_value=rising_edges):
        with mock.patch('allensdk.brain_observatory.sync_dataset.Dataset.get_falling_edges', return_value=falling_edges):
