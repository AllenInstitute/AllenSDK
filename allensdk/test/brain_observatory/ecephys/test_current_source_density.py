...
def test_extract_trial_windows(stim_table, stim_index):
    ...
    expected = [[0.8, 0.9, 1.0, 1.1, 1.2], [1.8, 1.9, 2.0, 2.1, 2.2]]
...
@pytest.mark.parametrize(
    "times,raw,channels,windows,volts_per_bit,expected",
    [
        [
            np.arange(10),
...
def test_accumulate_lfp_data(times, raw, channels, windows, volts_per_bit, expected):
    obtained = csd.accumulate_lfp_data(times, raw, channels, windows, volts_per_bit)
    assert np.allclose(obtained, expected)


@pytest.mark.parametrize(
    "trial_mean_accumulated,spacing,expected,expected_channels",
    [
        [
            np.nanmean(np.arange(36).reshape([2, 6, 3]) ** 3, axis=0),
            1.0,
            [
                [1728.0, 1926.0, 2142.0],
                [648.0, 702.0, 756.0],
                [810.0, 864.0, 918.0],
                [972.0, 1026.0, 1080.0],
                [1134.0, 1188.0, 1242.0],
                [-5292.0, -5706.0, -6138.0],
            ],
            np.arange(6),
        ]
    ],
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [2, 6, [[0, 20], [32, 20], [16, 40], [48, 40]]],
        [0, 8, [[16, 0], [48, 0], [0, 20], [32, 20], [16, 40], [48, 40], [0, 60], [32, 60]]],
        [4, 8, [[16, 40], [48, 40], [0, 60], [32, 60]]],
        [5, 6, [[48, 40]]],
    ],
)
def test_make_actual_channel_locations(min_chan, max_chan, expected):
    obtained = interp_utils.make_actual_channel_locations(min_chan=min_chan, max_chan=max_chan)
    assert np.allclose(obtained, expected)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [2, 6, [[24, 20], [24, 30], [24, 40], [24, 50]]],
        [7, 14, [[24, 70], [24, 80], [24, 90], [24, 100], [24, 110], [24, 120], [24, 130]]],
        [8, 9, [[24, 80]]],
    ],
)
def test_make_interp_channel_locations(min_chan, max_chan, expected):
    obtained = interp_utils.make_interp_channel_locations(min_chan=min_chan, max_chan=max_chan)
    assert np.allclose(obtained, expected)


@pytest.mark.parametrize(
    "lfp, actual_locs, interp_locs, expected",
    [
        [
            # lfp
            ...
            (
                [
                    [
                        [-1.48688877e01, -1.65987508e00, 2.25788198e01],
                        [1.84977651e02, 3.01621496e02, 4.52968522e02],
                        [5.82039685e02, 8.18476063e02, 1.11005335e03],
                        [1.23712914e03, 1.61285986e03, 2.05929375e03],
                        [2.03821497e03, 2.56276850e03, 3.17035171e03],
                        [0.00000000e00, 0.00000000e00, 0.00000000e00],
                    ],
                    [
                        [6.80643377e03, 7.93617702e03, 9.18494994e03],
                        [1.24901530e04, 1.41494541e04, 1.59514583e04],
                        [1.81704531e04, 2.03174258e04, 2.26275394e04],
                        [2.37138688e04, 2.62802568e04, 2.90253479e04],
                        [2.90797202e04, 3.20168080e04, 3.51449255e04],
                        [0.00000000e00, 0.00000000e00, 0.00000000e00],
                    ],
                ],
                0.01,
...
def test_interp_channel_locs(lfp, actual_locs, interp_locs, expected):
    obtained = interp_utils.interp_channel_locs(lfp=lfp, actual_locs=actual_locs, interp_locs=interp_locs)
...
# ------------ _filter_utils.py ------------
@pytest.mark.parametrize(
    "lfp, ref_channels, noisy_thresh, expected",
    [
        [
            ...
            np.array(
                [
                    [[0.1, 0.1, 0.1, 0.1], [0, 50, 500, 5000], [0, 0, 0, 0], [0.3, 0.3, 0.3, 0.3]],
...
def test_select_good_channels(lfp, ref_channels, noisy_thresh, expected):
    obtained = filt_utils.select_good_channels(lfp, ref_channels, noisy_thresh)
...
@pytest.mark.parametrize(
    "lfp, sampling_rate, filter_cuts, filter_order, expected",
    [
        [
            # lfp
...
def test_filter_lfp_channels(lfp, sampling_rate, filter_cuts, filter_order, expected):
    obtained = filt_utils.filter_lfp_channels(lfp, sampling_rate, filter_cuts, filter_order)
    assert np.allclose(obtained, expected)
