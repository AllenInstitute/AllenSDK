...
from allensdk.brain_observatory.ecephys.copy_utility._schemas import SessionUploadInputSchema, SessionUploadOutputSchema
...
def test_dst_dir_exists(already_exists, tmp_path, monkeypatch):
    ...
    args = {
        "files": [{"source": str(src_file), "destination": str(dst_file), "key": "something"}],
        "output_json": str(outj_path),
    }

    parser = argschema.ArgSchemaParser(
        args, schema_type=SessionUploadInputSchema, output_schema_type=SessionUploadOutputSchema, args=[]
    )
...
def test_SessionUploadSchema(tmpdir):
    ...
    test_data = {
        "files": [{"source": str(src_file), "destination": str(dst_file), "key": ""}],
        "output_json": str(output_json),
    }

    parser = argschema.ArgSchemaParser(
        test_data, schema_type=SessionUploadInputSchema, output_schema_type=SessionUploadOutputSchema, args=[]
    )
