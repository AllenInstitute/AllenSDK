...
def test_assign_sweep_values():
    ...
    stim_table = pd.DataFrame([
        {'Start': 0, 'End': 1, 'orientation': np.nan, 'color': np.nan, 'sweep_number': 2},
        {'Start': 1, 'End': 2, 'orientation': np.nan, 'color': np.nan, 'sweep_number': 0},
        {'Start': 2, 'End': 3, 'orientation': np.nan, 'color': np.nan, 'sweep_number': 1},
        {'Start': 3, 'End': 4, 'orientation': np.nan, 'color': np.nan, 'sweep_number': 3},
...
@pytest.mark.parametrize('table,column,new_columns,drop,expected', [
    [ 
...
@pytest.mark.parametrize('sweeps,disp_seq,expected', [
    [
    ...
    [
        ...
        [ [2, 4], [16, 36] ],
        {'Start': [3, 17, 29, 32], 'End': [5, 18, 32, 34], 'stimulus_block': [0, 1, 1, 1]}
    ]
])
def test_apply_display_sequence(sweeps, disp_seq, expected):
    ...
    pd.testing.assert_frame_equal(obt_table, expected_table, check_like=True, check_column_type=False)


@pytest.mark.parametrize('stimulus_tables,expected', [
    [ 
        [
...
def test_make_spontaneous_activity_tables(stimulus_tables, expected):
    ...
    if len(obtained) == 1:
        pd.testing.assert_frame_equal( obtained[0], expected[0], check_like=True )
    else:
        assert len(obtained) == len(expected)


# TODO: this test is really weird
@pytest.mark.parametrize('stimuli,stim_tabler,spon_tabler,sort_key,expected', [
    [
        [[1, 5], [3, 4]],
        lambda stimulus: [pd.DataFrame({'parameter': stimulus})],
        lambda stimuli: [pd.DataFrame({'parameter': [len(stimuli)]})],
        'parameter',
        pd.DataFrame({'parameter': [1, 2, 3, 4, 5], 'stimulus_block': [0, None, 1, 1, 0], 'stimulus_index': [0, None, 1, 1, 0]}),
    ]
])
def test_create_stim_table(stimuli, stim_tabler, spon_tabler, sort_key, expected):
    ...
    pd.testing.assert_frame_equal( obtained, expected, check_like=True, check_dtype=False )


@pytest.mark.parametrize('stim_table,frame_times,fps,eft,map_cols,expected', [
    [
        ...
        True,
        ('Start', 'End'),
        pd.DataFrame({'Start': [50, 25, 12.5, 6.25], 'End': [25, 12.5, 6.25, 6.35], 'data': [-1, -2, -3, -4]})
    ]
])
...
@pytest.mark.parametrize('stimulus,stf,start_key,end_key,expected', [
    [
        stimulus_psuedofixture_0(),
        lambda x: np.array(x),
        'Start',
        'End',
        [pd.DataFrame({'Start': [510, 520], 'End': [521, 526], 'a': [-3, 1], 'b': [-2, 2], 'c': [-1, 3], 'stimulus_block': [0, 0], 'stimulus_name': ['gabor_20_deg_250ms'] * 2 })]
    ],
    [
        ...
        'Start',
        'End',
        [pd.DataFrame({'Start': [510, 520], 'End': [521, 526], 'Image': [1, 0], 'stimulus_block': [0, 0], 'stimulus_name': ['gabor_20_deg_250ms'] * 2 })]
    ]
])
def test_build_stimuluswise_table(stimulus, stf, start_key, end_key, expected):
    ...
    for obtained_table, expected_table in zip(obtained, expected):
        pd.testing.assert_frame_equal( obtained_table, expected_table, check_like=True )
