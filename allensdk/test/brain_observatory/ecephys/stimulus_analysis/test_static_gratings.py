...
def test_stimulus(ecephys_api):
    ...
    assert set(sg.stim_table.columns).issuperset(
        {"spatial_frequency", "orientation", "phase", "start_time", "stop_time"}
    )
...
@pytest.mark.parametrize(
    "sf_tuning_responses,mean_sweeps_trials,expected",
    [
        (
...
@pytest.mark.parametrize(
    "sf_tuning_response,sf_vals,pref_sf_index,expected",
    [
        (
            np.array([2.69565217, 3.91836735, 2.36734694, 1.52, 2.21276596]),
            [0.02, 0.04, 0.08, 0.16, 0.32],
            1,
            (0.22704947240176027, 0.0234087755414, np.nan, np.nan),
        ),
        (
            np.array([1.14285714, 0.73469388, 7.44, 13.6, 11.6]),
            [0.02, 0.04, 0.08, 0.16, 0.32],
            3,
        ...
        (
            ...
            [0.02, 0.04, 0.08, 0.16, 0.32],
            0,
            (0.0, 0.019999999552965164, np.nan, 0.32),
        ),
    ],
)
