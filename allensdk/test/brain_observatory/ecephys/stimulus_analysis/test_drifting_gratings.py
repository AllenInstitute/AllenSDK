...
class MockDGSessionApi(MockSessionApi):
    ...
    def get_stimulus_presentations(self):
        ...
        if self._with_dg_contrast:
            features = np.array(
                np.meshgrid(
                    [0.0, 45.0, 90.0, 135.0],  # ORI
                    [0.01, 0.02, 0.04, 0.08, 0.13, 0.2, 0.35, 0.6, 1.0],
                )  # contrast
            ).reshape(2, 36)
...
def test_stimulus(ecephys_api):
    ...
    assert set(dg.stim_table.columns).issuperset(
        {"temporal_frequency", "orientation", "contrast", "start_time", "stop_time"}
    )
...
def test_metrics(ecephys_api):
    ...
    assert np.allclose(
        dg.metrics["f1_f0_dg"].loc[[0, 1, 2, 3, 4, 5]],
        [0.001572, np.nan, 1.999778, np.nan, 1.560436, 1.999978],
        equal_nan=True,
        atol=1.0e-06,
    )
...
def test_metric_with_contrast(ecephys_api_w_contrast):
    ...
    assert np.allclose(dg.metrics["c50_dg"].loc[[5]], 0.17585882, atol=0.5, rtol=1.0)


@pytest.mark.parametrize(
    "response,tf,sampling_rate,expected",
    [
...
@pytest.mark.parametrize(
    "contrast_vals,responses,expected",
    [
        (np.array([0.01, 0.02, 0.04, 0.08, 0.13, 0.2, 0.35, 0.6, 1.0]), np.array([]), np.nan),
        # invalid input
        (np.array([0.01, 0.02, 0.04, 0.08, 0.13, 0.2, 0.35, 0.6, 1.0]), np.full(9, 12.0), 0.0090),
        # flat non-zero curve
        (np.array([0.01, 0.02, 0.04, 0.08, 0.13, 0.2, 0.35, 0.6, 1.0]), np.zeros(9), None),
        ...
        (
            np.array([0.01, 0.02, 0.04, 0.08, 0.13, 0.2, 0.35, 0.6, 1.0]),
...
@pytest.mark.parametrize(
    "data_arr,tf,trial_duration,expected",
    [
        ...
        (np.array([[0, 0, 1, 1, 2, 0, 5, 1]]), 2.0, 1.0, 0.894427190999916),
