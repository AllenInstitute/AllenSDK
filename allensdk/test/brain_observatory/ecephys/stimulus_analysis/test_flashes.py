...
def test_metrics(ecephys_api):
    ...
    assert np.allclose(
        fl.metrics["on_off_ratio_fl"].loc[[0, 1, 2, 3, 4, 5]], [0.0, np.nan, 0.0, np.nan, 3.0, 2.0], equal_nan=True
    )  # Check _get_on_off_ratio() method

    assert "sustained_idx_fl" in fl.metrics.columns
    assert np.allclose(
        fl.metrics["sustained_idx_fl"].loc[[0, 1, 2, 3, 4, 5]].values,
        [0.00401606, np.nan, 0.01204819, np.nan, 0.02811245, 0.00401606],
        equal_nan=True,
    )
