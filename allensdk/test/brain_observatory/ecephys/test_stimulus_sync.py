...
@pytest.mark.parametrize(
    ...
    [
        [  # super basic, no vsyncs, no bad frames
            np.linspace(5, 30.0, 11),
            0.25,
            100,
            10,
            None,
            [np.arange(5, 30, 0.25), np.arange(5.25, 30.25, 0.25)],
        ],
        [  # also no bad frames
            np.array([5, 5.75, 6.5, 7.25]),
            0.25,
            9,
            3,
            None,
        ...
        [  # classic extra frame case
            ...
            [
                ...
                np.array([5.25, 5.50, 5.75, 6.00, 6.25, 6.50, 6.75, 7.25, 7.50, 7.75, 8.0, 8.25]),
            ],
        ],
        [  # long-short, using vsyncs
            np.array([5, 5.75, 6.5, 7.50, 8.0]),
            0.25,
            12,
            3,
...
@pytest.mark.parametrize(
    "process,pctiles", [[partial(np.random.rand, 1000), (5, 95)], [partial(np.random.rand, 1000), (45, 55)]]
)
def test_trimmed_stats(process, pctiles):
    ...
    data = np.concatenate([data, np.zeros(num_missing_lower) - 1000, np.zeros(num_missing_upper) + 1000])
...
@pytest.mark.parametrize("pd_times,vs_times, expected", [[[1, 2, 3, 4, 5], [1.8, 3, 4], [2, 3, 4]]])
...
@pytest.mark.parametrize(
    "pd_times,ndevs,cycle,max_offset,expected",
    [
...
@pytest.mark.parametrize("pd_times,cycle,expected", [[[0, 1, 2, 3, 4, 5.1, 6, 7, 8], 1, 1]])
...
@pytest.mark.parametrize(
    "ends,frame_duration,irregularity,expected",
