...
@pytest.mark.parametrize(
    "method_name,kwargs,response,checks,expected",
    [
        [
            "get_units",
            {},
            ...
            pd.DataFrame({"something": [12, 14]}, index=pd.Index(name="id", data=[5, 6])),
        ],
        [
            "get_units",
            {"session_ids": [1, 2, 3]},
        ...
        [
            ...
            pd.DataFrame({"id": [5, 6], "something": [12, 14]}),
            {
                "filters_channels": lambda st: re.compile(r".+and ec.id in \(1,2,3\).*", re.DOTALL).match(st)
                is not None,
        ...
        [
            ...
            pd.DataFrame({"id": [5, 6], "something": [12, 14]}),
            {
                "checks_pa_not_null": lambda st: re.compile(r".+and es.published_at is not null.*", re.DOTALL).match(st)
                is not None,
                "checks_pa": lambda st: re.compile(r".+and es.published_at <= '2019-10-22'.*", re.DOTALL).match(st)
                is not None,
            },
            pd.DataFrame({"something": [12, 14]}, index=pd.Index(name="id", data=[5, 6])),
        ],
        [
            ...
            pd.DataFrame({"id": [5, 6], "something": [12, 14]}),
            {
                ...
                "checks_pa": lambda st: re.compile(r".+and es.published_at <= '2019-10-22'.*", re.DOTALL).match(st)
                is not None,
                "filters_sessions": lambda st: re.compile(r".+and es.id in \(1,2,3\).*", re.DOTALL).match(st)
                is not None,
            },
            pd.DataFrame({"something": [12, 14]}, index=pd.Index(name="id", data=[5, 6])),
        ],
        ...
        [
            ...
            pd.DataFrame({"something": [12, 14], "genotype": ["foo", "wt"]}, index=pd.Index(name="id", data=[5, 6])),
        ],
        [
            ...
            {"filters_sessions": lambda st: re.compile(r".+and es.id in \(1,2,3\).*", re.DOTALL).match(st) is not None},
            pd.DataFrame(
                {"id": [5, 6], "a": [1, 3], "b": [2, 4]}, index=pd.Index(name="ecephys_unit_id", data=[10, 11])
            ),
        ],
    ],
)
...
@pytest.mark.parametrize(
    "method,kwargs,query_pattern,pg_engine_cls",
    [
        [
...
def test_file_getter(method, kwargs, query_pattern, pg_engine_cls):
    api = epla.EcephysProjectLimsApi(postgres_engine=pg_engine_cls(query_pattern), app_engine=MockHttpEngine())
