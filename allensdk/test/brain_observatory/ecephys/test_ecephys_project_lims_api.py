...
@pytest.mark.parametrize('method,conditions,expected_query', [
    [   
        "get_sessions",
        {"published": True}, 
        (
...
def test_get_session_data():
    ...
    class MockPgEngine:
        def select(self, rendered):
            pattern = re.compile(r".*and ear.ecephys_session_id = (?P<session_id>\d+).*", re.DOTALL)
    ...
    api = epla.EcephysProjectLimsApi(postgres_engine=MockPgEngine(), app_engine=MockHttpEngine())
