...
def test_extract_led_times(key, line_labels, led_vals):
    ...
    with mock.patch("allensdk.brain_observatory.sync_dataset.Dataset.get_all_times", return_value=led_vals):
        with mock.patch(
            "allensdk.brain_observatory.sync_dataset.Dataset.get_bit_changes", return_value=np.ones_like(led_vals)
        ) as q:
...
@pytest.mark.parametrize(
    ...
    [
        [  # expected timing, using vsyncs
            ...
            - 0.0625 * np.random.rand(298),  # num frames is (num_vsyncs - 1) * cycle + 1
            3,
            np.arange(5.0, 5 + (298 * 0.25), 0.25),
        ]
    ],
...
def test_extract_frame_times_from_photodiode(photodiode_times, vsyncs, cycle, expected):
    ...
    dataset = EcephysSyncDataset()
    with mock.patch(
        "allensdk.brain_observatory.ecephys.file_io.ecephys_sync_dataset.EcephysSyncDataset.get_edges",
