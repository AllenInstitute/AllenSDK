...
@pytest.fixture
def raw_spike_times():
    return {0: np.array([5, 6, 7, 8]), 1: np.array([2.5]), 2: np.array([1.01, 1.03, 1.02])}


@pytest.fixture
def raw_mean_waveforms():
    return {0: np.zeros((3, 20)), 1: np.zeros((3, 20)) + 1, 2: np.zeros((3, 20)) + 2}
...
def raw_probes():
    return pd.DataFrame(
        {"description": ["probeA", "probeB"], "location": ["VISp", "VISam"], "sampling_rate": [30000.0, 30000.0]},
...
@pytest.fixture
def just_stim_table_api(raw_stimulus_table):
...
def get_no_spikes_times(self):
    # A special method used for testing cases when there are no spikes for a
    # given session, will be swapped out for get_spike_times()
    return {0: np.array([]), 1: np.array([]), 2: np.array([])}
...
def test_get_stimulus_epochs(just_stim_table_api):
...
def test_build_stimulus_presentations(just_stim_table_api):
    expected_columns = [
        "start_time",
        "stop_time",
        "stimulus_name",
...
def test_presentationwise_spike_counts(spike_times_api):
    ...
    obtained = session.presentationwise_spike_counts(
        np.linspace(-0.1, 0.1, 3), session.stimulus_presentations.index.values, session.units.index.values
    )
...
@pytest.mark.parametrize(
    "spike_times,time_domain,expected",
    [
        [{1: [1.5, 2.5]}, [[1, 2, 3, 4], [1.1, 2.1, 3.1, 4.1]], np.array([[1, 1, 0], [1, 1, 0]])[:, :, None]],
        [{1: [1.5, 2.5]}, [[1, 2, 3, 4], [1.6, 2.0, 4.0, 4.1]], np.array([[1, 1, 0], [0, 1, 0]])[:, :, None]],
        [
            {1: [1.5, 2.5], 2: [1.5, 2.5]},
...
def test_presentationwise_spike_times(spike_times_api):
    session = EcephysSession(api=spike_times_api)
    obtained = session.presentationwise_spike_times(
        session.stimulus_presentations.index.values, session.units.index.values
    )
...
def test_empty_presentationwise_spike_times(spike_times_api):
    # Test that when there are no spikes presentationwise_spike_times
    # doesn't fail and instead returns a empty dataframe
...
def test_empty_conditionwise_spike_statistics(spike_times_api):
    ...
    obtained = session.conditionwise_spike_statistics(
        stimulus_presentation_ids=session.stimulus_presentations.index.values, unit_ids=session.units.index.values
    )
...
def test_get_stimulus_table(detailed, just_stim_table_api, raw_stimulus_table):
    session = EcephysSession(api=just_stim_table_api)
...
def test_filter_owned_df(just_stim_table_api):
    session = EcephysSession(api=just_stim_table_api)
    ids = [0, 2]
...
def test_filter_owned_df_scalar(just_stim_table_api):
    session = EcephysSession(api=just_stim_table_api)
    ids = 3
...
def test_build_inter_presentation_intervals(just_stim_table_api):
    session = EcephysSession(api=just_stim_table_api)
...
def test_get_inter_presentation_intervals_for_stimulus(just_stim_table_api):
    ...
    expected = pd.DataFrame(
        {"interval": [0, 0]},
        index=pd.MultiIndex(
            levels=[[0, 1], [1, 2]], codes=[[0, 1], [0, 1]], names=["from_presentation_id", "to_presentation_id"]
        ),
    )
...
def test_get_lfp_mask_invalid(lfp_masking_api):
    ...
    expected = xr.DataArray(
        data=np.array([[1, 2, 3, np.nan, np.nan], [6, 7, 8, np.nan, np.nan]]),
        dims=["channel", "time"],
        coords=[[2, 1], np.linspace(0, 2, 5)],
    )
    print(expected)
...
@pytest.mark.parametrize("inp,expected", [[[np.nan, np.nan, 4, 4, 4, 5, 5], [0, 2, 5, 7]]])
def test_nan_intervals(inp, expected):
    assert np.allclose(expected, nan_intervals(inp))
