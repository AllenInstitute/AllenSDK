...
def probes_config_fixture(behavior_ecephys_session_config_fixture):
    """
    ...
    input_data["probes"] = input_data["probes"][:3]
...
@pytest.fixture(scope="module")
def probes_fixture(probes_config_fixture):
    """
...
def test_read_write_nwb(roundtrip, data_object_roundtrip_fixture, probes_fixture, helper_functions):
    nwbfile = helper_functions.create_blank_nwb_file()
    probes_fixture.to_nwb(nwbfile=nwbfile)

    if roundtrip:
        obt = data_object_roundtrip_fixture(nwbfile=nwbfile, data_object_cls=Probes)
    else:
...
def test_skip_probes(probes_config_fixture):
    ...
    skip_probes = [names[0]]
    probes = Probes.from_json(probes=probes_config_fixture, skip_probes=skip_probes)
    assert sorted([p.name for p in probes]) == sorted([p for p in names if p not in skip_probes])
...
def test_probe_channels_strip_subregion(structure_acronym, strip_structure_subregion):
    ...
    if type(structure_acronym) is str:
        if strip_structure_subregion:
            ...
            expected = "LGd-sh" if structure_acronym == "LGd-sh" else "LGd"
