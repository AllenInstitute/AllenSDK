...
from allensdk.brain_observatory.behavior.data_objects.stimuli.fingerprint_stimulus import FingerprintStimulus
...
@pytest.mark.parametrize("stimulus_blocks, expected", [([0, 2, 3], [1]), ([0, 2, 4], [1, 3]), ([0, 1, 2], [])])
def test_get_spontaneous_block_indices(stimulus_blocks, expected):
    ...
    obtained = get_spontaneous_block_indices(stimulus_blocks=stimulus_blocks)
...
class TestFingerprintStimulus:
    ...
    def setup_class(cls):
                    ...
                    "items": {
                        "fingerprint": {
                            "static_stimulus": {
                                ...
                                "sweep_frames": np.array([(0, 1), (2, 3), (4, 5), (6, 7)]),
                            ...
                            "frame_indices": np.array([5, 6] + list(range(7, 7 + 4 * 2))),
                        }
                    }
                }
            }
        ...
        cls.stimulus_file = BehaviorStimulusFile(filepath=Path(tmpdir.name) / "behavior_stimulus.pkl")
        cls.stimulus_presentations_table = pd.DataFrame({"stimulus_block": [0]})
        cls.stimulus_timestamps = StimulusTimestamps(timestamps=np.arange(0, 20), monitor_delay=0.0)
    ...
    def test_add_fingerprint_stimulus(self):
        ...
        assert obt[obt["stimulus_name"] == "spontaneous"].shape[0] == expected_num_spontaneous_rows
...
class TestStimulusPresentations:
    ...
    def test_get_is_image_novel(self, image_names, expected):
        with patch.object(Mouse, attribute="from_behavior_session_id", wraps=lambda behavior_session_id: Mouse("1")):
            with patch.object(Mouse, attribute="get_images_shown", wraps=lambda up_to_behavior_session_id: {"A", "B"}):
                obt = Presentations._get_is_image_novel(image_names=image_names, behavior_session_id=1)
                assert obt == expected

    def test_add_is_image_novel(self):
        stimulus_presentations = pd.DataFrame({"image_name": ["A", "B", "C", np.nan]})
        is_image_novel = {"A": False, "B": False, "C": True}
        with patch.object(
            Presentations,
            attribute="_get_is_image_novel",
            wraps=lambda image_names, behavior_session_id: is_image_novel,
        ):
            Presentations._add_is_image_novel(stimulus_presentations=stimulus_presentations, behavior_session_id=1)
            assert (
                stimulus_presentations["is_image_novel"].tolist()
