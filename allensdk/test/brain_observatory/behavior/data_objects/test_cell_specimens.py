...
class TestLims:
    ...
    def test_from_lims(self):
        ...
        ots = OphysTimestamps(timestamps=np.linspace(start=0.1, stop=0.1 * number_of_frames, num=number_of_frames))
...
class TestJson:
    ...
    def setup_class(cls):
        ...
        cls.ophys_timestamps = OphysTimestamps(timestamps=np.array([0.1, 0.2, 0.3]))
    ...
    def test_roi_data_same_order_as_cell_specimen_table(self, data):
        ...
        data_cell_roi_ids = getattr(private_attr.value, "cell_roi_id" if data == "events" else "index").values
    ...
    def test_trace_rois_different_than_cell_specimen_table(self, trace_type, extra_in_trace):
        ...
        if extra_in_trace:
            ...
            private_trace_attr._value = private_trace_attr._value[private_trace_attr._value.index != csp_rois.iloc[0]]
...
class TestNWB:
    ...
    def test_read_write_nwb(self, roundtrip, data_object_roundtrip_fixture, exclude_invalid_rois):
        ...
        cell_specimens.to_nwb(nwbfile=self.nwbfile, ophys_timestamps=self.ophys_timestamps)
        ...
        if exclude_invalid_rois:
            cell_specimens._cell_specimen_table = cell_specimens._cell_specimen_table[
                cell_specimens._cell_specimen_table["cell_roi_id"].isin(valid_roi_id)
            ]
...
class TestFilterAndReorder:
    ...
    def test_missing_rois(self, raise_if_rois_missing):
        ...
        if raise_if_rois_missing:
            ...
            rois.filter_and_reorder(roi_ids=roi_ids, raise_if_rois_missing=raise_if_rois_missing)
