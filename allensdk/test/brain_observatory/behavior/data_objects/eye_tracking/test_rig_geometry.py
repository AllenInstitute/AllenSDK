...
from allensdk.brain_observatory.behavior.data_objects.eye_tracking.rig_geometry import RigGeometry, Coordinates
...
class TestFromLims(LimsTest):
    ...
    def test_from_lims(self):
        ...
        rg = RigGeometry.from_lims(behavior_session_id=behavior_session_id.value, lims_db=self.dbconn)
    ...
    def test_rig_geometry_newer_than_experiment(self):
        ...
        rg = RigGeometry.from_lims(ophys_experiment_id=ophys_experiment_id, lims_db=self.dbconn)
    ...
    def test_only_single_geometry_returned(self):
        ...
        df = pd.read_pickle(str(test_data_dir / "raw_eye_tracking_rig_geometry.pkl"))
...
class TestFromJson(LimsTest):
    ...
    def test_from_json(self):
        dict_repr = {"eye_tracking_rig_geometry": self.expected.to_dict()["rig_geometry"]}
...
class TestNWB:
    ...
    def test_read_write_nwb(self, roundtrip, data_object_roundtrip_fixture):
        ...
        if roundtrip:
            obt = data_object_roundtrip_fixture(nwbfile=self.nwbfile, data_object_cls=RigGeometry)
        else:
