...
from allensdk.brain_observatory.behavior.data_files.rigid_motion_transform_file import RigidMotionTransformFile
from allensdk.brain_observatory.behavior.data_objects.cell_specimens.cell_specimens import CellSpecimens, EventsParams
from allensdk.brain_observatory.behavior.data_objects.motion_correction import MotionCorrection
...
from allensdk.test.brain_observatory.behavior.data_objects.metadata.test_behavior_ophys_metadata import TestBOM
from allensdk.test.brain_observatory.behavior.data_objects.nwb_input_json import NwbInputJson
...
class TestFromDataFile(LimsTest):
    ...
    def test_from_data_file(self):
        ...
        mc = MotionCorrection.from_data_file(rigid_motion_transform_file=motion_correction_file)
...
class TestJson:
    ...
    def setup_class(cls):
        ...
        dict_repr["rigid_motion_transform_file"] = str(test_data_dir / "rigid_motion_transform_file.csv")
        cls.dict_repr = dict_repr
        cls.motion_correction_file = RigidMotionTransformFile.from_json(dict_repr=dict_repr)
        expected = pd.DataFrame({"x": [2, 3, 2], "y": [-3, -4, -4]})
        cls.expected = MotionCorrection(motion_correction=expected)

    def test_from_json(self):
        mc = MotionCorrection.from_data_file(rigid_motion_transform_file=self.motion_correction_file)
...
class TestNWB:
    ...
    def setup_method(self, method):
        ...
        def _write_cell_specimen():
            ...
            # write cell specimen
            ij = NwbInputJson()
            ophys_timestamps = OphysTimestamps(timestamps=np.array([0.1, 0.2, 0.3]))
            csp = CellSpecimens.from_json(
                dict_repr=ij.dict_repr,
                ophys_timestamps=ophys_timestamps,
                segmentation_mask_image_spacing=(0.78125e-3, 0.78125e-3),
                events_params=EventsParams(filter_scale_seconds=2.0 / 31.0, filter_n_time_steps=20),
            )
    ...
    def test_read_write_nwb(self, roundtrip, data_object_roundtrip_fixture):
        ...
        if roundtrip:
            obt = data_object_roundtrip_fixture(nwbfile=self.nwbfile, data_object_cls=MotionCorrection)
        else:
