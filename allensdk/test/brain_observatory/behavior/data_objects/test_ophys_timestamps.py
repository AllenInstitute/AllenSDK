...
class TestFromSyncFile(LimsTest):
    ...
    def test_from_sync_file(self):
        self.sync_file._data = {"ophys_frames": np.array([0.1, 0.2, 0.3])}
        ts = OphysTimestamps.from_sync_file(sync_file=self.sync_file).validate(number_of_frames=3)
    ...
    def test_too_long_single_plane(self):
        ...
        self.sync_file._data = {"ophys_frames": np.array([0.1, 0.2, 0.3])}
        ts = OphysTimestamps.from_sync_file(sync_file=self.sync_file).validate(number_of_frames=2)
    ...
    def test_too_long_multi_plane(self):
        ...
        with pytest.raises(RuntimeError):
            OphysTimestampsMultiplane.from_sync_file(sync_file=self.sync_file, group_count=2, plane_group=0).validate(
                number_of_frames=1
            )
    ...
    def test_too_short(self):
        ...
        self.sync_file._data = {"ophys_frames": np.array([0.1, 0.2, 0.3])}
        with pytest.raises(RuntimeError):
            OphysTimestamps.from_sync_file(sync_file=self.sync_file).validate(number_of_frames=4)

    def test_multiplane(self):
        """test timestamps properly extracted when multiplane"""
        self.sync_file._data = {"ophys_frames": np.array([0.1, 0.2, 0.3, 0.4])}
        ts = OphysTimestampsMultiplane.from_sync_file(sync_file=self.sync_file, group_count=2, plane_group=0).validate(
            number_of_frames=2
        )
    ...
    def test_process_ophys_plane_timestamps(self, timestamps, plane_group, group_count, expected):
        """Various test cases"""
        self.sync_file._data = {"ophys_frames": timestamps}
        number_of_frames = len(timestamps) if group_count == 0 else len(timestamps) / group_count
        if group_count == 0:
            ts = OphysTimestamps.from_sync_file(sync_file=self.sync_file)
        else:
            ts = OphysTimestampsMultiplane.from_sync_file(
                sync_file=self.sync_file, group_count=group_count, plane_group=plane_group
            )
