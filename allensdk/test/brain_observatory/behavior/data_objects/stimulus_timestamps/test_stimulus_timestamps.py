...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            # dict_repr
            {"behavior_stimulus_file": "mock_stimulus_file.pkl", "sync_file": "mock_sync_file.h5"},
            # has_pkl
            True,
            # has_sync
            True,
...
def test_stimulus_timestamps_from_json(monkeypatch, dict_repr, has_pkl, has_sync):
    mock_stimulus_file = create_autospec(BehaviorStimulusFile)
    mock_sync_file = create_autospec(SyncFile)

    mock_get_behavior_stimulus_timestamps = create_autospec(get_behavior_stimulus_timestamps)
    mock_get_ophys_stimulus_timestamps = create_autospec(get_ophys_stimulus_timestamps)
    ...
    with monkeypatch.context() as m:
        ...
        if has_pkl and has_sync:
            ...
            ts_from_sync = StimulusTimestamps.from_sync_file(sync_file=mock_sync_file_instance, monitor_delay=0.0)

    if has_pkl and has_sync:
        mock_get_ophys_stimulus_timestamps.assert_called_once_with(sync_path=mock_sync_file_instance.filepath)
        assert ts_from_sync._sync_file == mock_sync_file_instance
    else:
        assert ts_from_stim._stimulus_file == mock_stimulus_file_instance
        mock_get_behavior_stimulus_timestamps.assert_called_once_with(stimulus_pkl=mock_stimulus_file_instance.data)
...
def stimulus_file_fixture():
    ...
    return BehaviorStimulusFile.from_json(dict_repr={"behavior_stimulus_file": str(sf_path)})


def test_stimulus_timestamps_from_json2(stimulus_file_fixture):
    sf = stimulus_file_fixture
    stimulus_timestamps = StimulusTimestamps.from_stimulus_file(stimulus_file=sf, monitor_delay=0.0)
...
def test_stimulus_timestamps_from_json3(stimulus_file_fixture):
    ...
    expected = np.array([0.0, 0.0001, 0.0003, 0.0006, 0.001])
    np.testing.assert_array_almost_equal(stimulus_timestamps.value, expected, decimal=10)


@pytest.mark.parametrize("behavior_session_id, ophys_experiment_id", [(12345, None), (1234, 5678)])
def test_stimulus_timestamps_from_lims(monkeypatch, behavior_session_id, ophys_experiment_id):
    ...
    with monkeypatch.context() as m:
        ...
        mock_stimulus_file_instance = mock_stimulus_file.from_lims(mock_db_conn, behavior_session_id)
        ts_from_stim = StimulusTimestamps.from_stimulus_file(
            stimulus_file=mock_stimulus_file_instance, monitor_delay=0.0
        )
        assert ts_from_stim._stimulus_file == mock_stimulus_file_instance
        ...
        if behavior_session_id is not None and ophys_experiment_id is not None:
            mock_sync_file_instance = mock_sync_file.from_lims(mock_db_conn, ophys_experiment_id)
            ts_from_sync = StimulusTimestamps.from_sync_file(sync_file=mock_sync_file_instance, monitor_delay=0.0)
    ...
    if behavior_session_id is not None and ophys_experiment_id is not None:
        ...
        mock_stimulus_file.from_lims.assert_called_with(mock_db_conn, behavior_session_id)
        mock_get_behavior_stimulus_timestamps.assert_called_once_with(stimulus_pkl=mock_stimulus_file_instance.data)
...
@pytest.mark.parametrize(
    ...
    product((True, False), (np.arange(1, 6, 1), np.arange(6, 11, 1)), (0.1, 0.2)),
)
def test_stimulus_timestamps_nwb_roundtrip(
    nwbfile, data_object_roundtrip_fixture, roundtrip, raw_stimulus_timestamps_data, monitor_delay
):
    stimulus_timestamps = StimulusTimestamps(timestamps=raw_stimulus_timestamps_data, monitor_delay=monitor_delay)
    ...
    assert np.allclose(obt.value, raw_stimulus_timestamps_data + monitor_delay)


@pytest.fixture(scope="module")
def stimulus_timestamps_fixture(behavior_ecephys_session_config_fixture):
    """
    ...
    obj = StimulusTimestamps.from_multiple_stimulus_blocks(sync_file=sync_file, list_of_stims=[bsf, msf, rsf])
    return obj


@pytest.mark.requires_bamboo
@pytest.mark.parametrize("roundtrip", [True, False])
def test_read_write_nwb(roundtrip, data_object_roundtrip_fixture, stimulus_timestamps_fixture, helper_functions):
    nwbfile = helper_functions.create_blank_nwb_file()

    stimulus_timestamps_fixture.to_nwb(nwbfile=nwbfile)

    if roundtrip:
        obt = data_object_roundtrip_fixture(nwbfile=nwbfile, data_object_cls=StimulusTimestamps)
    else:
...
def test_substract_monitor_delay():
    ...
    monitor_delay = 0.57
    original_ts = StimulusTimestamps(timestamps=timestamps, monitor_delay=monitor_delay)

    np.testing.assert_array_equal(original_ts.value, timestamps + monitor_delay)
