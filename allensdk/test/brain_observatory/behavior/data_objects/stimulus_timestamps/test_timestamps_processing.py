...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            # pkl_data
            {"items": {"behavior": {"intervalsms": np.array([1000, 1001, 1002, 1003, 1004, 1005])}}},
            # expected
            np.array([0.0, 1.0, 2.001, 3.003, 4.006, 5.01, 6.015]),
        ),
...
@pytest.mark.parametrize(
    "sync_path, expected_timestamps",
    [
        ("/tmp/mock_sync_file.h5", [1, 2, 3]),
    ],
)
def test_get_ophys_stimulus_timestamps(monkeypatch, sync_path, expected_timestamps):
    mock_ophys_time_aligner = create_autospec(OphysTimeAligner)
    mock_aligner_instance = mock_ophys_time_aligner.return_value
    property_mock = PropertyMock(return_value=(expected_timestamps, "ignored_return_val"))
...
@pytest.mark.parametrize(
    ...
    [
        (np.arange(0, 1, 0.11), np.array([0.22, 0.13, 0.32, 0.77]), np.array([2, 1, 2, 7])),
        (np.arange(0, 1, 0.11), np.array([-0.1, 2.1, 0.55, 0.42]), np.array([0, 9, 5, 3])),
        (np.array([0.11, 0.11, 0.33, 0.33, 0.44]), np.array([0.05, 0.12, 0.22, 0.33, 0.77]), np.array([0, 1, 1, 2, 4])),
    ],
)
def test_get_frame_indices(frame_timestamps, event_timestamps, expected_indices):
    """
    Test that get_frame_indices correctly associates events
    with frames
    """
    actual = get_frame_indices(frame_timestamps=frame_timestamps, event_timestamps=event_timestamps)
...
def test_get_frame_indices_error():
    ...
    with pytest.raises(ValueError, match="frame_timestamps are not in ascending order"):
        get_frame_indices(frame_timestamps=frame_timestamps, event_timestamps=event_timestamps)
