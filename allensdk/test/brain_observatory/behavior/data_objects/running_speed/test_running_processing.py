...
@pytest.mark.parametrize("speed,expected", [(np.array([1.0]), [5.5033]), (np.array([0.0, 2.0]), [0.0, 11.0066])])
def test_deg_to_dist(speed, expected):
    np.testing.assert_allclose(deg_to_dist(speed), expected, atol=0.0001)


@pytest.mark.parametrize("lowpass", [True, False])
def test_get_running_df(running_data, timestamps, lowpass):
    ...
    np.testing.assert_array_equal(actual["v_sig"].values, running_data["items"]["behavior"]["encoders"][0]["vsig"])
    np.testing.assert_array_equal(actual["v_in"].values, running_data["items"]["behavior"]["encoders"][0]["vin"])
    np.testing.assert_array_equal(actual["dx"].values, running_data["items"]["behavior"]["encoders"][0]["dx"])
    if lowpass:
        assert np.count_nonzero(np.isnan(actual["speed"])) == 0


@pytest.mark.parametrize("lowpass", [True, False])
def test_get_running_df_one_fewer_timestamp_check_warning(running_data, timestamps, lowpass):
    with pytest.warns(UserWarning, match="Time array is 1 value shorter than encoder array.*"):
        # Call with one fewer timestamp, check for a warning
        _ = get_running_df(data=running_data, time=timestamps[:-1], lowpass=lowpass)


@pytest.mark.parametrize("lowpass", [True, False])
def test_get_running_df_one_fewer_timestamp_check_truncation(running_data, timestamps, lowpass):
    # Call with one fewer timestamp
    output = get_running_df(data=running_data, time=timestamps[:-1], lowpass=lowpass)
...
@pytest.mark.parametrize(
    ...
    [
        ([1, 2, 3], 1, None, np.array([np.nan, 1.0, 2.0])),
        ([1, 2, 3], 2, 99.0, np.array([99.0, 99.0, 1.0])),
        ([1, 2, 3], 3, 99.0, np.array([99.0, 99.0, 99.0])),
        ([1, 2, 3], 4, 99.0, np.array([99.0, 99.0, 99.0])),
        ([], 2, 30, np.array([])),
    ],
)
...
def test_identify_wraps(arr, min_threshold, max_threshold, expected):
    actual = _identify_wraps(arr, min_threshold=min_threshold, max_threshold=max_threshold)
    np.testing.assert_array_equal(
        actual[0], expected[0], f"error identifying positive wraps, got {actual[0]}, expected {expected[0]}"
    )
    np.testing.assert_array_equal(
        actual[1], expected[1], f"error identifying negative wraps, got {actual[1]}, expected {expected[1]}"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (  # Some diff artifacts, baseline
            ...
            5.1,
            2.0,
            np.array([np.nan, 1, 3, 5, 5.5, 6, 7.5, np.nan, 7.5, 8.5, 6.5, np.nan]),
        ),
...
def test_unwrap_voltage_signal(vsig, pos_wrap_ix, neg_wrap_ix, vmax, max_threshold, max_diff, expected):
    actual = _unwrap_voltage_signal(
        vsig, pos_wrap_ix, neg_wrap_ix, vmax=vmax, max_threshold=max_threshold, max_diff=max_diff
    )
    np.testing.assert_array_equal(actual, expected)
...
@pytest.mark.parametrize(
    "vsig, vmax, expected",
    [
        (np.array([1, 2, 3, 4, 5]), 2.0, np.array([np.nan, np.pi, np.pi, np.pi, np.pi])),
        (
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (np.array([99, 1, np.nan, 1, 1, 1]), 1.5, np.array([np.nan, 1, np.nan, 1, 1, 1])),
        (
            np.ones(
                1,
            ),
...
def test_clip_speed_wraps(speed, time, wrap_indices, span, expected, monkeypatch):
...
@pytest.mark.parametrize(
    "time, index, span",
    [
        (np.arange(10.0), 0, 2.0),  # no neighborhood before first point
        (np.arange(10.0), 9, 2.0),  # no neighborhood after last point
        (np.arange(10.0), 4, 0.25),  # data not sampled with enough frequency
    ],
)
...
@pytest.mark.parametrize(
    "time, index",
    [
        (np.array([5.0, 4.0, 3.0, 4.0, 5.0]), 2),
        (np.array([1.0, 2.0, 3.0, 2.0, 1.0]), 2),
        (np.array([3.0, 3.0, 3.0, 2.0, 1.0]), 2),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        (np.arange(10.0), 4, 2.0, (2.0, 6.0)),  # Spans > 1 element +/-
        (np.arange(10.0), 2, 1.0, (1.0, 3.0)),  # Spans = 1 element +/-
    ],
)
