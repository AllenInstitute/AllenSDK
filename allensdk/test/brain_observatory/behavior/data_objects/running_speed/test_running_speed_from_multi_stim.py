...
from allensdk.brain_observatory.behavior.data_objects.running_speed.running_speed import RunningSpeed

from allensdk.brain_observatory.behavior.data_objects.running_speed.running_acquisition import RunningAcquisition
...
def test_vbn_running_speed_round_trip(
    ...
    nwb_path = pathlib.Path(tempfile.mkstemp(dir=tmpdir, suffix=".nwb")[1])
    ...
    nwbfile = NWBFile(session_description="running", identifier="00001", session_start_time=start_time)
    ...
    with NWBHDF5IO(nwb_path, "r") as in_file:
        new_obj = RunningSpeed.from_nwb(nwbfile=in_file.read(), filtered=filtered)
...
def test_vbn_running_acq_round_trip(
    ...
    with NWBHDF5IO(nwb_path, "r") as in_file:
        new_obj = RunningAcquisition.from_nwb(nwbfile=in_file.read())
