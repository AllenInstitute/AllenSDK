...
def stimulus_file_frame_fixture(tmp_path_factory, helper_functions):
    """
    ...
    pkl_path_lookup["behavior"] = pathlib.Path(tempfile.mkstemp(dir=tmpdir, suffix=".pkl")[1])

    pkl_path_lookup["mapping"] = pathlib.Path(tempfile.mkstemp(dir=tmpdir, suffix=".pkl")[1])

    pkl_path_lookup["replay"] = pathlib.Path(tempfile.mkstemp(dir=tmpdir, suffix=".pkl")[1])

    frame_count_lookup = {"behavior": 13, "mapping": 44, "replay": 76}

    data = {"items": {"behavior": {"intervalsms": list(range(frame_count_lookup["behavior"] - 1))}}}
    ...
    for key in pkl_path_lookup:
        helper_functions.windows_safe_cleanup(file_path=pkl_path_lookup[key])
...
def merge_data_fixture():
    ...
    for key in ("behavior", "mapping", "replay"):
        ...
        skipped = np.sort(rng.choice(np.arange(nt, dtype=int), n_skip, replace=False))
        dx[skipped] = 0.0

        df = pd.DataFrame(data={"dx": dx, "v_in": v_in, "v_sig": v_sig, "speed": speed})
...
@pytest.fixture(scope="session")
def pkl_tmp_dir_fixture(tmp_path_factory):
    """
    A directory where the temporary data files can be written
    """
    tmpdir = pathlib.Path(tmp_path_factory.mktemp("self_consistent_multi_stim"))
    return tmpdir


@pytest.fixture(scope="session")
def behavior_pkl_fixture(pkl_tmp_dir_fixture, helper_functions):
    """
    ...
    pkl_path = pathlib.Path(tempfile.mkstemp(dir=pkl_tmp_dir_fixture, prefix="behavior_", suffix=".pkl")[1])
    n_frames = 77

    n_skip = rng.integers(10, n_frames // 2, 1)[0]
    to_skip = rng.choice(np.arange(n_frames, dtype=int), n_skip, replace=False)
...
def behavior_stim_file_fixture(behavior_pkl_fixture):
    yield BehaviorStimulusFile.from_json(dict_repr={"behavior_stimulus_file": behavior_pkl_fixture["path_to_pkl"]})


@pytest.fixture(scope="session")
def mapping_pkl_fixture(pkl_tmp_dir_fixture, helper_functions):
    """
    ...
    pkl_path = pathlib.Path(tempfile.mkstemp(dir=pkl_tmp_dir_fixture, prefix="mapping_", suffix=".pkl")[1])
    n_frames = 53
...
def mapping_stim_file_fixture(mapping_pkl_fixture):
    yield MappingStimulusFile.from_json(dict_repr={"mapping_stimulus_file": mapping_pkl_fixture["path_to_pkl"]})


@pytest.fixture(scope="session")
def replay_pkl_fixture(pkl_tmp_dir_fixture, helper_functions):
    """
    ...
    pkl_path = pathlib.Path(tempfile.mkstemp(dir=pkl_tmp_dir_fixture, prefix="replay_", suffix=".pkl")[1])
    n_frames = 47
...
def replay_stim_file_fixture(replay_pkl_fixture):
    yield ReplayStimulusFile.from_json(dict_repr={"replay_stimulus_file": replay_pkl_fixture["path_to_pkl"]})
...
def sync_path_fixture(
    ...
    sync_path = pathlib.Path(tempfile.mkstemp(dir=pkl_tmp_dir_fixture, prefix="example_", suffix=".sync")[1])
    ...
    with h5py.File(sync_path, "w") as out_file:
        ...
        out_file.create_dataset("meta", data=json.dumps(metadata).encode("utf-8"))
...
def sync_file_fixture(sync_path_fixture):
    return SyncFile.from_json(dict_repr={"sync_file": str(sync_path_fixture.resolve().absolute())})
