...
from allensdk.brain_observatory.behavior.data_objects import RunningAcquisition, StimulusTimestamps
...
def test_nonzero_monitor_delay_acq():
    ...
    with pytest.raises(RuntimeError, match="should be no monitor delay"):
        RunningAcquisition(running_acquisition=4.0, stimulus_file=None, stimulus_timestamps=OtherTimestamps())
...
@pytest.mark.parametrize(
    ...
    [
        (
            # dict_repr
            {"behavior_stimulus_file": "mock_stimulus_file.pkl", "monitor_delay": 0.0},
            # returned_running_acq_df
            pd.DataFrame(
                {"timestamps": [1, 2], "speed": [3, 4], "dx": [5, 6], "v_sig": [7, 8], "v_in": [9, 10]}
            ).set_index("timestamps"),
            # expected_running_acq_df
            pd.DataFrame({"timestamps": [1, 2], "dx": [5, 6], "v_sig": [7, 8], "v_in": [9, 10]}).set_index(
                "timestamps"
            ),
        ),
    ],
)
def test_running_acquisition_from_json(monkeypatch, dict_repr, returned_running_acq_df, expected_running_acq_df):
    ...
    with monkeypatch.context() as m:
        m.setattr(
            "allensdk.brain_observatory.behavior.data_objects.running_speed.running_acquisition.BehaviorStimulusFile",
            mock_stimulus_file,
        )
        m.setattr(
            "allensdk.brain_observatory.behavior.data_objects.running_speed.running_acquisition.StimulusTimestamps",
            mock_stimulus_timestamps,
        )
        m.setattr(
            "allensdk.brain_observatory.behavior.data_objects.running_speed.running_acquisition.get_running_df",
            mock_get_running_df,
        )
        obt = RunningAcquisition.from_stimulus_file(behavior_stimulus_file=mock_stimulus_file)
...
@pytest.mark.parametrize(
    ...
    [
        (
            # expected_running_acq_df
            pd.DataFrame(
                {"timestamps": [2.0, 4.0], "dx": [10.0, 12.0], "v_sig": [14.0, 16.0], "v_in": [18.0, 20.0]}
            ).set_index("timestamps")
        ),
    ],
)
def test_running_acquisition_nwb_roundtrip(nwbfile, data_object_roundtrip_fixture, roundtrip, running_acq_data):
