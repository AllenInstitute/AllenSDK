...
@pytest.mark.parametrize("use_lowpass, zscore", product((True, False), (5.0, 10.0)))
def test_extract_dx_basic(basic_running_stim_file_fixture, use_lowpass, zscore, tmp_path_factory, helper_functions):
    """
    ...
    time_array = np.linspace(0.0, 10.0, n_time)
    ...
    expected = get_running_df(data=expected_stim, time=time_array, lowpass=use_lowpass, zscore_threshold=zscore)

    pkl_path = pathlib.Path(tempfile.mkstemp(dir=tmpdir, suffix=".pkl")[1])
...
def test_extract_dx_time_mismatch(basic_running_stim_file_fixture, tmp_path_factory, helper_functions):
    """
    ...
    n_time = len(stim_file["items"]["behavior"]["encoders"][0]["dx"])
    time_array = np.linspace(0.0, 10.0, n_time - 5)
...
def test_merge_dx_data(merge_data_fixture, start_frame):
    """
    Test that _merge_dx_data correctly merges dataframes
    """

    frame_times = np.linspace(0.0, 10.0, merge_data_fixture["n_timesteps"])

    (velocity_df, raw_df) = _merge_dx_data(
    ...
    assert set(velocity_df.columns) == set(["velocity", "net_rotation", "frame_indexes", "frame_time"])
    ...
    for in_key, out_key in zip(("dx", "speed"), ("net_rotation", "velocity")):
        expected = []
    ...
    global_kept = np.concatenate(
        [
            merge_data_fixture["behavior"]["kept_mask"],
            merge_data_fixture["mapping"]["kept_mask"],
            merge_data_fixture["replay"]["kept_mask"],
        ]
    )
    np.testing.assert_array_equal(frame_times[global_kept], velocity_df.frame_time.values)

    np.testing.assert_array_equal(
        np.arange(start_frame, merge_data_fixture["n_timesteps"] + start_frame, dtype=int)[global_kept],
        velocity_df.frame_indexes.values,
    )

    # check contents of raw_df
    for in_key, out_key in zip(("dx", "v_in", "v_sig"), ("dx", "vin", "vsig")):
        expected = []
        for pkl_key in ("behavior", "mapping", "replay"):
    ...
    np.testing.assert_array_equal(raw_df.frame_time.values, frame_times)
...
def test_multi_stim_running_df_from_raw_data(
    ...
    b_stim = BehaviorStimulusFile.from_json(dict_repr={"behavior_stimulus_file": behavior_pkl_fixture["path_to_pkl"]})

    r_stim = ReplayStimulusFile.from_json(dict_repr={"replay_stimulus_file": replay_pkl_fixture["path_to_pkl"]})

    m_stim = MappingStimulusFile.from_json(dict_repr={"mapping_stimulus_file": mapping_pkl_fixture["path_to_pkl"]})

    (velocities_df, raw_df) = multi_stim_running_df_from_raw_data(
