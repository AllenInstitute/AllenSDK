...
from allensdk.brain_observatory.behavior.data_objects.stimuli.util import calculate_monitor_delay
from allensdk.brain_observatory.behavior.data_objects.task_parameters import TaskParameters
...
class TestFromBehaviorStimulusFile(LimsTest):
    ...
    def test_from_stimulus_file(self):
        stimulus_file, stimulus_timestamps, licks, rewards, response_window_start = self._get_trial_table_data()

        trials = Trials.from_stimulus_file(
            stimulus_file=stimulus_file, stimulus_timestamps=stimulus_timestamps, licks=licks, rewards=rewards
        )
    ...
    def _get_trial_table_data(self, stimulus_file: Optional[BehaviorStimulusFile] = None):
        ...
        stimulus_timestamps = StimulusTimestamps.from_stimulus_file(stimulus_file=stimulus_file, monitor_delay=0.02115)

        stimulus_timestamps_no_delay = StimulusTimestamps.from_stimulus_file(
            stimulus_file=stimulus_file, monitor_delay=0.0
        )

        licks = Licks.from_stimulus_file(stimulus_file=stimulus_file, stimulus_timestamps=stimulus_timestamps_no_delay)
        rewards = Rewards.from_stimulus_file(
            stimulus_file=stimulus_file, stimulus_timestamps=stimulus_timestamps_no_delay
        )

        response_window_start = TaskParameters.from_stimulus_file(stimulus_file=stimulus_file).response_window_sec[0]

        return stimulus_file, stimulus_timestamps, licks, rewards, response_window_start
...
class TestMonitorDelay:
    ...
    def test_monitor_delay(self, monkeypatch, equipment_name):
        ...
        with monkeypatch.context() as ctx:
            ctx.setattr(OphysTimeAligner, "_get_monitor_delay", dummy_delay)
            md = calculate_monitor_delay(sync_file=self.sync_file, equipment=equipment)
    ...
    def test_monitor_delay_lookup(self, monkeypatch):
        ...
        with monkeypatch.context() as ctx:
            ctx.setattr(OphysTimeAligner, "_get_monitor_delay", dummy_delay)
            for equipment, expected in self.lookup_table_expected_values.items():
    ...
    def test_unkown_rig_name(self, monkeypatch):
        ...
        with monkeypatch.context() as ctx:
            ...
            with pytest.raises(RuntimeError):
                calculate_monitor_delay(sync_file=self.sync_file, equipment=equipment)
...
class TestNWB:
    ...
    def test_read_write_nwb(self, roundtrip, data_object_roundtrip_fixture):
        ...
        with patch.object(TaskParameters, "from_nwb", lambda nwbfile: create_autospec(TaskParameters, instance=True)):
            if roundtrip:
                obt = data_object_roundtrip_fixture(nwbfile=self.nwbfile, data_object_cls=Trials)
            else:
        ...
        _, _, _, _, response_window_start = test._get_trial_table_data()
