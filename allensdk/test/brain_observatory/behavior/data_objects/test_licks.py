...
from allensdk.brain_observatory.behavior.data_files import BehaviorStimulusFile, SyncFile
...
class TestFromBehaviorStimulusFile:
    ...
    def setup_class(cls):
        ...
        cls.stimulus_file = BehaviorStimulusFile(filepath=test_data_dir / "behavior_stimulus_file.pkl")
        cls.sync_file = SyncFile(filepath=test_data_dir / "sync.h5")
    ...
    def test_monitor_delay_error(self):
        ...
        with pytest.raises(RuntimeError, match="monitor_delay should be zero"):
            Licks.from_stimulus_file(stimulus_file=self.stimulus_file, stimulus_timestamps=timestamps)

    def test_from_stimulus_file(self):
        st = StimulusTimestamps.from_stimulus_file(stimulus_file=self.stimulus_file, monitor_delay=0.0)
        licks = Licks.from_stimulus_file(stimulus_file=self.stimulus_file, stimulus_timestamps=st)
    ...
    def test_from_stimulus_and_sync_file(self):
        ...
        licks = Licks.from_stimulus_file(stimulus_file=self.stimulus_file, stimulus_timestamps=lick_times)
    ...
    def test_from_stimulus_file2(self, tmpdir):
        ...
        """
        stimulus_filepath = self._create_test_stimulus_file(lick_events=[12, 15, 90, 136], tmpdir=tmpdir)
        ...
        licks = Licks.from_stimulus_file(stimulus_file=stimulus_file, stimulus_timestamps=timestamps)

        expected_dict = {"timestamps": [0.12, 0.15, 0.90, 1.36], "frame": [12, 15, 90, 136]}
        expected_df = pd.DataFrame(expected_dict)
        assert expected_df.columns.equals(licks.value.columns)
        np.testing.assert_array_almost_equal(
            expected_df.timestamps.to_numpy(), licks.value["timestamps"].to_numpy(), decimal=10
        )
        np.testing.assert_array_almost_equal(expected_df.frame.to_numpy(), licks.value["frame"].to_numpy(), decimal=10)
    ...
    def test_empty_licks(self, tmpdir):
        ...
        stimulus_filepath = self._create_test_stimulus_file(lick_events=[], tmpdir=tmpdir)
        stimulus_file = BehaviorStimulusFile.from_json(dict_repr={"behavior_stimulus_file": str(stimulus_filepath)})
        timestamps = StimulusTimestamps(timestamps=np.arange(0, 2.0, 0.01), monitor_delay=0.0)
        licks = Licks.from_stimulus_file(stimulus_file=stimulus_file, stimulus_timestamps=timestamps)
        ...
        expected_df = pd.DataFrame(expected_dict)
        assert expected_df.columns.equals(licks.value.columns)
        np.testing.assert_array_equal(expected_df.timestamps.to_numpy(), licks.value["timestamps"].to_numpy())
        np.testing.assert_array_equal(expected_df.frame.to_numpy(), licks.value["frame"].to_numpy())
    ...
    def test_get_licks_failure(self, tmpdir):
        ...
        with pytest.raises(IndexError):
            Licks.from_stimulus_file(stimulus_file=stimulus_file, stimulus_timestamps=timestamps)
    ...
    def _create_test_stimulus_file(lick_events, tmpdir):
        ...
        data = {
            "items": {"behavior": {"trial_log": trial_log, "lick_sensors": lick_events}},
        }
...
class TestNWB:
    ...
    def setup_class(cls):
        ...
        stimulus_file = BehaviorStimulusFile(filepath=test_data_dir / "behavior_stimulus_file.pkl")
        ts = StimulusTimestamps.from_stimulus_file(stimulus_file=stimulus_file, monitor_delay=0.0)
        cls.licks = Licks.from_stimulus_file(stimulus_file=stimulus_file, stimulus_timestamps=ts)
    ...
    def test_read_write_nwb(self, roundtrip, data_object_roundtrip_fixture):
        ...
        if roundtrip:
            obt = data_object_roundtrip_fixture(nwbfile=self.nwbfile, data_object_cls=Licks)
        else:
