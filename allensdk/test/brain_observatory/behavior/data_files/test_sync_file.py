...
from allensdk.brain_observatory.behavior.data_files.sync_file import _get_sync_file_query_template
...
def test_sync_file_from_json(monkeypatch, sync_file_fixture):
    ...
    with monkeypatch.context() as m:
        m.setattr("allensdk.brain_observatory.behavior.data_files.sync_file.get_sync_data", mock_get_sync_data)
...
@pytest.mark.parametrize(
    "sync_file_fixture, ophys_experiment_id",
    [({"sync_data": [2, 3, 4, 5]}, 12), ({"sync_data": [2, 3, 4, 5]}, 8)],
    indirect=["sync_file_fixture"],
)
def test_sync_file_from_lims(monkeypatch, sync_file_fixture, ophys_experiment_id):
    sync_path, sync_data = sync_file_fixture
    ...
    query = _get_sync_file_query_template(behavior_session_id=ophys_experiment_id)
