...
@pytest.mark.parametrize(
    "speed,expected",
    [(np.array([1.0]), [5.5033]), (np.array([0.0, 2.0]), [0.0, 11.0066])],
)
...
def test_get_running_df_one_fewer_timestamp_check_warning(
    ...
    with pytest.warns(
        UserWarning, match="Time array is 1 value shorter than encoder array.*"
    ):
        # Call with one fewer timestamp, check for a warning
        _ = get_running_df(
            data=running_data, time=timestamps[:-1], lowpass=lowpass
        )
...
@pytest.mark.parametrize(
    ...
    [
        (
            np.array([0, 2, 5, 0, 2, 0, 5]),  # pos wrap 5-0  # neg wrap 0-5
            1.5,
            3.5,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (  # Some diff artifacts, baseline
            ...
            2.0,
            np.array(
                [np.nan, 1, 3, 5, 5.5, 6, 7.5, np.nan, 7.5, 8.5, 6.5, np.nan]
            ),
        ),
...
def test_clip_speed_wraps(
    ...
    monkeypatch.setattr(
        rp, "_local_boundaries", lambda x, y, z: (y - 1, y + 1)
    )
