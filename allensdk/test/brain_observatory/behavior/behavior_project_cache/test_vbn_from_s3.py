...
def test_vbn_metadata_tables(tmpdir, vbn_s3_cloud_cache_data):
    ...
    create_bucket(bucket_name, project_name, data["data"], data["metadata"])
    ...
    abnormal = cache.get_ecephys_session_table(filter_abnormalities=False)
...
def test_probe_nwb_file(monkeypatch, tmpdir, vbn_s3_cloud_cache_data):
    ...
    for probe in probe_meta_table.itertuples():
        with monkeypatch.context() as ctx:
            ctx.setattr(BehaviorEcephysSession, "from_nwb_path", lambda path, probe_meta: probe_meta)
            probe_meta = cache.get_ecephys_session(ecephys_session_id=probe.ecephys_session_id)

        probe_id = probe.Index
        probe_nwb = probe_meta[probe.name].lfp_csd_filepath()
        expected_path = cache_dir / f"{project_name}-0.{len(data)}.0" / "data" / f"probe_{probe_id}_lfp.nwb"
...
def test_manifest_methods(tmpdir, vbn_s3_cloud_cache_data):
    ...
    for mname in ("behavior_sessions", "ecephys_sessions", "probes"):
        print(change_msg)
...
@mock_s3
def test_local_cache_construction(tmpdir, vbn_s3_cloud_cache_data, monkeypatch):
    data, versions = vbn_s3_cloud_cache_data
    ...
    with monkeypatch.context() as ctx:
        ctx.setattr(
            ...
            "from_nwb_path",
            lambda path, probe_meta: create_autospec(BehaviorEcephysSession, instance=True),
        )
...
@mock_s3
def test_load_out_of_date_manifest(tmpdir, vbn_s3_cloud_cache_data, monkeypatch):
    """
    ...
    for sess_id in (333, 444):
        with monkeypatch.context() as ctx:
            ctx.setattr(BehaviorSession, "from_nwb_path", lambda path: create_autospec(BehaviorSession, instance=True))
...
def test_file_linkage(tmpdir, vbn_s3_cloud_cache_data, delete_cache, monkeypatch):
    """
...
def test_when_data_updated(tmpdir, vbn_s3_cloud_cache_data, data_update):
    ...
    load_dataset(data_update["data"], data_update["metadata"], later_version, bucket_name, project_name, client)
