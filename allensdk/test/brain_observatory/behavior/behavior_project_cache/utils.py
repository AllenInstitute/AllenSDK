...
def load_dataset(
    ...
    for fname in data_blobs:
        client.put_object(Bucket=bucket_name, Key=f"{project_name}/data/{fname}", Body=data_blobs[fname]["data"])

    if metadata_blobs is not None:
        for fname in metadata_blobs:
            client.put_object(
                Bucket=bucket_name, Key=f"{project_name}/project_metadata/{fname}", Body=metadata_blobs[fname]
            )
...
def create_bucket(test_bucket_name: str, project_name: str, datasets: dict, metadatasets: dict) -> None:
    """
    ...
    for v in datasets.keys():
        if metadatasets is not None:
            ...
            m = None
        load_dataset(datasets[v], m, v, test_bucket_name, project_name, client)
