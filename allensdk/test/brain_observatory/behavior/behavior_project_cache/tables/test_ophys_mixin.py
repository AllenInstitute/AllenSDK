...
from allensdk.brain_observatory.behavior.behavior_project_cache.tables.ophys_mixin import OphysMixin


class TestOhysMixin:
    def test__merge_column_values_nothing_to_merge(self):
        """when nothing to merge, df stays same"""
        df = pd.DataFrame({"date_of_acquisition": [1, 2], "foo": [3, 4]})
    ...
    def test__merge_column_values(self):
        ...
        expected = pd.DataFrame({"date_of_acquisition": [3, 4], "session_type": ["foo", "baz"], "foo": [1, 2]})
        pd.testing.assert_frame_equal(expected.sort_index(axis=1), mixin._df.sort_index(axis=1))
