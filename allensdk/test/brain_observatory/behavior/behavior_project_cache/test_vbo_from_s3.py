...
def test_manifest_methods(tmpdir, vbo_s3_cloud_cache_data):
    ...
    for mname in ("behavior_session_table", "ophys_session_table", "ophys_experiment_table"):
...
@mock_s3
def test_local_cache_construction(tmpdir, vbo_s3_cloud_cache_data, monkeypatch):
    data, versions = vbo_s3_cloud_cache_data
    ...
    with monkeypatch.context() as ctx:
        ctx.setattr(
            ...
            "from_nwb_path",
            lambda nwb_path: create_autospec(BehaviorOphysExperiment, instance=True),
        )
...
@mock_s3
def test_load_out_of_date_manifest(tmpdir, vbo_s3_cloud_cache_data, monkeypatch):
    """
    ...
    for sess_id in (333, 444):
        with monkeypatch.context() as ctx:
            ctx.setattr(
                BehaviorSession, "from_nwb_path", lambda nwb_path: create_autospec(BehaviorSession, instance=True)
            )
    ...
    expected = {"ophys_file_1.nwb", "ophys_file_2.nwb", "behavior_file_3.nwb", "behavior_file_4.nwb"}
...
def test_file_linkage(tmpdir, vbo_s3_cloud_cache_data, delete_cache, monkeypatch):
    """
    ...
    # check symlinks
    for name in ("ophys_file_2.nwb", "behavior_file_3.nwb", "behavior_file_4.nwb"):
