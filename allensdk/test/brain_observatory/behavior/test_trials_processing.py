...
def test_calculate_reward_rate(kwargs, expected):
    with patch.object(Trials, "_calculate_response_latency_list", wraps=lambda: kwargs["response_latency"]):
        with patch.object(Trials, "start_time", new_callable=PropertyMock) as mock_start_time:
            ...
            reward_rate = trials.calculate_reward_rate(
                trial_window=kwargs["trial_window"], initial_trials=kwargs["initial_trials"]
            )
...
def trial_data_and_expectation_1():
    test_trial = {
        "index": 4,
        "cumulative_rewards": 1,
        "licks": [(324.1935569847751, 19091), (324.34329131981696, 19100), (324.49368158882305, 19109)],
        "stimulus_changes": [],
        "success": False,
        "cumulative_volume": 0.005,
        ...
        "rewards": [],
        "events": [
            ["trial_start", "", 322.2688823113451, 18976],
...
@pytest.mark.parametrize(
    ...
    [
        (
            pd.DataFrame({"change_time": [1, 2, 3, 4], "lick_times": [[1.1], [2.1, 2.2], [3.3, 3.4], [4.4]]}),
            0.0,
            [0.1, 0.1, 0.3, 0.4],
        ),
        (
            pd.DataFrame({"change_time": [1, 2, 3, 4], "lick_times": [[1.1], [], [3.3, 3.4], [4.4]]}),
            0.0,
...
def test_calculate_response_latency_list(trials, response_window_start, expected):
...
def trials_example():
    ...
    trials_dict = {
        "start_time": {8: 368.305066913832, 9: 378.0631642451044, 10: 386.31999971927144, 11: 394.57686825376004},
        "change_time": {8: 1, 9: 2, 10: 3, 11: 4},
        "lick_times": {8: np.array([]), 9: np.array([]), 10: np.array([]), 11: np.array([])},
...
def test_construct_rolling_performance_df(trials_example, is_passive):
    ...
    trials_example["reward_volume"] = 0 if is_passive else [0, 0.25, 0, 0]
