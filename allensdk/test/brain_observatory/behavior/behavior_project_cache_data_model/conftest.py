...
from allensdk.brain_observatory.behavior.data_objects.metadata.subject_metadata.full_genotype import FullGenotype
...
def session_type_lookup(behavior_session_id_list):
    ...
    possible = ("TRAINING_1_gratings", "OPHYS_1_images_A", "OPHYS_1_images_B")

    vals = rng.choice(possible, size=len(behavior_session_id_list), replace=True)

    return {ii: vv for ii, vv in zip(behavior_session_id_list, vals)}
...
def genotype_lookup(behavior_session_id_list):
    ...
    possible = ("foo-SlcCre", "Vip-IRES-Cre/wt;Ai148(TIT2L-GC6f-ICL-tTA2)/wt", "bar", "foobar")
    chosen = rng.choice(possible, size=len(behavior_session_id_list), replace=True)
    return {ii: val for ii, val in zip(behavior_session_id_list, chosen)}
...
@pytest.fixture(scope="session")
def behavior_session_data_fixture(
...
def behavior_session_table(behavior_session_data_fixture):
    ...
    df = pd.DataFrame(data, index=pd.Index(index, name="behavior_session_id"))
    return df
...
@pytest.fixture(scope="session")
def ophys_session_data_fixture(
    project_code_lookup,
    session_name_lookup,
    date_of_acquisition_lookup,
    specimen_id_lookup,
...
def ophys_session_table(ophys_session_data_fixture):
    ...
    df = pd.DataFrame(data, index=pd.Index(index, name="ophys_session_id"))
    return df


@pytest.fixture(scope="session")
def ophys_experiment_data_fixture(
    ophys_session_data_fixture, experiment_state_lookup, container_state_lookup, ophys_experiment_to_container_map
):
    """
    List of dicts.
...
def ophys_experiments_table(ophys_experiment_data_fixture):
    ...
    df = pd.DataFrame(data, index=pd.Index(index, name="ophys_experiment_id"))
    return df


@pytest.fixture()
def intermediate_behavior_table(behavior_session_table, mock_api):
    """
    ...
    df["reporter_line"] = df["reporter_line"].apply(ReporterLine.parse)
    df["cre_line"] = df["full_genotype"].apply(lambda x: FullGenotype(full_genotype=x).parse_cre_line())
    df["indicator"] = df["reporter_line"].apply(lambda x: ReporterLine(reporter_line=x).parse_indicator())

    df["prior_exposures_to_session_type"] = get_prior_exposures_to_session_type(df=df)
    df["prior_exposures_to_image_set"] = get_prior_exposures_to_image_set(df=df)
    df["prior_exposures_to_omissions"] = get_prior_exposures_to_omissions(df=df, fetch_api=mock_api)
    return df


@pytest.fixture()
def expected_behavior_session_table(
    ...
    for ophys_session in ophys_session_data_fixture:
        ...
        for exp_id in ophys_session["ophys_experiment_id"]:
            ...
            for container_id in ophys_experiment_to_container_map[exp_id]:
                is_published = container_state_lookup[container_id] == "published"
...
@pytest.fixture()
def expected_experiments_table(
    ophys_experiments_table, container_state_lookup, experiment_state_lookup, intermediate_behavior_table, request
):
    """
    ...
    expected = expected.join(behavior_table[["session_name"]], on="behavior_session_id", rsuffix="_behavior")
...
@pytest.fixture()
def expected_ophys_session_table(
    ophys_session_table,
    intermediate_behavior_table,
    container_state_lookup,
    experiment_state_lookup,
    ...
    expected = expected.join(
        behavior_table[["specimen_id", "session_name"]], on="behavior_session_id", rsuffix="_behavior", lsuffix="_ophys"
    )
...
@pytest.fixture
def mock_api(ophys_session_table, behavior_session_table, ophys_experiments_table):
    class MockApi:
...
def TempdirBehaviorCache(mock_api, request):
    ...
    yield VisualBehaviorOphysProjectCache(fetch_api=mock_api(), cache=request.param, manifest=manifest)
    temp_dir.cleanup()
