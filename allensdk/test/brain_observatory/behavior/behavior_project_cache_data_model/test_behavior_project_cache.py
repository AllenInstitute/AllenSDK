...
def test_session_table_reads_from_cache(TempdirBehaviorCache, caplog):
...
@pytest.mark.parametrize("TempdirBehaviorCache", [True], indirect=True)
def test_behavior_table_reads_from_cache(TempdirBehaviorCache, caplog):
    caplog.set_level(logging.INFO, logger="call_caching")
    cache = TempdirBehaviorCache
...
def test_get_ophys_session_table_by_experiment(TempdirBehaviorCache, expected_ophys_session_table):
    raw = expected_ophys_session_table["df"][["ophys_experiment_id"]]
    data = []
    for session_id, exp_id_list in zip(raw.index.values, raw.ophys_experiment_id.values):
        for exp_id in exp_id_list:
            data.append({"ophys_session_id": session_id, "ophys_experiment_id": exp_id})

    expected = pd.DataFrame(data).set_index("ophys_experiment_id")

    actual = TempdirBehaviorCache.get_ophys_session_table(index_column="ophys_experiment_id")[["ophys_session_id"]]
...
def test_cloud_manifest_errors(TempdirBehaviorCache):
    ...
    with pytest.raises(NotImplementedError, match=msg.format(mname="construct_local_manifest")):
        TempdirBehaviorCache.construct_local_manifest()

    with pytest.raises(NotImplementedError, match=msg.format(mname="compare_manifests")):
        TempdirBehaviorCache.compare_manifests("a", "b")

    with pytest.raises(NotImplementedError, match=msg.format(mname="load_latest_manifest")):
        TempdirBehaviorCache.load_latest_manifest()

    this_msg = msg.format(mname="latest_downloaded_manifest_file")
    with pytest.raises(NotImplementedError, match=this_msg):
        TempdirBehaviorCache.latest_downloaded_manifest_file()

    with pytest.raises(NotImplementedError, match=msg.format(mname="latest_manifest_file")):
        TempdirBehaviorCache.latest_manifest_file()

    with pytest.raises(NotImplementedError, match=msg.format(mname="load_manifest")):
        TempdirBehaviorCache.load_manifest("a")

    with pytest.raises(NotImplementedError, match=msg.format(mname="current_manifest")):
