...
def pytest_assertrepr_compare(config, op, left, right):
    if isinstance(left, WhitespaceStrippedString) and op == "==":
        if isinstance(right, WhitespaceStrippedString):
            ...
            right_compare = right
        return ["Comparing strings with whitespace stripped. ", f"{left.orig} != {right_compare}.", "Diff:"] + left.diff
...
def behavior_stimuli_data_fixture(request):
    ...
    """
    images_set_log = request.param.get("images_set_log", [("Image", "im065", 5.809, 0)])
    images_draw_log = request.param.get("images_draw_log", [([0] + [1] * 3 + [0] * 3)])
    grating_set_log = request.param.get("grating_set_log", [("Ori", 90, 3.585, 0)])
    grating_draw_log = request.param.get("grating_draw_log", [([0] + [1] * 3 + [0] * 3)])
    omitted_flash_frame_log = request.param.get("omitted_flash_frame_log", {"grating_0": []})
    grating_phase = request.param.get("grating_phase", None)
    grating_spatial_frequency = request.param.get("grating_spatial_frequency", None)
    ...
    image_data = {
        ...
        "image_path": os.path.join(resources_dir, "stimulus_template", "input", "test_image_set.pkl"),
    }
    ...
    data = {"items": {"behavior": {"stimuli": {}, "omitted_flash_frame_log": omitted_flash_frame_log}}}
