...
@pytest.mark.parametrize(
    ...
    [
        (
            pd.DataFrame(
                {"trial_type": ["go", "go", "catch", "catch", "aborted"], "detect": [True, False, True, True, True]}
            ),
            "detect",
            ["go", "catch"],
            0.75,
...
def test_response_bias(trials, detect_col, trial_types, expected):
    assert metrics.response_bias(trials, detect_col, trial_types) == pytest.approx(expected, nan_ok=True)
