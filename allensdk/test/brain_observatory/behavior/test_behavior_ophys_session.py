...
def test_session_from_json(
    tmpdir_factory, session_data, get_expected, get_from_session
):
...
def test_nwb_end_to_end(tmpdir_factory):
    ...
    nwb_filepath = os.path.join(
        str(tmpdir_factory.mktemp("test_nwb_end_to_end")), "nwbfile.nwb"
    )
...
def test_visbeh_ophys_data_set():
    ...
    assert data_set.api.extractor.get_foraging_id() == str(
        data_set.api.get_behavior_session_uuid()
    )
    ...
    assert len(data_set.corrected_fluorescence_traces) == 269 and sorted(
        data_set.corrected_fluorescence_traces.columns
    ) == ["cell_roi_id", "corrected_fluorescence"]
    np.testing.assert_array_almost_equal(
        data_set.running_speed.timestamps, data_set.stimulus_timestamps
    )
...
@pytest.mark.requires_bamboo
@pytest.mark.parametrize(
    "ophys_experiment_id", [pytest.param(789359614), pytest.param(792813858)]
)
...
@pytest.mark.parametrize(
    ...
    [(5, 9, None), (1, 2, None), (3, 3, pd.DataFrame([5, 6, 7]))],
)
