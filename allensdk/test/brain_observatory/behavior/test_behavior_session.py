...
def test_nwb_end_to_end_session(tmpdir_factory, helper_functions):
    session_id = 870987812
    tmpdir = tmpdir_factory.mktemp("session_nwb_end_to_end")
    tmpdir = pathlib.Path(tmpdir)
    nwb_path = tmpdir / f"session_{session_id}.nwb"
    session = BehaviorSession.from_lims(behavior_session_id=session_id)
    nwb_file = session.to_nwb()
    with NWBHDF5IO(nwb_path, "w") as nwb_file_writer:
        nwb_file_writer.write(nwb_file)

    roundtrip = BehaviorSession.from_nwb_path(nwb_path=str(nwb_path.resolve().absolute()))

    assert sessions_are_equal(session, roundtrip, reraise=True)
...
def session_data_fixture():
    return {
        ...
        "behavior_stimulus_file": "/allen/programs/braintv/production/visualbehavior/prod0/"
...
class TestBehaviorSession(LimsTest):
    ...
    def test_eye_tracking_loaded_with_metadata_frame(self):
        ...
        sess = BehaviorSession.from_lims(behavior_session_id=sess_id, lims_db=self.dbconn)
