...
@pytest.mark.parametrize("vs_times, expected", [[[0.016, 0.033, 0.051, 0.067, 3.0], [0.016, 0.033, 0.051, 0.067]]])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            [0.08, 0.090, 0.100, 0.110],
        ),
    ],
    indirect=["mock_dataset_fixture"],
)
def test_get_synchronized_frame_times(monkeypatch, mock_dataset_fixture, sync_line_label_keys, expected):
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"eye_tracking_timings": [0.020, 0.030, 0.040, 0.050, 3.0]},
            Dataset.EYE_TRACKING_KEYS,
            [0.020, 0.030, 0.040, 0.050, 3.0],
        ),
        (
            {"behavior_tracking_timings": [0.080, 0.090, 0.100, 0.110, 8.0]},
            Dataset.BEHAVIOR_TRACKING_KEYS,
            [0.08, 0.090, 0.100, 0.110, 8.0],
        ),
    ],
    indirect=["mock_dataset_fixture"],
)
def test_get_synchronized_frame_times_no_trim(monkeypatch, mock_dataset_fixture, sync_line_label_keys, expected):
    monkeypatch.setattr(su, "Dataset", mock_dataset_fixture)
...
@pytest.mark.parametrize(
    "mock_dataset_fixture,sync_line_label_keys,expected",
    [
        ({"eye_tracking_timings": [0.020, 0.030, 3.0, 0.040, 0.050, 0.040]}, Dataset.EYE_TRACKING_KEYS, [0.020, 0.030]),
        (
...
@pytest.mark.parametrize(
    "mock_dataset_fixture,sync_line_label_keys,expected",
    [
        ({"eye_tracking_timings": [3.0, 0.030, 0.040, 0.050]}, Dataset.EYE_TRACKING_KEYS, []),
        ({"behavior_tracking_timings": [8.0, 0.080, 0.090, 0.100, 0.110]}, Dataset.BEHAVIOR_TRACKING_KEYS, []),
    ],
    indirect=["mock_dataset_fixture"],
)
def test_get_synchronized_frame_times_trim_all(monkeypatch, mock_dataset_fixture, sync_line_label_keys, expected):
    monkeypatch.setattr(su, "Dataset", mock_dataset_fixture)
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            Dataset.EYE_TRACKING_KEYS,
            [0.020, 0.030, 3.0, 0.050, 0.040],
        ),
        (
            {"behavior_tracking_timings": [0.080, 8.0, 0.090, 0.100, 0.110]},
            Dataset.BEHAVIOR_TRACKING_KEYS,
            [0.080, 8.0, 0.090, 0.100, 0.110],
        ),
    ],
    indirect=["mock_dataset_fixture"],
)
...
@pytest.mark.parametrize(
    "mock_dataset_fixture,sync_line_label_keys,expected",
    [
        ({"eye_tracking_timings": [0.020, 0.030, 0.040, 0.050, 3.0]}, Dataset.EYE_TRACKING_KEYS, [0.020, 0.030, 0.050]),
        (
            {"behavior_tracking_timings": [0.080, 0.090, 0.100, 0.110, 8.0]},
            Dataset.BEHAVIOR_TRACKING_KEYS,
            [0.080, 0.090, 0.110],
...
def test_get_synchronized_frame_times_drop_frame(monkeypatch, mock_dataset_fixture, sync_line_label_keys, expected):
    monkeypatch.setattr(su, "Dataset", mock_dataset_fixture)

    obtained = su.get_synchronized_frame_times("dummy_path", sync_line_label_keys, drop_frames=[2])
    assert np.allclose(obtained, expected)
