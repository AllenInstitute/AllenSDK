...
from allensdk.brain_observatory.sync_stim_aligner import _get_start_frames, get_stim_timestamps_from_stimulus_blocks
...
@pytest.fixture
def line_to_edges_fixture(sync_sample_fixture):
    ...
    for line_name in ("lineA", "lineB"):
        ...
        result[line_name] = {"rising_idx": np.array(this_rising), "falling_idx": np.array(this_falling)}
    ...
    result["stim_running"] = {"rising_idx": np.array(this_rising), "falling_idx": np.array(this_falling)}
    ...
    result["vsync_stim"] = {"rising_idx": v_rising, "falling_idx": v_falling}

    return result


@pytest.fixture
def expected_start_frames_fixture(line_to_edges_fixture):
    """
...
def test_get_start_frames_exact(sync_file_fixture, line_to_edges_fixture, sync_sample_fixture, sync_freq_fixture):
    """
    ...
    np.testing.assert_array_equal(start_frames, line_to_edges_fixture["stim_running"]["rising_idx"])


@pytest.mark.parametrize("stimulus_frame_counts, expected", [([33, 66, 132], [12, 100, 500]), ([66, 132], [100, 500])])
def test_get_start_frames_skip_one(
    sync_file_fixture, sync_sample_fixture, sync_freq_fixture, stimulus_frame_counts, expected
):
    """
    ...
    np.testing.assert_array_equal(start_frames, expected)


@pytest.mark.parametrize(
    "tolerance, stimulus_frame_counts, expected",
    [(0.1, [35, 61, 127], [12, 100, 500]), (0.05, [32, 68, 136], [12, 100, 500]), (0.05, [67, 96], [100, 204])],
)
def test_get_start_frames_tolerance(
    sync_file_fixture, sync_sample_fixture, sync_freq_fixture, tolerance, stimulus_frame_counts, expected
):
    """
...
@pytest.mark.parametrize("tolerance, stimulus_frame_counts", [(0.1, [32, 40, 99]), (0.05, [33, 66, 300])])
def test_get_start_frames_tolerance_failures(
    sync_file_fixture, line_to_edges_fixture, sync_sample_fixture, sync_freq_fixture, tolerance, stimulus_frame_counts
):
    """
...
def test_get_start_frames_too_many_pkl(sync_file_fixture, sync_sample_fixture, sync_freq_fixture):
    """
...
def test_user_facing_get_stims_error(sync_file_fixture):
    """
    ...
    """
    with pytest.raises(ValueError, match="Cannot parse raw_frame_time_direction"):
        get_stim_timestamps_from_stimulus_blocks(
            stimulus_files=[DummyStim(n_frames=2), DummyStim(n_frames=4)],
...
@pytest.mark.parametrize("edge_type", ["rising", "falling"])
def test_user_facing_get_stim_timestamps_smoke(
    ...
    stim_list = [DummyStim(n_frames=33), DummyStim(n_frames=66), DummyStim(n_frames=99), DummyStim(n_frames=132)]
    ...
    for ii, (this_array, this_start_frame, this_stim) in enumerate(
        zip(result["timestamps"], result["start_frames"], stim_list)
    ):
...
def test_user_facing_get_stim_timestamps(
    ...
    expected_start = [expected_start_frames_fixture[edge_type][idx] for idx in expected_idx]
