...
def test_choose_line(sync_file_fixture):
    """
...
def test_get_rising_times(
    sync_file_fixture, sync_freq_fixture, sync_sample_fixture, line_to_edges_fixture, specified_lines, expected_line
):
    """
    ...
    with SyncDataset(sync_file_fixture) as data:
        actual = _get_rising_times(data=data, sync_lines=specified_lines)
...
@pytest.mark.parametrize("specified_lines", ["lineZ", ("lineU", "lineW")])
def test_get_rising_times_exception(sync_file_fixture, specified_lines):
    """
    ...
    with SyncDataset(sync_file_fixture) as data:
        with pytest.raises(RuntimeError, match="Could not find one of"):
            _get_rising_times(data=data, sync_lines=specified_lines)
...
def test_get_falling_times(
    ...
    with SyncDataset(sync_file_fixture) as data:
        actual = _get_falling_times(data=data, sync_lines=specified_lines)
...
def test_get_line_starts_and_ends(
    ...
    """
    with SyncDataset(sync_file_fixture) as data:
        actual = _get_line_starts_and_ends(data=data, sync_lines=specified_lines)
...
@pytest.mark.parametrize("specified_lines", ["lineZ", ("lineU", "lineW")])
def test_get_falling_times_exception(sync_file_fixture, specified_lines):
    """
    ...
    with SyncDataset(sync_file_fixture) as data:
        with pytest.raises(RuntimeError, match="Could not find one of"):
            _get_falling_times(data=data, sync_lines=specified_lines)
