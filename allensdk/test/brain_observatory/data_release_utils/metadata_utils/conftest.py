...
def some_files_fixture(tmp_path_factory, helper_functions):
    """
    Create some temporary files; return a list of paths to them
    """
    tmpdir = pathlib.Path(tmp_path_factory.mktemp("id_generator"))
    path_list = []
    for idx in range(4):
...
@pytest.fixture
def metadata_table_fixture(some_files_fixture):
    """
    ...
    for i, file_path in enumerate(some_files_fixture):
        ...
        element = {"file_index": file_idx, "some_data": float(rng.random()), "session_id": i}
        input_data.append(element)

    # Not real files, never written
    input_data.append({"file_index": 11232, "some_data": float(rng.random()), "session_id": 11232})
    input_data.append({"file_index": 4455, "some_data": float(rng.random()), "session_id": 4455})
