...
def test_not_a_path_error(some_files_fixture):
    """
...
def test_symlink_error(some_files_fixture, tmp_path_factory, helper_functions):
    """
    ...
    generator = FileIDGenerator()
    with pytest.raises(ValueError, match="is a symlink; must be an actual path"):
        generator.id_from_path(file_path=symlink_path)
    helper_functions.windows_safe_cleanup(file_path=symlink_path)


def test_file_id_generator(some_files_fixture):
    """
