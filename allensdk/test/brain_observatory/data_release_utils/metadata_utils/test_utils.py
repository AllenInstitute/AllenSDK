...
from allensdk.brain_observatory.data_release_utils.metadata_utils.utils import add_file_paths_to_metadata_table


def test_add_file_paths_to_metadata_table_on_missing_error(some_files_fixture, metadata_table_fixture):
    """
    Test that an error is raised by add_file_paths_to_metadata_table
    when on_missing_file is a nonsense value
    """
    with pytest.raises(ValueError, match="on_missing_file must be one of"):
...
def test_add_file_paths_to_metadata_table_no_file_error(some_files_fixture, metadata_table_fixture):
    """
    Test that an error is raised by add_file_paths_to_metadata_table
    when files are missing (if requested)
    """
    with pytest.raises(RuntimeError, match="The following files do not exist"):
        add_file_paths_to_metadata_table(
            metadata_table=metadata_table_fixture,
            id_generator=FileIDGenerator(),
            file_dir=some_files_fixture[0].parent,
...
@pytest.mark.parametrize("on_missing_file", ["skip", "warn"])
def test_add_file_paths_to_metadata_table(some_files_fixture, metadata_table_fixture, on_missing_file):
    """
    ...
    with pytest.warns(UserWarning, match="The following files do not exist"):
    ...
    for file_idx in expected.file_index:
        ...
        if file_path.exists():
            ...
            expected = expected.drop(expected.loc[expected.file_index == file_idx].index)
