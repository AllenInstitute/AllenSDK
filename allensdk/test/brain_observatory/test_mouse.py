        ...
        return [
            BehaviorMetadata(
                date_of_acquisition=DateOfAcquisition(
                    date_of_acquisition=datetime.datetime(year=2022, month=12, day=day)
                ),
        ...
        d = {
            "items": {
                "behavior": {
                    "stimuli": {
                        # unused
                        "": {"set_log": [(stimulus_category, image_name, "", "") for _ in range(10)]}
                    }
                }
            }
        }
        with open(Path(self.tmpdir.name) / f"stim_{behavior_session_id}.pkl", "wb") as f:
            pickle.dump(d, f)
        return BehaviorStimulusFile(filepath=Path(self.tmpdir.name) / f"stim_{behavior_session_id}.pkl")

    @pytest.mark.parametrize("upto_behavior_session_id", (None, 1, 2, 6, 9))
    def test_images_shown(self, upto_behavior_session_id):
        with patch(
            "allensdk.internal.brain_observatory.mouse.db_connection_creator", wraps=lambda fallback_credentials: None
        ):
            with patch.object(Mouse, attribute="get_behavior_sessions", wraps=self.get_behavior_sessions):
                with patch.object(BehaviorStimulusFile, attribute="from_lims", wraps=self.get_behavior_stimulus_file):
                    obt = self.mouse.get_images_shown(up_to_behavior_session_id=upto_behavior_session_id, n_workers=1)
