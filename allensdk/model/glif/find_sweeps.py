...
def get_sweeps_by_name(sweeps, sweep_type):
    if isinstance(sweeps, dict):
        return [ s for sn,s in sweeps.iteritems() if s[u'ephys_stimulus'][u'ephys_stimulus_type'][u'name'] == sweep_type ]    
    else:
        return [ s for s in sweeps if s[u'ephys_stimulus'][u'ephys_stimulus_type'][u'name'] == sweep_type ]

def get_sweeps_by_name_ext(sweeps, sweep_name):
    if isinstance(sweeps, dict):
...
def find_ramp_sweeps(sweeps):
    ...
    if len(out['suprathreshold']) == 0:
        raise MissingSweepException("no suprathreshold ramp")
